 ...
 * @constructor
 */
export function IFPageGuide(guides) {
...
IFPageGuide.prototype.map = function (x, y) {
      ...
      for (var i = 0; i < sides.length; ++i) {
        ...
        if (resX === null && Math.abs(x - pivot.getX()) <= snapDistance) {
          ...
          if (sides[i] == IFRect.Side.CENTER) {
            guideX = [
              new IFPoint(resX, tl.getY()),
              new IFPoint(resX, br.getY()),
            ];
          }
        }
        if (resY === null && Math.abs(y - pivot.getY()) <= snapDistance) {
          ...
          if (sides[i] == IFRect.Side.CENTER) {
            guideY = [
              new IFPoint(tl.getX(), resY),
              new IFPoint(br.getX(), resY),
            ];
          }
        }
      }
  ...
  if (this._scene.getProperty("singlePage")) {
    ...
    for (
      ...
      child !== null && (resX === null || resY === null);
      child = child.getNext()
    ) {
      // Snap to pages
