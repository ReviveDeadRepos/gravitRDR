...
import { IFShapeBoxGuide } from "./shapeboxguide";
import { IFPageGuide } from "./pageguide";
import { IFGridGuide } from "./gridguide";
import { IFUnitGuide } from "./unitguide";
 ...
 * @constructor
 */
export function IFGuides(scene) {
  this._scene = scene;
  this._guides = [];
  this._counter = 0;
  this._visuals = [];
...
IFGuides.prototype.mapPoint = function (point) {
  ...
  for (
    var i = 0;
    i < this._guides.length && (resX === null || resY === null);
    ++i
  ) {
...
IFGuides.prototype.getBBoxSnapZones = function (bBox, location) {
  ...
  if (
    ...
    bBox &&
    !bBox.isEmpty() &&
    bBox.expanded(pDst, pDst, pDst, pDst).containsPoint(location)
  ) {
    visLines = [];
