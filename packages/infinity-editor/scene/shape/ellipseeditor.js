 ...
 * @constructor
 */
export function IFEllipseEditor(ellipse) {
...
/** @override */
IFEllipseEditor.prototype.getCustomBBox = function (
  transform,
  ...
  if (
    ...
    bbox = ifAnnotation.getAnnotationBBox(
      trf,
      center,
      IFElementEditor.OPTIONS.centerCrossSize * 2,
    );
  }
  return bbox;
};
...
/** @override */
IFEllipseEditor.prototype.movePart = function (
  partId,
  partData,
  position,
  ...
  if (
    partId === IFEllipseEditor.START_ANGLE_PART_ID ||
    partId === IFEllipseEditor.END_ANGLE_PART_ID
  ) {
    ...
    if (!this._elementPreview) {
      ...
      this._elementPreview.transferProperties(
        this._element,
        [IFShape.GeometryProperties, IFEllipse.GeometryProperties],
        true,
      );
    }
    ...
    var moveStart =
      this._partSelection.indexOf(IFEllipseEditor.START_ANGLE_PART_ID) >= 0;
    var moveEnd =
      this._partSelection.indexOf(IFEllipseEditor.END_ANGLE_PART_ID) >= 0;
...
IFEllipseEditor.prototype._postPaint = function (transform, context) {
  ...
  if (this._showSegmentDetails()) {
    this._iterateArcEnds(
      ...
      function (args) {
        var annotation =
          args.id == IFEllipseEditor.START_ANGLE_PART_ID
            ? IFElementEditor.Annotation.Diamond
            : IFElementEditor.Annotation.Circle;

        var selected =
          this._partSelection && this._partSelection.indexOf(args.id) >= 0;
...
/** @override */
IFEllipseEditor.prototype._getPartInfoAt = function (
  location,
  transform,
  tolerance,
) {
  ...
  if (this._showSegmentDetails()) {
    ...
    this._iterateArcEnds(
      ...
      function (args) {
        if (
          this._getAnnotationBBox(transform, args.position)
  ...
  var itArgs = [
    {
      ...
      position: transform.mapPoint(
        new IFPoint(Math.cos(startA), Math.sin(startA)),
      ),
    },
    {
