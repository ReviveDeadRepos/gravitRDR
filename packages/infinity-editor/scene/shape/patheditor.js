...
import { IFPathBaseEditor } from "./pathbaseeditor";
...
import { IFShape } from "@gravitrdr/infinity-core";
/**
 ...
 * @constructor
 */
export function IFPathEditor(path) {
  ...
  for (var i = 0; i < selectedAnchorPoints.length; ++i) {
    ...
    this._partSelection.push({
      type: IFPathEditor.PartType.Point,
      point: selectedAnchorPoints[i],
    });
  }
}
    ...
    this._iteratePoints(
      ...
      function (args) {
        ...
        if (args.leftHandlePosition) {
          _addToBBox(
            this._getAnnotationBBox(transform, args.leftHandlePosition, true),
          );
        }
        if (args.rightHandlePosition) {
          _addToBBox(
            this._getAnnotationBBox(transform, args.rightHandlePosition, true),
          );
        }
        ...
        if (args.leftShoulderPosition) {
          _addToBBox(
            this._getAnnotationBBox(transform, args.leftShoulderPosition, true),
          );
        }
        if (args.rightShoulderPosition) {
          _addToBBox(
            this._getAnnotationBBox(
              transform,
              args.rightShoulderPosition,
              true,
            ),
          );
        }
...
/** @override */
IFPathEditor.prototype.movePart = function (
  partId,
  partData,
  position,
  ...
  switch (partId.type) {
    case IFPathEditor.PartType.LeftHandle:
      this._movePreviewPointCoordinates(
        partId.point,
        "hlx",
        "hly",
        position,
...
IFPathEditor.prototype.applyPartMove = function (partId, partData) {
  switch (partId.type) {
    case IFPathEditor.PartType.LeftHandle:
      this._assignPreviewPointPropertiesToSourcePoint(partId.point, [
        "hlx",
        "hly",
        "ah",
      ]);
...
IFPathEditor.prototype.transform = function (transform, partId, partData) {
  if (this._partSelection && this._partSelection.length > 0) {
    ...
    for (var i = 0; i < this._partSelection.length; ++i) {
      ...
      if (selectedPartId.type === IFPathEditor.PartType.Point) {
        ...
        if (!selectedPoint.getProperty("ah")) {
          if (
            selectedPoint.getProperty("hlx") != null &&
            selectedPoint.getProperty("hly") != null
          ) {
            this._transformPreviewPointCoordinates(
              selectedPoint,
              "hlx",
          ...
          if (
            selectedPoint.getProperty("hrx") != null &&
            selectedPoint.getProperty("hry") != null
          ) {
            this._transformPreviewPointCoordinates(
              selectedPoint,
              "hrx",
        ...
        selectedPartId.type === IFPathEditor.PartType.Segment &&
        partId &&
        ...
        var apL = new IFPoint(
          apLeftPreview.getProperty("x"),
          apLeftPreview.getProperty("y"),
        );
        var apR = new IFPoint(
          apRightPreview.getProperty("x"),
          apRightPreview.getProperty("y"),
        );
        ...
        if (!partData.fixedHDirLpt && !partData.fixedHDirRpt) {
          ...
          var newTransform = transform.translated(
            -dPt.getX() + dPt.getX() * partData.cL,
            -dPt.getY() + dPt.getY() * partData.cL,
          );

          this._transformPreviewPointCoordinates(
            selectedPartId.apLeft,
            "hrx",
            "hry",
            newTransform,
            partData.apLhr,
          ...
          var newTransform = transform.translated(
            -dPt.getX() + dPt.getX() * partData.cR,
            -dPt.getY() + dPt.getY() * partData.cR,
          );

          this._transformPreviewPointCoordinates(
            selectedPartId.apRight,
            "hlx",
            "hly",
            newTransform,
            partData.apRhl,
          ...
          var prPt = ifMath.getVectorProjection(
            apL.getX(),
            apL.getY(),
            partData.apLhr.getX(),
            partData.apLhr.getY(),
          ...
          var newTransform = transform.translated(
            -dPt.getX() + prDPt.getX() * partData.cL,
          ...
          var newTransform = transform.translated(
            -dPt.getX() + prDPt.getX() * partData.cR,
          ...
          if (partData.fixedHDirLpt && !partData.fixedHDirRpt) {
            ...
            // !partData.fixedHDirLpt && partData.fixedHDirRpt
            var prPt = ifMath.getVectorProjection(
              apR.getX(),
              apR.getY(),
              partData.apRhl.getX(),
    ...
    IFPathBaseEditor.prototype.transform.call(
      this,
      transform,
      partId,
      partData,
...
IFPathEditor.prototype.canApplyTransform = function () {
  return (
    (this._partSelection && this._partSelection.length > 0) ||
...
IFPathEditor.prototype.subSelectDragStartAction = function (partInfo) {
  ...
  if (!partInfo.isolated && partInfo.id.type == IFPathEditor.PartType.Point) {
    ...
    if (
      IFPathBase.isCornerType(aPtType) &&
    ...
    if (idType != IFPathEditor.PartType.Point) {
      ...
      if (!partInfo.data.apSelected || !aPt.hasFlag(IFNode.Flag.Selected)) {
        ...
        if (!aPt.hasFlag(IFNode.Flag.Selected)) {
          // might be some error, select a point
          this.selectOnePoint(aPt);
        }
        newPartInfo = new IFElementEditor.PartInfo(
          this,
          { type: IFPathEditor.PartType.Point, point: aPt },
          { apSelected: true },
          isolated,
          selectable,
        );
      } else if (hlx === null || hly === null || hrx === null || hry === null) {
        // option b) or c)
        this.requestInvalidation();
        this._createPathPreviewIfNecessary(aPt);
        var previewPoint = this.getPathPointPreview(aPt);
        if (!previewPoint) {
          ...
          if (hrx === null || hry === null) {
            ...
            previewPoint.setProperties(
              ["ah", "hrx", "hry"],
              [false, aPt.getProperty("x"), aPt.getProperty("y")],
            );
          } else {
            // hlx === null || hly === null
            // option c)
    ...
    if (apLeft && apRight) {
      ...
      if (!apLeftPreview || !apRightPreview) {
        // might be some error
        newPartInfo = null;
      } else {
        ...
        if (
          (apLeft.getProperty("hrx") === null ||
            apLeft.getProperty("hry") === null) &&
          (apRight.getProperty("hlx") === null ||
            apRight.getProperty("hly") === null)
        ) {
          ...
          newPartInfo = new IFElementEditor.PartInfo(
            this,
            {
              type: IFPathEditor.PartType.Segment,
              point: null,
              apLeft: apLeft,
              apRight: apRight,
            },
            {
              ...
              cL: 4 / 3,
              apLhr: new IFPoint(hrx, hry),
              fixedHDirLpt: false,
              cR: 4 / 3,
              apRhl: new IFPoint(hlx, hly),
              fixedHDirRpt: false,
            },
            null,
            false,
...
IFPathEditor.prototype._detach = function () {
  ...
  for (
    var anchorPoint = this._element.getAnchorPoints().getFirstChild();
    anchorPoint != null;
 ...
 */
IFPathEditor.prototype.hitAnchorPoint = function (
  anchorPt,
  location,
  transform,
  tolerance,
  ...
  if (anchorPt) {
    ...
    if (transform) {
      transformToApply = transformToApply
        ? transformToApply.multiplied(transform)
        : transform;
    }

    return this._getAnnotationBBox(
      transformToApply,
      new IFPoint(anchorPt.getProperty("x"), anchorPt.getProperty("y")),
      true,
    )
...
IFPathEditor.prototype.getPointCoord = function (anchorPt) {
  ...
  if (
    anchorPt.getPath() == this._element ||
    (this._elementPreview && anchorPt.getPath() == this._elementPreview)
  ) {
...
/** @override */
IFPathEditor.prototype._getPartInfoAt = function (
  location,
  transform,
  tolerance,
) {
  if (this._showAnnotations()) {
      ...
      function (args) {
        ...
        if (partType) {
          result = new IFElementEditor.PartInfo(
            this,
            { type: partType, point: args.anchorPoint },
            { apSelected: args.anchorPoint.hasFlag(IFNode.Flag.Selected) },
            isolated,
            selectable,
          );
          return true;
    ...
    if (result) {
      ...
      var pathHitResult = this._element.pathHitTest(
        location,
        transform,
        false,
        this._element.getScene().getProperty("pickDist"),
      );
      if (pathHitResult) {
        var hitRes = pathHitResult.data;
        var apLeft = this._element
          .getAnchorPoints()
          .getChildByIndex(hitRes.segment - 1);
        var apRight = apLeft
          ? this._element.getAnchorPoints().getNextPoint(apLeft)
          : null;
        return new IFElementEditor.PartInfo(
          ...
          { type: IFPathEditor.SegmentData.HitRes, hitRes: pathHitResult.data },
          false,
          true,
        );
      }
      ...
      function (args) {
        ...
        if (args.rightHandlePosition) {
          this._paintHandle(
            ...
            context,
            args.position,
            args.rightHandlePosition,
          );
        }

        // Paint shoulders
        if (args.leftShoulderPosition) {
          this._paintAnnotation(
            context,
            transform,
            args.leftShoulderPosition,
        ...
        this._paintAnnotation(
          ...
          args.position,
          args.annotation,
          args.anchorPoint.hasFlag(IFNode.Flag.Selected),
          true,
        );
      }.bind(this),
    );
    ...
    for (var i = 0; i < this._partSelection.length; ++i) {
      ...
      if (newSelection) {
        for (var k = 0; k < newSelection.length; ++k) {
          if (
            (newSelection[k].point === part.point && part.point) ||
            (part.type == IFPathEditor.PartType.Segment &&
              part.type == newSelection[k].type &&
              part.apLeft == newSelection[k].apLeft &&
              part.apRight == newSelection[k].apRight)
          ) {
...
IFPathEditor.prototype._iteratePoints = function (paintElement, iterator) {
  ...
  for (
    var anchorPoint = anchorPoints.getFirstChild();
    anchorPoint != null;
    anchorPoint = anchorPoint.getNext()
  ) {
    ...
    var position = new IFPoint(
      anchorPoint.getProperty("x"),
      anchorPoint.getProperty("y"),
    );
    ...
    if (
      anchorPoint.hasFlag(IFNode.Flag.Selected) ||
      (previousPt && previousPt.hasFlag(IFNode.Flag.Selected))
    ) {
      var pt = new IFPoint(
        anchorPoint.getProperty("hlx"),
        anchorPoint.getProperty("hly"),
      );
    ...
    if (
      anchorPoint.hasFlag(IFNode.Flag.Selected) ||
      (nextPt && nextPt.hasFlag(IFNode.Flag.Selected))
    ) {
      var pt = new IFPoint(
        anchorPoint.getProperty("hrx"),
        anchorPoint.getProperty("hry"),
      );
      if (pt.getX() !== null && pt.getY() !== null) {
    ...
    if (transform) {
      ...
      if (itArgs.leftHandlePosition) {
        itArgs.leftHandlePosition = transform.mapPoint(
          itArgs.leftHandlePosition,
        );
      }

      if (itArgs.rightHandlePosition) {
        itArgs.rightHandlePosition = transform.mapPoint(
          itArgs.rightHandlePosition,
        );
      }
 ...
 */
IFPathEditor.prototype.shiftPreviewTable = function (
  shiftVal,
  shiftFrom,
  shiftTo,
) {
  ...
  shiftTo =
    shiftTo != null ? shiftTo : this._sourceIndexToPreviewIndex.length - 1;
...
IFPathEditor.prototype.extendPreviewToFull = function () {
  ...
  try {
    while (!hasPreview && ap) {
      ...
      previewAnchorPoint.transferProperties(ap, [
 ...
 */
IFPathEditor.prototype.constrainPosition = function (
  position,
  worldToViewTransform,
  sourcePoint,
) {
  var basePt = new IFPoint(
    sourcePoint.getProperty("x"),
    sourcePoint.getProperty("y"),
  );
  var transformToApply = this._element.getTransform();
  transformToApply = transformToApply
    ? transformToApply.multiplied(worldToViewTransform)
    : worldToViewTransform;
 ...
 * @private
 */
IFPathEditor.prototype._createPathPreviewIfNecessary = function (
  selectedAnchorPoint,
) {
  if (!this._elementPreview) {
    ...
    var _anchorPointIsSelected = function (anchorPoint) {
      return (
        (selectedAnchorPoint && anchorPoint === selectedAnchorPoint) ||
        (!selectedAnchorPoint && anchorPoint.hasFlag(IFNode.Flag.Selected))
      );
    };
    ...
    var relPt;
    for (
      var anchorPoint = sourceAnchorPoints.getFirstChild();
      ...
      if (!firstSelPoint) {
        if (!prevPt && _anchorPointIsSelected(anchorPoint)) {
          ...
          if (
            !_anchorPointIsSelected(anchorPoint) &&
            nextPt &&
            _anchorPointIsSelected(nextPt)
          ) {
            ...
            if (
              !_anchorPointIsSelected(relPt) &&
              (!sourceAnchorPoints.getPreviousPoint(relPt) ||
                !_anchorPointIsSelected(
                  sourceAnchorPoints.getPreviousPoint(relPt),
                ))
            ) {
              firstSelPoint = relPt;
    ...
    if (firstSelPoint && lastSelPoint) {
      ...
      for (
        var anchorPoint = sourceAnchorPoints.getNextPoint(lastSelPoint);
    ...
    firstSelPoint = firstSelPoint
      ? firstSelPoint
      : sourceAnchorPoints.getFirstChild();
    var finished = false;
    ...
    try {
      while (!finished) {
        var previewAnchorPoint = new IFPathBase.AnchorPoint();
        previewAnchorPoint.transferProperties(anchorPoint, [
          IFPathBase.AnchorPoint.GeometryProperties,
        ]);
        if (_anchorPointIsSelected(anchorPoint)) {
          previewAnchorPoint.setFlag(IFNode.Flag.Selected);
        ...
        var previewIndex =
          previewAnchorPoints.getIndexOfChild(previewAnchorPoint);
    ...
    if (
      firstSelPoint.getProperty("ah") ||
      (lastSelPoint && lastSelPoint.getProperty("ah"))
    ) {
      this.extendPreviewToFull();
    } else {
...
IFPathEditor.prototype.getTransformFromNative = function (transform) {
  ...
  if (transform) {
    transformToNewPos = transformToNewPos
      ? transformToNewPos.multiplied(transform)
      : transform;
  }
 ...
 */
IFPathEditor.prototype.movePoint = function (
  anchorPoint,
  newPosition,
  transform,
  origPoint,
  ...
  if (anchorPoint.getProperty("ah")) {
    anchorPoint.setProperties(
      ["x", "y"],
      [newNativePos.getX(), newNativePos.getY()],
    );
  } else {
 ...
 * @private
 */
IFPathEditor.prototype._movePreviewPointCoordinates = function (
  sourcePoint,
  xProperty,
  yProperty,
  position,
  ...
  var sourcePosition = new IFPoint(
    sourcePoint.getProperty(xProperty),
    sourcePoint.getProperty(yProperty),
  );
  ...
  this._transformPreviewPointCoordinates(
    ...
    new IFTransform(
      ...
      0,
      1,
      newPos.getX() - sourcePosition.getX(),
      newPos.getY() - sourcePosition.getY(),
    ),
  );
 ...
 * @private
 */
IFPathEditor.prototype._transformPreviewPointCoordinates = function (
  sourcePoint,
  xProperty,
  yProperty,
  transform,
 ...
 * @private
 */
IFPathEditor.prototype._movePreviewPointShoulders = function (
  partId,
  position,
  ratio,
) {
  var pathTransform = this._element.getTransform();
  var sourcePosition = new IFPoint(
    partId.point.getProperty("x"),
    partId.point.getProperty("y"),
  );
  ...
  var newShoulderPt = ifMath.getVectorProjection(
    sourcePosition.getX(),
    sourcePosition.getY(),
    shoulderLimitPt.getX(),
    shoulderLimitPt.getY(),
    position.getX(),
    position.getY(),
    true,
  );

  var newVal = ifMath.ptDist(
    newShoulderPt.getX(),
    newShoulderPt.getY(),
    sourcePosition.getX(),
    sourcePosition.getY(),
  ...
  if (ratio) {
    if (
      this.hasFlag(IFElementEditor.Flag.Detail) &&
      previewPoint.getProperty("cu") != true
    ) {
 ...
 * @private
 */
IFPathEditor.prototype._assignPreviewPointPropertiesToSourcePoint = function (
  sourcePoint,
  properties,
) {
  var previewPoint = this.getPathPointPreview(sourcePoint);
  if (previewPoint) {
    ...
    sourcePoint.setProperties(
      properties,
      previewPoint.getProperties(properties),
    );
  }
};
...
IFPathEditor.prototype._transferPreviewProperties = function (point, element) {
  ...
  if (previewPoint) {
    elSourcePoint.transferProperties(previewPoint, [
      IFPathBase.AnchorPoint.GeometryProperties,
    ]);
  }
};
  ...
  for (var i = 0; i < selection.length; ++i) {
    if (selection[i].type != IFPathEditor.PartType.Point) {
      ...
      for (var k = 0; k < selection.length; ++k) {
        if (
          selection[k].type == IFPathEditor.PartType.Segment &&
          (selection[i].point == selection[k].apLeft ||
            selection[i].point == selection[k].apRight)
        ) {
...
IFPathEditor.prototype._geometryChange = function (evt) {
  if (
    evt.type == IFElement.GeometryChangeEvent.Type.After &&
    evt.element == this._element
  ) {
    if (this._elementPreview) {
      this.releasePathPreview();
      this.requestInvalidation();
