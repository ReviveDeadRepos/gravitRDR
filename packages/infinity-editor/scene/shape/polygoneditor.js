 ...
 * @constructor
 */
export function IFPolygonEditor(polygon) {
...
/** @override */
IFPolygonEditor.prototype.getCustomBBox = function (
  transform,
  includeEditorTransform,
) {
  var bbox = null;
...
/** @override */
IFPolygonEditor.prototype.movePart = function (
  partId,
  partData,
  position,
  ...
  if (
    partId === IFPolygonEditor.INSIDE_PART_ID ||
    partId === IFPolygonEditor.OUTSIDE_PART_ID
  ) {
    ...
    if (!this._elementPreview) {
      ...
      this._elementPreview.transferProperties(
        this._element,
        [IFShape.GeometryProperties, IFPolygon.GeometryProperties],
        true,
      );
    }
    ...
    var angle =
      Math.atan2(newPos.getY() - center.getY(), newPos.getX() - center.getX()) -
      partData;
    var distance = ifMath.ptDist(
      newPos.getX(),
      newPos.getY(),
      center.getX(),
      center.getY(),
    ...
    var moveInner =
      this._partSelection.indexOf(IFPolygonEditor.INSIDE_PART_ID) >= 0;
    var moveOuter =
      this._partSelection.indexOf(IFPolygonEditor.OUTSIDE_PART_ID) >= 0;
    ...
    if (this._partSelection.length == 1) {
      ...
      if (partId == IFPolygonEditor.INSIDE_PART_ID) {
        ...
        var oPt_new = new IFPoint(
          or * Math.cos(oa) + moveX,
          or * Math.sin(oa) + moveY,
        );
        ...
        var iPt_new = new IFPoint(
          ir * Math.cos(ia) + moveX,
          ir * Math.sin(ia) + moveY,
        );
...
IFPolygonEditor.prototype.applyPartMove = function (partId, partData) {
  if (
    ...
    var propertyValues = this._elementPreview.getProperties([
      "oa",
      "or",
      "ia",
      "ir",
...
IFPolygonEditor.prototype._postPaint = function (transform, context) {
  ...
  if (this._showSegmentDetails()) {
    ...
    element.iterateSegments(
      function (point, inside, angle) {
        var annotation = inside
          ? IFElementEditor.Annotation.Circle
          : IFElementEditor.Annotation.Diamond;
        var partId = inside
          ? IFPolygonEditor.INSIDE_PART_ID
          : IFPolygonEditor.OUTSIDE_PART_ID;
        this._paintAnnotation(
          context,
          transform,
          point,
          annotation,
          this._partSelection && this._partSelection.indexOf(partId) >= 0,
          false,
        );
      }.bind(this),
      true,
...
/** @override */
IFPolygonEditor.prototype._getPartInfoAt = function (
  location,
  transform,
  tolerance,
) {
  ...
  if (this._showSegmentDetails()) {
    ...
    this._element.iterateSegments(
      function (point, inside, angle) {
        if (
          this._getAnnotationBBox(transform, point)
  ...
  return IFShapeEditor.prototype._getPartInfoAt.call(
    this,
    location,
    transform,
    tolerance,
