 ...
 * @constructor
 */
export function IFTextEditor(rectangle) {
...
IFTextEditor.prototype.getProperty = function (property, computed) {
  if (IFText.GeometryProperties.hasOwnProperty(property)) {
    ...
    if (IFText.Block.Properties.hasOwnProperty(property)) {
      if (activeSpan) {
        return IFText.Block.cssToProperty(
          property,
          computed ? window.getComputedStyle(activeSpan) : activeSpan.style,
        );
      } else if (activeParagraph) {
        return IFText.Block.cssToProperty(
          property,
          computed
            ? window.getComputedStyle(activeParagraph)
            : activeParagraph.style,
        );
      } else {
      ...
      if (activeParagraph) {
        return IFText.Paragraph.cssToProperty(
          property,
          computed
            ? window.getComputedStyle(activeParagraph)
            : activeParagraph.style,
...
IFTextEditor.prototype.setProperties = function (properties, values) {
  ...
  if (this.isInlineEdit()) {
    ...
    this.getElement()
      .getContent()
      .setProperties(paragraphProperties, paragraphValues);
  }
      ...
      if (newWidth || newHeight) {
        ...
        if (bbox && cbox) {
          var transform = new IFTransform()
            ...
            .scaled(
              newWidth ? cbox.getWidth() / bbox.getWidth() : 1,
              newHeight ? cbox.getHeight() / bbox.getHeight() : 1,
            )
            .translated(bbox.getX(), bbox.getY());
...
IFTextEditor.prototype.adjustInlineEditForView = function (view, position) {
  ...
  var height = "";
  if (
    this.getElement().getProperty("aw") === false &&
    sceneBBox.getWidth() > 0
  ) {
    width = sceneBBox.getWidth() + "px";
  }
  if (
    this.getElement().getProperty("ah") === false &&
    sceneBBox.getHeight() > 0
  ) {
 ...
 */
IFTextEditor.prototype.createSelectionFromPosition = function (
  startPos,
  endPos,
) {
  var doc = document;
...
IFTextEditor.prototype._prePaint = function (transform, context) {
  if (
    ...
    if (textRect) {
      ...
      var w =
        Math.ceil(transformedRect.getX() + transformedRect.getWidth()) - x;
      var h =
        Math.ceil(transformedRect.getY() + transformedRect.getHeight()) - y;

      context.canvas.strokeRect(
        x + 0.5,
        y + 0.5,
        w,
        h,
  ...
  if (editor.hasEventListeners(IFEditor.InlineEditorEvent)) {
    editor.trigger(
      new IFEditor.InlineEditorEvent(
        this,
        IFEditor.InlineEditorEvent.Type.SelectionChanged,
      ),
    );
  }
};
