...
import { IFPatternPaint } from "@gravitrdr/infinity-core";
...
import { IFVertexTransformer } from "@gravitrdr/infinity-core";
 ...
 * @constructor
 */
export function IFShapeEditor(shape) {
  IFBlockEditor.call(this, shape);
}
IFObject.inherit(IFShapeEditor, IFBlockEditor);

/** @override */
IFShapeEditor.prototype.acceptDrop = function (
  position,
  type,
  source,
  hitData,
  ...
  if (
    IFElementEditor.prototype.acceptDrop.call(
      this,
      position,
      type,
      source,
    ...
    if (
      hitData instanceof IFStyle.HitResult &&
      hitData.entry instanceof IFPatternPaint
    ) {
...
IFShapeEditor.prototype._postPaint = function (transform, context) {
  ...
  if (
    ...
    var targetTransform = sourceTransform
      ? sourceTransform
      : new IFTransform(1, 0, 0, 1, 0, 0);
    targetTransform = transform
      ? targetTransform.multiplied(transform)
      : targetTransform;
    var crossHalfSizeMax = IFElementEditor.OPTIONS.centerCrossSize * 2;
    var tMatrix = targetTransform.getMatrix();

    if (
      Math.abs(tMatrix[0]) * element.getOrigHalfWidth() > crossHalfSizeMax &&
