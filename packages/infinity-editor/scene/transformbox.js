...
import { IFVertexSource } from "@gravitrdr/infinity-core";
...
import { ifAnnotation } from "@gravitrdr/infinity-core";
import { IFVertexPixelAligner } from "@gravitrdr/infinity-core";
...
import { IFVertexInfo, ifVertexInfo } from "@gravitrdr/infinity-core";
import { ifMath } from "@gravitrdr/infinity-core";
import { IFGuides } from "../guide/guides";
 ...
 * @constructor
 */
export function IFTransformBox(bbox, cx, cy) {
...
IFTransformBox.prototype._calculateGeometryBBox = function () {
  ...
  var yMax = yMin;
  var edges = [
    IFTransformBox.Handles.TOP_LEFT,
    IFTransformBox.Handles.TOP_RIGHT,
    IFTransformBox.Handles.BOTTOM_RIGHT,
    IFTransformBox.Handles.BOTTOM_LEFT,
  ];
...
IFTransformBox.prototype.paint = function (transform, context) {
  ...
  this._extTransform = transform
    ? canvasTransform.multiplied(transform)
    : canvasTransform;
  ...
  if (!this._centerOnly) {
    ...
    for (var i = 0; i < sides.length; ++i) {
      ...
      ifAnnotation.paintAnnotation(
        ...
        null,
        pt,
        ifAnnotation.AnnotType.Rectangle,
        false,
        IFTransformBox.ANNOT_SIZE,
        IFColor.WHITE,
        IFColor.BLACK,
  ...
  ifAnnotation.paintAnnotation(
    context,
    null,
    this._getPoint(IFTransformBox.Handles.ROTATION_CENTER),
    ifAnnotation.AnnotType.Circle,
    false,
    IFTransformBox.ANNOT_SIZE,
    IFColor.WHITE,
    IFColor.BLACK,
 ...
 */
IFTransformBox.prototype.getPartInfoAt = function (
  location,
  transform,
  tolerance,
) {
  ...
  for (var i = 0; i < sides.length; ++i) {
    ...
    if (
      ifAnnotation
        .getAnnotationBBox(null, handle, IFTransformBox.ANNOT_SIZE)
    ...
    if (
      ...
      if (hitRes.outline) {
        ...
        result = new IFElementEditor.PartInfo(
          null,
          IFTransformBox.OUTLINE,
          edgeType,
        );
      } else {
        result = new IFElementEditor.PartInfo(
          null,
          IFTransformBox.INSIDE,
          null,
        );
      }
    } else {
...
IFTransformBox.prototype.getRotationSegment = function (location, transform) {
  ...
  var angle =
    Math.atan2(location.getY() - cntr.getY(), location.getX() - cntr.getX()) +
    (Math.PI * 7) / 8;
  if (angle < 0) {
    angle += ifMath.PI2;
  }
 ...
 * @returns {IFTransform}
 */
IFTransformBox.prototype.calculateTransformation = function (
  partInfo,
  startPtTr,
  endPtTr,
  guides,
  ...
  if (partInfo.id < 8) {
    var sides = [
      IFRect.Side.TOP_LEFT,
      IFRect.Side.TOP_CENTER,
      IFRect.Side.TOP_RIGHT,
      IFRect.Side.RIGHT_CENTER,
      IFRect.Side.BOTTOM_RIGHT,
      IFRect.Side.BOTTOM_CENTER,
      IFRect.Side.BOTTOM_LEFT,
      IFRect.Side.LEFT_CENTER,
    ];

    var tBoxRect = new IFRect(
      this.tlx,
      this.tly,
      this.brx - this.tlx,
      this.bry - this.tly,
    ...
    var transform2 = new IFTransform(
      1,
      (dy * 2) / (this.brx - this.tlx),
      (-dx * 2) / (this.bry - this.tly),
      1,
      0,
      0,
    );
...
IFTransformBox.prototype._getPoint = function (side, noTransform) {
  ...
  switch (side) {
    case IFTransformBox.Handles.TOP_LEFT:
      ...
      pt = new IFPoint(
        pt.getX() - IFTransformBox.TRANSFORM_MARGIN,
        pt.getY() - IFTransformBox.TRANSFORM_MARGIN,
      );
      break;
    case IFTransformBox.Handles.TOP_CENTER:
      pt = _transform(
        new IFPoint((this.tlx + this.trx) / 2, (this.tly + this.try) / 2),
      );
    ...
    case IFTransformBox.Handles.RIGHT_CENTER:
      pt = _transform(
        new IFPoint((this.trx + this.brx) / 2, (this.try + this.bry) / 2),
      );
    ...
    case IFTransformBox.Handles.BOTTOM_CENTER:
      pt = _transform(
        new IFPoint((this.blx + this.brx) / 2, (this.bly + this.bry) / 2),
      );
      pt = new IFPoint(pt.getX(), pt.getY() + IFTransformBox.TRANSFORM_MARGIN);
      break;
    ...
    case IFTransformBox.Handles.LEFT_CENTER:
      pt = _transform(
        new IFPoint((this.tlx + this.blx) / 2, (this.tly + this.bly) / 2),
      );
      pt = new IFPoint(pt.getX() - IFTransformBox.TRANSFORM_MARGIN, pt.getY());
      break;
...
IFTransformBox.prototype._collectResizeHandles = function (transform) {
  ...
  var sides = [];
  if (
    transformedBBox.getHeight() > (IFTransformBox.ANNOT_SIZE + 2) * 2 &&
  ...
  if (transformedBBox.getHeight() > (IFTransformBox.ANNOT_SIZE + 2) * 3) {
    sides = sides.concat([
      IFTransformBox.Handles.RIGHT_CENTER,
      IFTransformBox.Handles.LEFT_CENTER,
    ]);
  }
  if (transformedBBox.getWidth() > (IFTransformBox.ANNOT_SIZE + 2) * 3) {
    sides = sides.concat([
      IFTransformBox.Handles.TOP_CENTER,
      IFTransformBox.Handles.BOTTOM_CENTER,
    ]);
  }
  return sides;
};
