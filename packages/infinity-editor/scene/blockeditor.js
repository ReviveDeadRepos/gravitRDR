 ...
 * @constructor
 */
export function IFBlockEditor(block) {
...
/** @override */
IFBlockEditor.prototype.movePart = function (
  partId,
  partData,
  position,
  ...
  IFElementEditor.prototype.movePart.call(
    this,
    partId,
    partData,
    position,
  ...
  if (partId === IFBlockEditor.RESIZE_HANDLE_PART_ID) {
    ...
    var transform = sourceBBox.getResizeTransform(
      partData.side,
      delta.getX(),
      delta.getY(),
      shift,
      ...
      this._iterateResizeHandles(
        function (point, side) {
          this._paintAnnotation(
            ...
            transform,
            point,
            IFElementEditor.Annotation.Rectangle,
            false,
            true,
          );
        }.bind(this),
...
/** @override */
IFBlockEditor.prototype._getPartInfoAt = function (
  location,
  transform,
  tolerance,
) {
      ...
      function (point, side) {
        if (this._getAnnotationBBox(transform, point).containsPoint(location)) {
          result = new IFElementEditor.PartInfo(
            this,
            IFBlockEditor.RESIZE_HANDLE_PART_ID,
...
IFBlockEditor.prototype._showResizeHandles = function () {
  return (
    ...
    (this.hasFlag(IFBlockEditor.Flag.ResizeEdges) ||
      this.hasFlag(IFBlockEditor.Flag.ResizeCenters))
  );
};
...
IFBlockEditor.prototype._iterateResizeHandles = function (iterator, transform) {
  ...
  if (bbox && !bbox.isEmpty()) {
    ...
    if (
      this.hasFlag(IFBlockEditor.Flag.ResizeEdges) &&
      transformedBBox.getHeight() >
        (IFElementEditor.OPTIONS.annotationSizeSmall + 2) * 2 &&
      transformedBBox.getWidth() >
        (IFElementEditor.OPTIONS.annotationSizeSmall + 2) * 2
    ) {
      sides = sides.concat([
    ...
    if (this.hasFlag(IFBlockEditor.Flag.ResizeCenters)) {
      if (
        transformedBBox.getHeight() >
        (IFElementEditor.OPTIONS.annotationSizeSmall + 2) * 3
      ) {
        sides = sides.concat([
 ...
 * @private
 */
IFBlockEditor.prototype._paintBBoxOutline = function (
  transform,
  context,
  color,
) {
