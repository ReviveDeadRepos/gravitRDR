...
import { IFTransformBox } from "./transformbox";
 ...
 * @constructor
 */
export function IFSceneEditor(scene) {
...
IFSceneEditor.prototype.getCursor = function (partInfo) {
  ...
  switch (partInfo.id) {
    ...
    case IFTransformBox.OUTLINE:
      cursor = partInfo.data
        ? IFCursor.SelectSkewHoriz
        : IFCursor.SelectSkewVert;
      break;
...
IFSceneEditor.prototype.updateTBoxUnderMouse = function (
  location,
  transform,
  view,
) {
  var pInfo = null;
  if (this._tBoxData) {
    pInfo = new IFElementEditor.PartInfo(
      this._tBoxData.editor,
      this._tBoxData.id,
      this._tBoxData.data,
    );
    ...
    pInfo = this._transformBox.getPartInfoAt(
      location,
      transform,
      this._element.getProperty("pickDist"),
    );
  }

  if (
    !this._mouseInfo ||
    this._mouseInfo.id != pInfo.id ||
    this._mouseInfo.data != pInfo.data
  ) {
  ...
  this._visuals = null;
  if (
    this._tBoxMode == IFSceneEditor.TBoxMode.PASSIVE &&
    this._mouseInfo.id == IFTransformBox.INSIDE
  ) {
    ...
    if (selBBox && !selBBox.isEmpty()) {
      ...
      var visuals = this._getGraphicEditor()
        .getGuides()
        .getBBoxSnapZones(bBox, location);
  ...
  visualsArea = visualsArea
    ? view.getViewTransform().mapRect(visualsArea)
    : null;
...
IFSceneEditor.prototype.getTBoxPartInfoAt = function (
  location,
  transform,
  tolerance,
) {
...
IFSceneEditor.prototype.startTBoxTransform = function (partInfo) {
  ...
  this._tBoxData = new IFElementEditor.PartInfo(
    partInfo.editor,
    partInfo.id,
    partInfo.data,
  );
  ...
  if (this._tBoxData.id == IFTransformBox.OUTLINE) {
    ...
    this._tBoxData.id >= 0 &&
    this._tBoxData.id < IFTransformBox.Handles.ROTATION_CENTER
  ) {
 ...
 */
IFSceneEditor.prototype.transformTBox = function (
  startPos,
  curPos,
  option,
  ratio,
  ...
  if (
    this._tBoxMode != IFSceneEditor.TBoxMode.PASSIVE &&
    this._tBoxMode != IFSceneEditor.TBoxMode.NA
  ) {
    var guides = this._getGraphicEditor().getGuides();
    guides
      .getShapeBoxGuide()
      .useExclusions(this._getGraphicEditor().getSelection());
    guides.beginMap();
    var rStep = ratioStep;
    if (!rStep && this._tBoxMode == IFSceneEditor.TBoxMode.SKEW) {
      rStep = this._element.getProperty("gridSizeX");
    }
    var transform = this._transformBox.calculateTransformation(
      this._tBoxData,
      startPos,
      curPos,
      guides,
