 ...
 * @constructor
 */
export function IFElementEditor(element) {
...
IFElementEditor.prototype.updatePartSelection = function (toggle, selection) {
  if (this.hasFlag(IFElementEditor.Flag.Selected)) {
    ...
    if (!toggle || !this._partSelection) {
      newSelection =
        selection && selection.length > 0 ? selection.slice() : null;
    } else {
      if (selection) {
        newSelection = [];
 ...
 */
IFElementEditor.prototype.getPartInfoAt = function (
  location,
  transform,
  acceptor,
  tolerance,
  ...
  if (this._editors && this._editors.length) {
    for (var i = this._editors.length - 1; i >= 0; --i) {
      var result = this._editors[i].getPartInfoAt(
        location,
        transform,
        acceptor,
        tolerance,
 ...
 * @return {IFRect} the bbox in view coordinates
 */
IFElementEditor.prototype.getCustomBBox = function (
  transform,
 ...
 */
IFElementEditor.prototype.movePart = function (
  partId,
  partData,
  position,
...
IFElementEditor.prototype.canApplyTransform = function () {
  ...
  return (
    ...
    element.hasMixin(IFElement.Transform) &&
    !element.hasFlag(IFElement.Flag.Locked)
  );
};
 ...
 */
IFElementEditor.prototype.acceptDrop = function (
  position,
  type,
  source,
  hitData,
  ...
  if (this._editors) {
    for (var i = 0; i < this._editors.length; ++i) {
      if (
        this._editors[i].acceptDrop(position, type, source, hitData) === true
      ) {
        return true;
      }
    }
 ...
 */
IFElementEditor.prototype._getPartInfoAt = function (
  location,
  transform,
  tolerance,
) {
 ...
 */
IFElementEditor.prototype._paintAnnotation = function (
  context,
  transform,
  center,
  annotation,
  ...
  var size = small
    ? IFElementEditor.OPTIONS.annotationSizeSmall
    : IFElementEditor.OPTIONS.annotationSizeRegular;
 ...
 */
IFElementEditor.prototype._getAnnotationBBox = function (
  transform,
  center,
  small,
) {
...
IFElementEditor.prototype._showAnnotations = function () {
  return (
    this.hasFlag(IFElementEditor.Flag.Selected) &&
    !this.hasFlag(IFElementEditor.Flag.Outline)
  );
};
