import { IFPathTool } from "./pathtool";
...
IFPenTool.prototype._mouseDown = function (event) {
  ...
  if (this._mode != IFPathTool.Mode.Edit) {
    if (this._newPoint && this._pathEditor) {
      ...
      if (event.button == GUIMouseEvent.BUTTON_RIGHT) {
        if (ifPlatform.modifiers.optionKey) {
          ...
          this._editPt.setProperties(
            ["tp", "cu"],
            [IFPathBase.CornerType.Rounded, true],
          );
        }
      ...
      // We just switched from Edit mode, end point was clicked
      if (this._mode == IFPathTool.Mode.Append) {
        this._refPt = this._pathRef.getAnchorPoints().getLastChild();
      } else {
        // this._mode == IFPathTool.Mode.Prepend
...
IFPenTool.prototype._mouseMove = function (event) {
  ...
  if (this._mode == IFPathTool.Mode.Edit) {
    ...
    // _mode == Append || Prepend
    var newPos = event.client;
    if (!this._newPoint && this._pathEditor) {
      newPos = this._constrainIfNeeded(
        event.client,
        this._view.getWorldTransform(),
        this._pathRef,
      );
...
IFPenTool.prototype._updateHandles = function (newPos) {
  ...
  if (
    ...
    if (this._mode != IFPathTool.Mode.Edit) {
      if (this._mode == IFPathTool.Mode.Append) {
        if (
          this._editPt.getProperty("hlx") !== null &&
          this._editPt.getProperty("hly") !== null
        ) {
          this._editPt.setProperties(
            ["tp", "hrx", "hry"],
            [IFPathBase.AnchorPoint.Type.Symmetric, null, null],
          );
        } else {
          this._editPt.setProperties(
            ["tp", "hrx", "hry"],
            [IFPathBase.AnchorPoint.Type.Asymmetric, null, null],
          );
        }
      } else {
        // _mode == Prepend
        if (
          this._editPt.getProperty("hrx") !== null &&
          this._editPt.getProperty("hry") !== null
        ) {
          this._editPt.setProperties(
            ["tp", "hlx", "hly"],
            [IFPathBase.AnchorPoint.Type.Symmetric, null, null],
      ...
      if (!ifPlatform.modifiers.optionKey) {
        this._editPt.setProperties(
          ["tp", "hlx", "hly", "hrx", "hry"],
          [IFPathBase.AnchorPoint.Type.Asymmetric, null, null, null, null],
        );
      } else {
        this._editPt.setProperties(
          ["tp", "hrx", "hry"],
    ...
    var transformToNewPos = this._pathEditor.getTransformFromNative(
      this._view.getWorldTransform(),
    );
    ...
    if (
      ...
      this._editPt.setProperties(
        ["tp", "hrx", "hry"],
        [IFPathBase.AnchorPoint.Type.Asymmetric, hrx, hry],
      );
      ...
      if (ifPlatform.modifiers.optionKey) {
        ...
        if (
          ...
          ((this._editPt.getPrevious() == null &&
            this._editPt.getNext() != null) ||
            (this._editPt.getPrevious() != null &&
              this._editPt.getNext() == null))
        ) {
          if (this._mode == IFPathTool.Mode.Prepend) {
          ...
          if (this._mode == IFPathTool.Mode.Prepend) {
            this._editPt.setProperties(
              ["tp", "hrx", "hry", "hlx", "hly"],
              [newTp, h2x, h2y, h1x, h1y],
            );
          } else {
            this._editPt.setProperties(
              ["tp", "hrx", "hry", "hlx", "hly"],
              [newTp, h1x, h1y, h2x, h2y],
            );
          }
        }
      }
      ...
      if (
        ...
        if (prevPt) {
          ...
          if (!ifMath.isEqualEps(dirLen, 0)) {
            ...
            var hLen = ifMath.vDotProduct(
              ex,
              ey,
              newNativePos.getX() - ptx,
              newNativePos.getY() - pty,
            );
            if (hLen > 0) {
              hrx = ptx + ex * hLen;
              hry = pty + ey * hLen;
...
IFPenTool.prototype._mouseDrag = function (event) {
  ...
  if (!this._released && this._editPt) {
    ...
    if (!this._dragStartPt) {
      ...
      if (
        event.button == GUIMouseEvent.BUTTON_LEFT &&
        this._editPt.getProperty("tp") != IFPathBase.AnchorPoint.Type.Connector
      ) {
        this._editPt.setProperty("tp", IFPathBase.AnchorPoint.Type.Symmetric);
      }
    }
        ...
        if (!this._dragStarted && this._refPt && !this._editPt) {
          ...
          if (this._transactionType == IFPathTool.Transaction.NoTransaction) {
            this._startTransaction(
              IFPathTool.Transaction.ModifyPointProperties,
            );
          }
