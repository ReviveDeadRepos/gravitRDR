 ...
 * @constructor
 */
export function IFShapeTool(keepRatio, fromCenter) {
  IFTool.call(this);
...
IFShapeTool.prototype.paint = function (context) {
  if (this._shape) {
    ...
    if (this._hasCenterCross()) {
      ...
      if (
        geometryBBox &&
        !geometryBBox.isEmpty() &&
        geometryBBox.getWidth() > crossSizeMax &&
        geometryBBox.getHeight() > crossSizeMax
      ) {
...
IFShapeTool.prototype._modifiersChanged = function (event) {
  if (
    (this._keepRatio && event.changed.shiftKey) ||
  ...
  if (this._dragStart && this._dragCurrent) {
    ...
    if (IFPoint.equals(this._dragStart, dragCurrent)) {
      ...
      if (this._fromCenter && ifPlatform.modifiers.optionKey) {
        dragArea = IFRect.fromPoints(
          new IFPoint(x0 - (x1 - x0), y0 - (y1 - y0)),
          new IFPoint(x0 + (x1 - x0), y0 + (y1 - y0)),
        );
        dragLine = [
          new IFPoint(x0 - (x2 - x0), y0 - (y2 - y0)),
          new IFPoint(x0 + (x2 - x0), y0 + (y2 - y0)),
        ];
      } else {
      ...
      var dragLineView = [
        worldTransform.mapPoint(dragLine[0]),
        worldTransform.mapPoint(dragLine[1]),
      ];
...
IFShapeTool.prototype._invalidateShapeArea = function (shape) {
  ...
  if (shape) {
    ...
    if (
      geometryBBox &&
      (geometryBBox.getWidth() > 0 || geometryBBox.getHeight() > 0)
    ) {
