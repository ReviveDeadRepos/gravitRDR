...
import { IFPointerTool } from "./pointertool";
import { IFSubSelectTool } from "./subselecttool";
import { IFHandTool } from "./handtool";
import { IFZoomTool } from "./zoomtool";
/**
...
IFToolManager.prototype.indexOf = function (tool) {
  return this._typeIdToIndexMap.hasOwnProperty(IFObject.getTypeId(tool))
    ? this._typeIdToIndexMap[IFObject.getTypeId(tool)]
    : -1;
};
...
IFToolManager.prototype._updateTemporaryTool = function () {
  ...
  if (
    ...
    !ifPlatform.modifiers.spaceKey &&
    this._activeTool !== pointerToolInstance &&
    this._temporaryActiveTool !== pointerToolInstance
  ) {
  ...
  if (
    ...
    (temporaryTool === pointerToolInstance ||
      this._activeTool instanceof IFPointerTool ||
      this._temporaryActiveTool === pointerToolInstance)
  ) {
  ...
  if (
    ifPlatform.modifiers.spaceKey &&
    this._temporaryActiveTool !== handToolInstance
  ) {
  ...
  if (
    ifPlatform.modifiers.metaKey &&
    (temporaryTool === handToolInstance ||
      this._activeTool instanceof IFHandTool)
  ) {
