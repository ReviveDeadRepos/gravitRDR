...
import { GUIMouseEvent } from "@gravitrdr/infinity-core";
import { GUIKeyEvent } from "@gravitrdr/infinity-core";
import { ifPlatform, GUIPlatform } from "@gravitrdr/infinity-core";
import { IFElementEditor } from "../scene/elementeditor";
import { IFNode } from "@gravitrdr/infinity-core";
import { IFVertexContainer } from "@gravitrdr/infinity-core";
import { IFVertex } from "@gravitrdr/infinity-core";
import { IFKey } from "@gravitrdr/infinity-core";
import { ifMath } from "@gravitrdr/infinity-core";
import { IFPathEditor } from "../scene/shape/patheditor";
...
IFSelectTool.prototype.activate = function (view) {
  ...
  ifPlatform.addEventListener(
    GUIPlatform.ModifiersChangedEvent,
...
IFSelectTool.prototype.deactivate = function (view) {
  ...
  ifPlatform.removeEventListener(
    GUIPlatform.ModifiersChangedEvent,
    this._modifiersChanged,
  );
  ...
  if (this._visuals) {
    ...
    for (var i = 0; i < this._visuals.length; ++i) {
      ...
      context.canvas.strokeLine(
        Math.floor(pt0.getX()) + 0.5,
        Math.floor(pt0.getY()) + 0.5,
        Math.floor(pt1.getX()) + 0.5,
        Math.floor(pt1.getY()) + 0.5,
        1,
        context.highlightOutlineColor,
      );
    }
...
IFSelectTool.prototype._mouseDown = function (event) {
  ...
  if (sceneEditor && sceneEditor.isTransformBoxActive()) {
    ...
    this._editorMovePartInfo = sceneEditor.getTBoxPartInfoAt(
      event.client,
      this._view.getWorldTransform(),
    ...
    var stacked =
      ifPlatform.modifiers.metaKey &&
      this._manager.getTemporaryActiveTool() == null;

    if (!stacked) {
      // Try to get a part of an editor, first
      var docEditor = IFElementEditor.getEditor(this._scene);
      if (docEditor) {
        var partInfo = docEditor.getPartInfoAt(
          event.client,
          this._view.getWorldTransform(),
          function (editor) {
        ...
        if (partInfo) {
          ...
          if (
            ...
            (!editor.isPartSelected(partId) && selectable)
          ) {
  ...
  if (this._mode === IFSelectTool._Mode.Select) {
    ...
    if (hitRes) {
      ...
      var elementHits = this._scene.hitTest(
        event.client,
        this._view.getWorldTransform(),
        null,
        stacked,
    ...
    if (selectableElements.length > 0) {
      ...
      if (selectableElements.length > 1) {
        ...
        if (
          lastSelIndex == null ||
          lastSelIndex + 1 >= selectableElements.length
        ) {
          ...
          this._editor.updateSelection(ifPlatform.modifiers.shiftKey, [
            selectableElements[lastSelIndex + 1],
          ]);
        }
      } else {
        ...
        if (
          ifPlatform.modifiers.shiftKey ||
          !this._elementUnderMouse.hasFlag(IFNode.Flag.Selected)
        ) {
...
IFSelectTool.prototype._mouseDrag = function (event) {
  ...
  if (
    this._mode == IFSelectTool._Mode.Moving ||
    this._mode == IFSelectTool._Mode.Transforming
  ) {
    // Save current
  ...
  if (this._mode == IFSelectTool._Mode.Moving) {
    if (this._editorMovePartInfo && this._editorMovePartInfo.isolated) {
      ...
      try {
        ...
        var nodeNameTranslated = this._editorMovePartInfo.editor
          .getElement()
    ...
    if (this._hasSelectArea()) {
      ...
      var mappedSelectArea = this._view
        .getViewTransform()
        .mapRect(this._selectArea);
      var x0 = mappedSelectArea.getX(),
        y0 = mappedSelectArea.getY();
      var x2 = x0 + mappedSelectArea.getWidth(),
        y2 = y0 + mappedSelectArea.getHeight();
      ...
      var collisions = this._scene.getCollisions(
        collisionArea,
        IFElement.CollisionFlag.GeometryBBox,
      );
...
IFSelectTool.prototype._keyDown = function (evt) {
  if (
    evt.key === IFKey.Constant.UP ||
    evt.key === IFKey.Constant.DOWN ||
    evt.key === IFKey.Constant.LEFT ||
    evt.key === IFKey.Constant.RIGHT
  ) {
    // Shift selection if any
    if (
      this._editor.hasSelection() &&
      (!this._mode || this._mode === IFSelectTool._Mode.Moving)
    ) {
      if (this._mode !== IFSelectTool._Mode.Moving) {
        this._updateMode(IFSelectTool._Mode.Moving);
      }
      ...
      var crDistance = ifPlatform.modifiers.shiftKey
        ? this._scene.getProperty("crDistBig")
        : this._scene.getProperty("crDistSmall");
      ...
      this._keyDelta = this._keyDelta
        ? this._keyDelta.add(new IFPoint(dx, dy))
        : new IFPoint(dx, dy);
...
IFSelectTool.prototype._openTransformBox = function () {
  ...
  sceneEditor = sceneEditor
    ? sceneEditor
    : IFElementEditor.openEditor(this._scene);
  ...
  if (this._mode == IFSelectTool._Mode.Moving) {
    ...
    if (this._editorMovePartInfo && this._editorMovePartInfo.isolated) {
      this._editor
        .getGuides()
        .getShapeBoxGuide()
        .useExclusions(this._editor.getSelection());
      this._editor.getGuides().beginMap();
      this._editorMovePartInfo.editor.movePart(
        this._editorMovePartInfo.id,
        this._editorMovePartInfo.data,
        position,
        this._view.getViewTransform(),
        this._editor.getGuides(),
        ifPlatform.modifiers.shiftKey,
        ifPlatform.modifiers.optionKey,
      );
      ...
      if (ifPlatform.modifiers.shiftKey) {
        ...
        position = ifMath.convertToConstrain(
          this._moveStart.getX(),
          this._moveStart.getY(),
          position.getX(),
          position.getY(),
          crConstraint,
        );
      ...
      if (
        ...
        if (this._editorMovePartInfo.id.type == IFPathEditor.PartType.Point) {
          var moveStart = this._editorMovePartInfo.editor.getPointCoord(
    ...
    if (sceneEditor && sceneEditor.isTransformBoxActive() && this._moveStart) {
      var moveCurrentTransformed = this._view
        .getViewTransform()
        .mapPoint(this._moveCurrent);
      sceneEditor.transformTBox(
        this._moveStartTransformed,
        moveCurrentTransformed,
        ifPlatform.modifiers.optionKey,
        ifPlatform.modifiers.shiftKey,
      );
...
IFSelectTool.prototype._updateEditorUnderMouse = function (mouse) {
  ...
  if (this._mode == IFSelectTool._Mode.Transforming) {
    ...
    if (sceneEditor && sceneEditor.isTransformBoxActive()) {
      ...
      sceneEditor.updateTBoxUnderMouse(
        mouse,
        this._view.getWorldTransform(),
        this._view,
      );
  ...
  if (!this._mode) {
    var docEditor = IFElementEditor.getEditor(this._scene);
    if (docEditor) {
      partInfo = docEditor.getPartInfoAt(
        mouse,
        this._view.getWorldTransform(),
        function (editor) {
          // Ensure to allow selected editors for part, only
    ...
    if (!item) {
      ...
      if (elementHits && elementHits.length) {
        for (var i = 0; i < elementHits.length && !item; ++i) {
          if (
            elementHits[i].element instanceof IFItem &&
            !elementHits[i].element.hasFlag(IFNode.Flag.Selected)
          ) {
...
IFSelectTool.prototype._getSelectableElements = function (elements) {
  ...
  for (var i = 0; i < elements.length; ++i) {
    ...
    if (
      selectableElement &&
      selectableElements.indexOf(selectableElement) < 0
    ) {
...
IFSelectTool.prototype._getSelectableElement = function (element) {
  ...
  for (var p = element; p !== null; p = p.getParent()) {
    if (
      p instanceof IFItem &&
      (!p.getParent() || !(p.getParent() instanceof IFItem))
    ) {
      return p;
    }
  }
...
IFSelectTool.prototype._hasSelectArea = function () {
  return (
    this._selectArea &&
    (this._selectArea.getHeight() > 0 || this._selectArea.getWidth() > 0)
  );
};
