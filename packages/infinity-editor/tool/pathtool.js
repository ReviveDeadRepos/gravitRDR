...
IFPathTool.prototype._checkMode = function () {
  ...
  if (!this._pathEditor) {
    ...
    if (this._pathRef.getProperty("closed")) {
      ...
      if (
        selType == IFPathEditor.PointsSelectionType.No ||
  ...
  if (this._pathRef && this._editPt) {
    ...
    newPos = this._view
      ...
      .mapPoint(
        this._editor
          .getGuides()
          .mapPoint(this._view.getViewTransform().mapPoint(newPos)),
      );
 ...
 * @private
 */
IFPathTool.prototype._addPoint = function (
  anchorPt,
  draft,
  nativeCoord,
  oldPreviewSelection,
      ...
      if (this._mode == IFPathTool.Mode.Append) {
        if (!oldPreviewSelection) {
          this._dpathRef
            .getAnchorPoints()
            .getLastChild()
            .removeFlag(IFNode.Flag.Selected);
        }
 ...
 * @private
 */
IFPathTool.prototype._constrainIfNeeded = function (
  pt,
  transform,
  path,
  orientPt,
...
IFPathTool.prototype._mouseDownOnEdit = function (event, customizer) {
  ...
  var partInfo = this._pathEditor.getPartInfoAt(
    eventPt,
    this._view.getWorldTransform(),
    null,
  ...
  if (partInfo && partInfo.id.type == IFPathEditor.PartType.Point) {
    ...
    if (
      !this._pathRef.getProperty("closed") &&
      ...
      if (
        (this._refPt.getProperty("hlx") !== null &&
    ...
    partInfo &&
    partInfo.id.type == IFPathEditor.PartType.Segment &&
    ...
    if (anchorPt) {
      if (
        event.button == GUIMouseEvent.BUTTON_RIGHT &&
    ...
    if (this._pathEditor) {
      ...
      if (partInfo && partInfo.id.type == IFPathEditor.PartType.Point) {
        ...
        if (
          ...
          if (this._mode == IFPathTool.Mode.Edit) {
            if (
              (anchorPt.getProperty("hlx") !== null &&
                anchorPt.getProperty("hly") !== null) ||
              (anchorPt.getProperty("hrx") !== null &&
                anchorPt.getProperty("hry") !== null)
            ) {
