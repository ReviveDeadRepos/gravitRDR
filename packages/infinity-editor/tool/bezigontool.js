...
IFBezigonTool.prototype._mouseDown = function (event) {
  ...
  if (
    ...
    if (this._mode != IFPathTool.Mode.Edit) {
      clickPt = this._constrainIfNeeded(
        event.client,
        this._view.getWorldTransform(),
        this._pathRef,
      );
...
IFBezigonTool.prototype._closeIfNeeded = function () {
  if (
    ...
    if (
      ...
      if (ifPlatform.modifiers.optionKey) {
        otherPt.setProperties(
          ["ah", "tp"],
          [false, IFPathBase.AnchorPoint.Type.Asymmetric],
        );
      }
      if (!otherPt.getProperty("ah")) {
        otherPt.setProperties(
          ["hlx", "hly"],
          [anchorPt.getProperty("hlx"), anchorPt.getProperty("hly")],
        );
      }
    ...
    try {
      ...
      if (
        ...
        if (this._dragStarted && this._editPt) {
          ...
          if (this._editPt) {
            // The case when path has just been closed on mouseDown
            this._commitChanges();
          } else {
            // this._refPt
