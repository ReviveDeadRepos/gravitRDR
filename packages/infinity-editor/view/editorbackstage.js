...
import { IFPaintCanvas } from "@gravitrdr/infinity-core";
 ...
 * @constructor
 */
export function IFEditorBackStage(view) {
  IFStage.call(this, view);
  view
    .getScene()
    .addEventListener(
      IFScene.InvalidationRequestEvent,
      this._sceneInvalidationRequest,
      this,
    );
}
...
IFEditorBackStage.prototype._renderPages = function (context) {
  ...
  for (
    var node = this._view.getScene().getFirstChild();
    node !== null;
    node = node.getNext()
  ) {
    if (
      node instanceof IFPage &&
      node.isRenderable(context) &&
      (!singlePage || node.hasFlag(IFNode.Flag.Active))
    ) {
...
IFEditorBackStage.prototype._renderPage = function (context, transform, page) {
  ...
  var x = transformedPageRect.getX(),
    y = transformedPageRect.getY(),
    w = transformedPageRect.getWidth(),
    h = transformedPageRect.getHeight();
  var mx = transformedMarginRect.getX(),
    my = transformedMarginRect.getY(),
    mw = transformedMarginRect.getWidth(),
    mh = transformedMarginRect.getHeight();

  // Paint page color or chessboard if transparent
  if (!PAGE_CHESSBOARD_FILL) {
    PAGE_CHESSBOARD_FILL = IFPaintCanvas.createChessboard(
      8,
      "white",
      "rgb(205, 205, 205)",
    );
