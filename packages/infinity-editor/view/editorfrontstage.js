 ...
 * @constructor
 */
export function IFEditorFrontStage(view) {
  ...
  view
    ...
    .addEventListener(
      ...
      this._guidesInvalidationRequest,
      this,
    );
...
IFEditorFrontStage.prototype.paint = function (context) {
  this._view
    .getEditor()
    .getGuides()
    .paint(this._view.getWorldTransform(), context);
...
IFEditorFrontStage.prototype._sceneAfterPropertiesChanged = function (event) {
  if (
    event.properties.indexOf("gridSizeX") >= 0 ||
    event.properties.indexOf("gridSizeY") >= 0 ||
