...
import { IFEditorPaintConfiguration } from "../editorpaintconfiguration";
import { GUIWidget } from "@gravitrdr/infinity-core";
import { IFObject } from "@gravitrdr/infinity-core";
import { IFEditorToolStage } from "./editortoolstage";
import { IFEditorBackStage } from "./editorbackstage";
import { IFSceneStage } from "@gravitrdr/infinity-core";
import { IFEditorFrontStage } from "./editorfrontstage";
import { IFEditorSceneStage } from "./editorscenestage";
 ...
 * @version 1.0
 */
export function IFEditorView(editor) {
  ...
  $(this._htmlElement)
    ...
    .on(
      ...
      function (evt) {
        ...
        event.preventDefault();
        event.stopPropagation();
        var client = GUIWidget.convertClientPositionFromMousePosition(
          this._htmlElement,
          event,
        );
  ...
  if (dataTransfer.files && dataTransfer.files.length > 0) {
    ...
    for (var i = 0; i < dataTransfer.items.length; ++i) {
      ...
      if (item.type === IFPattern.MIME_TYPE) {
        ...
        source =
          source && source !== "" ? IFPattern.parsePattern(source) : null;
      ...
      if (type !== null) {
        ...
        var stackedHits = this._scene.hitTest(
          position,
        ...
        if (stackedHits && stackedHits.length > 0) {
          for (var j = 0; j < stackedHits.length; ++j) {
            ...
            if (
              editor &&
              editor.acceptDrop(scenePosition, type, source, hit.data)
            ) {
          ...
          if (
            ...
            source.transform(
              new IFTransform(
                ...
                0,
                1,
                scenePosition.getX() - (elBBox ? elBBox.getX() : 0),
                scenePosition.getY() - (elBBox ? elBBox.getY() : 0),
              ),
            );
