...
import { IFBlock } from "@gravitrdr/infinity-core";
 ...
 * @constructor
 */
export function IFEditor(scene) {
  this._scene = scene;
    ...
    if (history) {
      if (this._lastCloneSelection) {
        for (var i = 0; i < this._lastCloneSelection.length; ++i) {
          ...
          if (
            source.hasMixin(IFElement.Transform) &&
            target.hasMixin(IFElement.Transform)
          ) {
            var sourceTransform = source.getTransform();
            var targetTransform = target.getTransform();
            if (targetTransform) {
              var newTransform = sourceTransform
                ? targetTransform.preMultiplied(sourceTransform.inverted())
                : targetTransform;
  ...
  if (this._selection && this._selection.length > 0) {
    ...
    try {
      ...
      for (var i = 0; i < orderedSelection.length; ++i) {
        ...
        if (
          selElement instanceof IFItem &&
          !selElement.hasFlag(IFElement.Flag.Locked)
        ) {
 ...
 */
IFEditor.prototype.moveSelection = function (
  delta,
  align,
  partId,
  partData,
  ...
  this.transformSelection(
    new IFTransform(1, 0, 0, 1, translation.getX(), translation.getY()),
    partId,
    partData,
  );
};
 ...
 */
IFEditor.prototype.scaleSelection = function (
  sx,
  sy,
  dx,
  dy,
 ...
 * transaction (undo/redo), defaults to false
 */
IFEditor.prototype.applySelectionTransform = function (
  cloneSelection,
  noTransaction,
) {
  if (this._selection && this._selection.length) {
          ...
          if (editor) {
            ...
            if (cloneSelection) {
              if (selectionElement.hasMixin(IFNode.Store)) {
                ...
                selectionElement
                  .getParent()
                  .appendChild(elementToApplyTransform);
 ...
 * transaction (undo/redo), defaults to false
 */
IFEditor.prototype.insertElements = function (
  elements,
  noInitial,
  noTransaction,
) {
...
IFEditor.prototype.updateByMousePosition = function (position, transformation) {
  if (this._scene.getProperty("singlePage") === false) {
    ...
    var pageHits = this._scene.hitTest(
      position,
      transformation,
      function (hit) {
    ...
    if (pageHits && pageHits.length === 1) {
      for (
        var child = this._scene.getFirstChild();
        child !== null;
        child = child.getNext()
      ) {
...
IFEditor.prototype.beginTransaction = function () {
  ...
  this._transactionStack.push({
    ...
    transformBoxCenter: sceneEditor
      ? sceneEditor.getTransformBoxCenter()
      : null,
  });
};
...
IFEditor.prototype._transactionRedo = function (data) {
  ...
  var sceneEditor = IFElementEditor.getEditor(this._scene);
  if (
    data.newTransformBoxCenter ||
    (sceneEditor && sceneEditor.isTransformBoxActive())
  ) {
      ...
      if (
        ...
        if (propertiesAreEqual) {
          ...
          for (var i = 0; i < action.properties.length; ++i) {
            ...
            previousAction.values[previousAction.properties.indexOf(property)] =
              action.values[i];
          }
...
IFEditor.prototype.commitTransaction = function (name) {
  ...
  if (transaction.actions.length > 0) {
    ...
    var data = {
      ...
      newTransformBoxCenter: sceneEditor
        ? sceneEditor.getTransformBoxCenter()
        : null,
    };
    ...
    this.pushState(
      ...
      data,
      this._transactionMerge.bind(this),
    );
  }
};
  ...
  if (editor && editor.canInlineEdit()) {
    if (this.hasEventListeners(IFEditor.InlineEditorEvent)) {
      this.trigger(
        new IFEditor.InlineEditorEvent(
          editor,
          IFEditor.InlineEditorEvent.Type.BeforeOpen,
        ),
      );
    }
    ...
    if (this.hasEventListeners(IFEditor.InlineEditorEvent)) {
      this.trigger(
        new IFEditor.InlineEditorEvent(
          editor,
          IFEditor.InlineEditorEvent.Type.AfterOpen,
        ),
      );
    }
...
IFEditor.prototype._beforeNodeRemove = function (evt) {
  ...
  if (
    evt.node instanceof IFPage &&
    this._scene.getProperty("singlePage") === true
  ) {
...
IFEditor.prototype._tryCloseEditor = function (node) {
  ...
  if (
    !editor ||
    editor.hasFlag(IFElementEditor.Flag.Selected) ||
    editor.hasFlag(IFElementEditor.Flag.Highlighted)
  ) {
  ...
  if (editor && editor.isInlineEdit()) {
    if (this.hasEventListeners(IFEditor.InlineEditorEvent)) {
      this.trigger(
        new IFEditor.InlineEditorEvent(
          editor,
          IFEditor.InlineEditorEvent.Type.BeforeClose,
        ),
      );
    }
    ...
    if (this.hasEventListeners(IFEditor.InlineEditorEvent)) {
      this.trigger(
        new IFEditor.InlineEditorEvent(
          editor,
          IFEditor.InlineEditorEvent.Type.AfterClose,
        ),
      );
    }
 ...
 * @private
 */
IFEditor.prototype._savePageSelection = function (
  page,
  noOverwrite,
  checkOverwrite,
) {
