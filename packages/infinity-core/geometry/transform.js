 ...
 * @version 1.0
 */
export function IFTransform(sx, shy, shx, sy, tx, ty) {
...
IFTransform.equals = function (left, right) {
  if (left && left == right) {
    ...
    return (
      ifMath.isEqualEps(left._sx, right._sx) &&
      ifMath.isEqualEps(left._shy, right._shy) &&
      ifMath.isEqualEps(left._shx, right._shx) &&
      ifMath.isEqualEps(left._sy, right._sy) &&
      ifMath.isEqualEps(left._tx, right._tx) &&
      ifMath.isEqualEps(left._ty, right._ty)
    );
  } else {
    return false;
  }
...
IFTransform.prototype.isIdentity = function () {
  return (
    ifMath.isEqualEps(this._sx, 1.0) &&
    ifMath.isEqualEps(this._shy, 0.0) &&
    ifMath.isEqualEps(this._shx, 0.0) &&
    ifMath.isEqualEps(this._sy, 1.0) &&
    ifMath.isEqualEps(this._tx, 0.0) &&
    ifMath.isEqualEps(this._ty, 0.0)
  );
};
...
IFTransform.prototype.isValid = function () {
  return (
    Math.abs(this._sx) > ifMath.defaultEps &&
    Math.abs(this._sy) > ifMath.defaultEps
  );
};
...
IFTransform.prototype.skewed = function (sx, sy) {
  return this.multiplied(
    new IFTransform(1.0, Math.tan(sy), Math.tan(sx), 1.0, 0.0, 0.0),
  );
};
