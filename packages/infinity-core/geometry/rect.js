import { IFPoint } from "./point";
import { ifMath } from "../core/math";
import { IFTransform } from "./transform";
 ...
 * @version 1.0
 */
export function IFRect(x, y, width, height) {
  this._x = x ? x : 0;
  this._y = y ? y : 0;
...
IFRect.equals = function (left, right) {
  if (!left && left === right) {
    ...
    return (
      ifMath.isEqualEps(left._x, right._x) &&
      ifMath.isEqualEps(left._y, right._y) &&
      ifMath.isEqualEps(left._width, right._width) &&
      ifMath.isEqualEps(left._height, right._height)
    );
  } else {
    return false;
  }
...
IFRect.prototype.containsPointXY = function (px, py) {
  var l = this._x;
  var r = this._x;
  if (this._width < 0) l += this._width;
  else r += this._width;
  if (l == r) return false;

  if (px < l || px > r) return false;

  var t = this._y;
  var b = this._y;
  if (this._height < 0) t += this._height;
  else b += this._height;
  if (t == b) return false;

  if (py < t || py > b) return false;
...
IFRect.prototype.containsRect = function (rect) {
  var l1 = this._x;
  var r1 = this._x;
  if (this._width < 0) l1 += this._width;
  else r1 += this._width;
  if (l1 == r1) return false;

  var l2 = rect._x;
  var r2 = rect._x;
  if (rect._width < 0) l2 += rect._width;
  else r2 += rect._width;
  ...
  var t1 = this._y;
  var b1 = this._y;
  if (this._height < 0) t1 += this._height;
  else b1 += this._height;
  if (t1 == b1) return false;

  var t2 = rect._y;
  var b2 = rect._y;
  if (rect._height < 0) t2 += rect._height;
  else b2 += rect._height;
  if (t2 == b2) return false;
...
IFRect.prototype.united = function (otherRect) {
  ...
  if (l1 == r1) return otherRect;

  var l2 = otherRect._x;
  var r2 = otherRect._x;
  if (otherRect._width < 0) l2 += otherRect._width;
  else r2 += otherRect._width;
  ...
  if (t1 == b1) return otherRect;

  var t2 = otherRect._y;
  var b2 = otherRect._y;
  if (otherRect._height < 0) t2 += otherRect._height;
  else b2 += otherRect._height;
  if (t2 == b2) return this;
...
IFRect.prototype.intersected = function (otherRect) {
  ...
  if (l1 == r1) return new IFRect(0, 0, 0, 0);

  var l2 = otherRect._x;
  var r2 = otherRect._x;
  if (otherRect._width < 0) l2 += otherRect._width;
  else r2 += otherRect._width;
  if (l2 == r2) return new IFRect(0, 0, 0, 0);

  if (l1 >= r2 || l2 >= r1) return new IFRect(0, 0, 0, 0);

  var t1 = this._y;
  var b1 = this._y;
  if (this._height < 0) t1 += this._height;
  else b1 += this._height;
  if (t1 == b1) return new IFRect(0, 0, 0, 0);

  var t2 = otherRect._y;
  var b2 = otherRect._y;
  if (otherRect._height < 0) t2 += otherRect._height;
  else b2 += otherRect._height;
  if (t2 == b2) return new IFRect(0, 0, 0, 0);

  if (t1 >= b2 || t2 >= b1) return new IFRect(0, 0, 0, 0);
