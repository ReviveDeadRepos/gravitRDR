...
IFPoint.min = function () {
  ...
  for (var i = 0; i < arguments.length; ++i) {
    if (result._x == null || arguments[i]._x < result._x)
      result._x = arguments[i]._x;
    if (result._y == null || arguments[i]._y < result._y)
      result._y = arguments[i]._y;
  }
  return result;
};
...
IFPoint.prototype.rotated = function (angle) {
  ...
  return new IFPoint(
    cos_ * this._x - sin_ * this._y,
    sin_ * this._x + cos_ * this._y,
  );
};
...
IFPoint.prototype.rotatedAt = function (angle, origin) {
  ...
  return new IFPoint(
    cos_ * (this._x - origin._x) - sin_ * (this._y - origin._y) + origin._x,
    sin_ * (this._x - origin._x) + cos_ * (this._y - origin._y) + origin._y,
  );
};
