import { IFVertexSource } from "./vertexsource";
import { IFObject } from "../core/object";
import { IFPoint } from "../geometry/point";
import { IFVertex } from "./vertex";
import { ifMath } from "../core/math";
import { IFVertexContainer } from "./vertexcontainer";
/**
 ...
 * @constructor
 */
export function IFVertexOffsetter(source, offset, inset, outset, tol) {
...
IFVertexOffsetter.PolyOffsetSegment = function (
  basepoint,
  point1,
  point2,
  bulge,
...
IFObject.inherit(
  IFVertexOffsetter.PolyOffsetSegment,
  IFVertexOffsetter.PolySegment,
);
...
IFVertexOffsetter.IntersectionResult = function () {
  this.intTypes = [
    new IFVertexOffsetter.IntersectionType(),
    new IFVertexOffsetter.IntersectionType(),
  ];
};
...
IFVertexOffsetter.PolySegmentContainer.prototype.insertSegment = function (
  polySegment,
  next,
) {
...
IFVertexOffsetter.PolySegmentContainer.prototype.deleteSegment = function (
  polySegment,
) {
 ...
 */
IFVertexOffsetter.prototype.addCurveToPolyline = function (
  B0,
  B1,
  B2,
  tolerance,
  ...
  // N(u) = D(u) / ||D(u)||
  var D0 = new IFPoint(
    a * (T1.getX() - T0.getX() * cosEtha),
    a * (T1.getY() - T0.getY() * cosEtha),
  );
  var tmp1 = a * cosEtha - b;
  var tmp2 = b * cosEtha - a;
  var D1 = new IFPoint(
    D0.getX() + tmp1 * T0.getX() + tmp2 * T1.getX(),
    D0.getY() + tmp1 * T0.getY() + tmp2 * T1.getY(),
  );

  tmp1 = Math.sqrt(
    ifMath.vDotProduct(D0.getX(), D0.getY(), D0.getX(), D0.getY()),
  );
  tmp2 = Math.sqrt(
    ifMath.vDotProduct(D1.getX(), D1.getY(), D1.getX(), D1.getY()),
  );
  // TODO: check for zeros
  ...
  var tMax = sigma;
  roots = [];
  ifMath.getQuadraticRoots(
    tmp1,
    3 * (abcosEtha - aSqr),
    2 * aSqr - Rb0 * b * sinEtha,
    roots,
  );
  if (roots[0] != null && roots[0] < sigma && roots[0] > 0) {
    var Qt01 = new IFPoint(
      ifMath.getCurveAtT(B0.getX(), B2.getX(), B1.getX(), roots[0]),
      ifMath.getCurveAtT(B0.getY(), B2.getY(), B1.getY(), roots[0]),
    );
    var pho01 = Math.abs(
      Rb0 - ifMath.ptDist(Qt01.getX(), Qt01.getY(), C0.getX(), C0.getY()),
    );
    if (delta < pho01) {
      delta = pho01;
      tMax = roots[0];
  ...
  if (roots[1] != null && roots[1] < sigma && roots[1] > 0) {
    var Qt02 = new IFPoint(
      ifMath.getCurveAtT(B0.getX(), B2.getX(), B1.getX(), roots[1]),
      ifMath.getCurveAtT(B0.getY(), B2.getY(), B1.getY(), roots[1]),
    );
    var pho02 = Math.abs(
      Rb0 - ifMath.ptDist(Qt02.getX(), Qt02.getY(), C0.getX(), C0.getY()),
    );
    if (delta < pho02) {
      delta = pho02;
      tMax = roots[1];
  ...
  ifMath.getQuadraticRoots(
    tmp1,
    -2 * aSqr + bSqr + abcosEtha,
    aSqr + abcosEtha - Rb2 * a * sinEtha,
    roots,
  );
  if (roots[0] != null && roots[0] < 1 && roots[0] > sigma) {
    var Qt11 = new IFPoint(
      ifMath.getCurveAtT(B0.getX(), B2.getX(), B1.getX(), roots[0]),
      ifMath.getCurveAtT(B0.getY(), B2.getY(), B1.getY(), roots[0]),
    );
    var pho11 = Math.abs(
      Rb2 - ifMath.ptDist(Qt11.getX(), Qt11.getY(), C1.getX(), C1.getY()),
    );
    if (delta < pho11) {
      delta = pho11;
      tMax = roots[0];
  ...
  if (roots[1] != null && roots[1] < 1 && roots[1] > sigma) {
    var Qt12 = new IFPoint(
      ifMath.getCurveAtT(B0.getX(), B2.getX(), B1.getX(), roots[1]),
      ifMath.getCurveAtT(B0.getY(), B2.getY(), B1.getY(), roots[1]),
    );
    var pho12 = Math.abs(
      Rb2 - ifMath.ptDist(Qt12.getX(), Qt12.getY(), C1.getX(), C1.getY()),
    );
    if (delta < pho12) {
      delta = pho12;
      tMax = roots[1];
  ...
  if (delta > tolerance && counter < IFVertexOffsetter.MAX_RECURS) {
    ...
    this.addCurveToPolyline(
      B0,
      new IFPoint(ctrls1X[1], ctrls1Y[1]),
      new IFPoint(ctrls1X[2], ctrls1Y[2]),
      tolerance,
      counter,
    );
    this.addCurveToPolyline(
      new IFPoint(ctrls2X[0], ctrls2Y[0]),
      new IFPoint(ctrls2X[1], ctrls2Y[1]),
      B2,
      tolerance,
      counter,
    );
    ...
    // G = V + lambda*T
    var G = new IFPoint(
      B0.getX() + lambda * (T0.getX() + T.getX()),
      B0.getY() + lambda * (T0.getY() + T.getY()),
    );
 ...
 * @param {Number} tolerance
 */
IFVertexOffsetter.prototype.addCubicCurveToPolyline = function (
  B0,
  B1,
  B2,
  B3,
...
IFVertexOffsetter.prototype._isQudraticCurveFlat = function (
  B0,
  B1,
  B2,
  tolerance,
...
IFVertexOffsetter.prototype._isCubicCurveFlat = function (
  ctrlsx,
  ctrlsy,
  tolerance,
) {
 ...
 * @param {Number} counter
 */
IFVertexOffsetter.prototype._addCubicSegmToPolyline = function (
  ctrlsx,
  ctrlsy,
  tolerance,
  counter,
  ...
  var C;
  var hndl1 =
    !ifMath.isEqualEps(xA, ctrlsx[1]) || !ifMath.isEqualEps(yA, ctrlsy[1]);
  var hndl2 =
    !ifMath.isEqualEps(xB, ctrlsx[2]) || !ifMath.isEqualEps(yB, ctrlsy[2]);
  if (hndl1 && hndl2) {
    var result = [];
    C = ifMath.getIntersectionPoint(
      xA,
      yA,
      ctrlsx[1],
      ctrlsy[1],
    ...
    this._polyline.insertSegment(
      new IFVertexOffsetter.PolySegment(new IFPoint(xA, yA), 0),
    );
    return;
  }
  ...
  if (
    ifMath.isEqualEps(xA, xG) ||
    ifMath.isEqualEps(yA, yG) ||
    ifMath.isEqualEps(xB, xG) ||
    ifMath.isEqualEps(yB, yG)
  ) {
    ...
    var mB = (yB - yG) / (xB - xG);
    var xO =
      (mA * mB * (yA - yB) + mB * (xA + xG) - mA * (xG + xB)) / 2 / (mB - mA);
    ...
    if (nRoots > 0) {
      ...
      if (nRoots > 1) {
        ...
        nSignVars = [];
        fVals = [v51, v52];
        nRoots = ifMath.countRootsNSturm(
          coeffFDeriv,
          5,
          coeffFDeriv2,
          t1,
    ...
    if (maxDst > tolerance && counter < IFVertexOffsetter.MAX_RECURS) {
      ...
      var arc = new IFVertexOffsetter.PolySegment(
        new IFPoint(xA, yA),
        tgHalfGamma,
        new IFPoint(xO, yO),
        R,
...
IFVertexOffsetter.prototype.generatePolyLine = function (
  tolerance,
  startVertex,
) {
    ...
    switch (vertex2.command) {
      ...
      case IFVertex.Command.Line:
        if (!vertex1) {
          ...
          polySegm = new IFVertexOffsetter.PolySegment(
            new IFPoint(vertex1.x, vertex1.y),
            0,
          );
      ...
      case IFVertex.Command.Curve:
        if (!vertex1) {
          ...
          if (this._source.readVertex(vertex2)) {
            ...
            this.addCurveToPolyline(
              B0,
              new IFPoint(vertex2.x, vertex2.y),
              new IFPoint(vertex1.x, vertex1.y),
              tolerance,
              counter,
            );
      ...
      case IFVertex.Command.Curve2:
        if (!vertex1) {
          ...
          if (
            this._source.readVertex(vertex2) &&
            this._source.readVertex(vertex3)
          ) {
            this.addCubicCurveToPolyline(
              B0,
              new IFPoint(vertex2.x, vertex2.y),
              new IFPoint(vertex3.x, vertex3.y),
              B3,
              tolerance,
            );
  ...
  if (vertex1 && this._polyline.count) {
    ...
    this._polyline.insertSegment(polySegm);
    if (
      ifMath.isEqualEps(vertex1.x, this._polyline.head.point.getX()) &&
...
IFVertexOffsetter.prototype.generatePolyOffset = function (
  offset,
  inset,
  outset,
  tolerance,
  ...
  var offsSegm;
  for (
    var i = 0, curSegm = this._polyline.head;
    i < this._polyline.count;
    ++i
  ) {
    if (outset) {
      ...
      for (var i = 0; i < outSplit.length; ++i) {
        ...
        if (intPts.length == 0) {
          ...
          for (var j = 0; j < intPtsMain.length; ++j) {
            if (
              intPtsMain[j].segmIdx == 0 ||
              intPtsMain[j].segmIdx == this._polyline.count - 1
            ) {
              endSegm = true;
              break;
            }
...
IFVertexOffsetter.prototype.generateOffset = function (
  inset,
  outset,
  tolerance,
) {
...
IFVertexOffsetter.prototype._mergePieces = function (piecesTmp) {
  ...
  while (piecesTmp.length > 0) {
    ...
    var repeat =
      piece.stPt.getX() != piece.endPt.getX() ||
      piece.stPt.getY() != piece.endPt.getY();
    while (repeat) {
      repeat = false;
      var pc = null;
      for (var i = piecesTmp.length - 1; i >= 0; --i) {
        pc = piecesTmp[i];
        if (
          pc.stPt.getX() == piece.endPt.getX() &&
          pc.stPt.getY() == piece.endPt.getY()
        ) {
          ...
          piece.stPt.getX() == pc.endPt.getX() &&
          piece.stPt.getY() == pc.endPt.getY()
        ) {
 ...
 * @private
 */
IFVertexOffsetter.prototype._offsetPolySegment = function (
  segm,
  offset,
  tolerance,
) {
 ...
 * @private
 */
IFVertexOffsetter.prototype._insersectOffsetSegments = function (
  psegm1,
  psegm2,
  intResult,
) {
  if (psegm1.bulge == 0 && psegm2.bulge == 0) {
    // line segments
    var res = [null, null];
    ...
    if (psegm1.bulge == 0) {
      ...
      if (res[0] != null && res[1] != null) {
        var x2 = xL + dxL * res[1];
        var y2 = yL + dyL * res[1];
        var sDst1 = ifMath.ptSqrDist(
          x,
          y,
          psegm2.basepoint.getX(),
          psegm2.basepoint.getY(),
        );
        var sDst2 = ifMath.ptSqrDist(
          x2,
          y2,
          psegm2.basepoint.getX(),
          psegm2.basepoint.getY(),
    ...
    // two arcs
    var res = [null, null];
    ifMath.circleCircleIntersection(
      psegm1.center.getX(),
      psegm1.center.getY(),
      psegm1.radius,
      psegm2.center.getX(),
    ...
    if (ptIdx !== null) {
      ...
      this._fillArcIntType(
        psegm1,
        res[ptIdx].getX(),
        res[ptIdx].getY(),
        intResult.intTypes[0],
      );
      this._fillArcIntType(
        psegm2,
        res[ptIdx].getX(),
        res[ptIdx].getY(),
        intResult.intTypes[1],
      );
    }
  }
};
...
IFVertexOffsetter.prototype._fillArcIntType = function (psegm, x, y, intType) {
  ...
  if (
    ifMath.segmentSide(sp1x, sp1y, sp2x, sp2y, x, y) ==
    ...
    if (
      ifMath.segmentSide(sp1x, sp1y, pOppX, pOppY, x, y) ==
 ...
 * @private
 */
IFVertexOffsetter.prototype._trimOffsetPoly = function (
  polyOffset,
  offset,
  polyONew,
) {
  ...
  if (segm2) {
    polyONew.insertSegment(
      new IFVertexOffsetter.PolySegment(
        segm2.point,
        segm2.bulge,
        segm2.center,
        segm2.radius,
    ...
    for (var i = 0; i < polyOffset.count - 1; ++i) {
      ...
      segm1 = new IFVertexOffsetter.PolyOffsetSegment(
        segm2.basepoint,
        polyONew.end.point,
        segm2.point2,
        polyONew.end.bulge,
      ...
      if (
        !ifMath.isEqualEps(segm1.point2.getX(), segm2.point.getX(), eps) ||
        !ifMath.isEqualEps(segm1.point2.getY(), segm2.point.getY(), eps)
      ) {
        iRes.clear();
        this._insersectOffsetSegments(segm1, segm2, iRes);

        if (segm1.bulge == 0 && segm2.bulge == 0) {
          // Two line segments: use Algorithm 1
          if (!iRes.point) {
            // case 1
            polyONew.insertSegment(
              new IFVertexOffsetter.PolySegment(segm1.point2, 0),
            );
          } else if (
            (iRes.intTypes[0].TIP && iRes.intTypes[1].TIP) || // case 2a
            (iRes.intTypes[0].FIP &&
              iRes.intTypes[1].FIP &&
              iRes.intTypes[0].PFIP)
          ) {
            // case 2b part1
            polyONew.insertSegment(
              new IFVertexOffsetter.PolySegment(iRes.point, 0),
            );
          } else {
            // case 2b part2 || case 2c
            polyONew.insertSegment(
              new IFVertexOffsetter.PolySegment(segm1.point2, 0),
            );
            polyONew.insertSegment(
              new IFVertexOffsetter.PolySegment(segm2.point, 0),
            );
          }
        } else if (segm1.bulge == 0 && segm2.bulge != 0) {
          // Line segment and arc segment: use Algorithm 2
          if (iRes.point) {
            // case 1
            if (
              (iRes.intTypes[0].TIP && iRes.intTypes[1].TIP) || // case 1a
              // TIP && NFIP || PFIP && TIP -> should not be possible, might be some error,
              // behave the same as when TIP for both
              (iRes.intTypes[0].TIP &&
                iRes.intTypes[1].FIP &&
                !iRes.intTypes[1].PFIP) ||
              ...
              var bulge = this._calculateBulge(
                iRes.point.getX(),
                iRes.point.getY(),
                segm2.point2.getX(),
                segm2.point2.getY(),
            ...
            } else if (iRes.intTypes[0].PFIP && iRes.intTypes[1].FIP) {
              // case 1b
              ...
              //iRes.intTypes[0].FIP && !iRes.intTypes[0].PFIP && iRes.intTypes[1].TIP ||  case 1c
          ...
          // Arc segment and line segment: use Algorithm 3
          if (iRes.point) {
            // case 1
            if (iRes.intTypes[0].TIP && iRes.intTypes[1].TIP) {
              // case 1a
              var newBulge = this._calculateBulge(
                segm1.point.getX(),
                segm1.point.getY(),
                iRes.point.getX(),
                iRes.point.getY(),
          ...
          // two arc segments: use Algorithm 4
          if (iRes.point) {
            // case 1
            if (
              !(iRes.intTypes[0].FIP && !iRes.intTypes[0].PFIP) &&
              !iRes.intTypes[1].PFIP
            ) {
              // case 1a
    ...
    if (this._polyline.closed) {
      ...
      polyONew.insertSegment(
        new IFVertexOffsetter.PolySegment(segm2.point2, 0),
      );
    }
  }
};
 ...
 * @private
 */
IFVertexOffsetter.prototype._constructJoinArc = function (
  segm1,
  segm2,
  collinear,
) {
  ...
  if (segm1.bulge) {
    ...
    sign = -ifMath.segmentSide(
      segm1.point2.getX(),
      segm1.point2.getY(),
      segm1.point2.getX() + dx,
      segm1.point2.getY() + dy,
      segm2.point.getX(),
      segm2.point.getY(),
    ); // * segm1.bulge;
  } else {
  ...
  var bulge = 0;
  if (sign) {
    bulge = this._calculateBulge(
      segm1.point2.getX(),
      segm1.point2.getY(),
      segm2.point.getX(),
      segm2.point.getY(),
      segm2.basepoint.getX(),
      segm2.basepoint.getY(),
      sign,
    );
  }
  if (bulge) {
    arc = new IFVertexOffsetter.PolySegment(
      segm1.point2,
      bulge,
      segm2.basepoint,
      ifMath.ptDist(
...
IFVertexOffsetter.prototype._calculateBulge = function (
  p1x,
  p1y,
  p2x,
  p2y,
  ...
  if (ifMath.isEqualEps(d1, d2, 0.00000001)) {
    ...
    if (
      exampBulge &&
      ((exampBulge < 0 && bulge > 0) || (exampBulge > 0 && bulge < 0))
    ) {
 ...
 * @private
 */
IFVertexOffsetter.prototype._calcIntersectionPoints = function (
  polyLn1,
  polyLn2,
  intPts1,
  intPts2,
 ...
 * @private
 */
IFVertexOffsetter.prototype._calcSelfIntersectionPoints = function (
  polyLn,
  intPts,
) {
  var s1 = polyLn.head;
  ...
  for (var i = 0; i < polyLn.count - 2; ++i) {
    ...
    for (var j = i + 1; j < polyLn.count - 1; ++j) {
      this._calcSegmIntersectionPoints(
        s1,
        s2,
        i,
        j,
...
IFVertexOffsetter.prototype._calcSegmIntersectionPoints = function (
  ...
  if (!s1.bulge && !s2.bulge) {
    if (!ignoreCommonEnd) {
      var res = [null, null];
      var iPt = ifMath.getIntersectionPoint(
        s1.point.getX(),
        s1.point.getY(),
        s1.next.point.getX(),
        s1.next.point.getY(),
      ...
      if (iPt && 0 <= res[0] && res[0] <= 1 && 0 <= res[1] && res[1] <= 1) {
        intPts1.push(
          new IFVertexOffsetter.IntersectionPt(
            iPt.getX(),
            iPt.getY(),
            res[0],
            s1,
    ...
    if (
      ...
      res[0] >= 0 &&
      res[0] <= 1 &&
      (!ignoreCommonEnd || !ifMath.isEqualEps(res[0], 1))
    ) {
      ...
      if (
        (bulge < 0 && bulge >= s2.bulge) ||
        (bulge > 0 && bulge <= s2.bulge)
      ) {
        intPts1.push(
          new IFVertexOffsetter.IntersectionPt(x, y, res[0], s1, idx1),
        );
        intPts2.push(
          new IFVertexOffsetter.IntersectionPt(x, y, bulge, s2, idx2),
        );
      }
    }
    ...
    if (
      ...
      res[1] >= 0 &&
      res[1] <= 1 &&
      (!ignoreCommonEnd || !ifMath.isEqualEps(res[1], 1))
    ) {
      ...
      if (
        ...
        intPts1.push(
          new IFVertexOffsetter.IntersectionPt(x, y, res[1], s1, idx1),
        );
        intPts2.push(
          new IFVertexOffsetter.IntersectionPt(x, y, bulge, s2, idx2),
        );
    ...
    if (
      ...
      res[0] >= 0 &&
      res[0] <= 1 &&
      (!ignoreCommonEnd || !ifMath.isEqualEps(res[0], 0))
    ) {
      ...
      if (
        (bulge < 0 && bulge >= s1.bulge) ||
        (bulge > 0 && bulge <= s1.bulge)
      ) {
        intPts1.push(
          new IFVertexOffsetter.IntersectionPt(x, y, bulge, s1, idx1),
        );
        intPts2.push(
          new IFVertexOffsetter.IntersectionPt(x, y, res[0], s2, idx2),
        );
      }
    }
    ...
    if (
      ...
      res[1] >= 0 &&
      res[1] <= 1 &&
      (!ignoreCommonEnd || !ifMath.isEqualEps(res[1], 0))
    ) {
      ...
      if (
        ...
        intPts2.push(
          new IFVertexOffsetter.IntersectionPt(x, y, res[1], s2, idx2),
        );
      }
    }
  } else {
    ...
    if (
      ...
      var bulge01 = this._calculateBulge(
        s1.point.getX(),
        s1.point.getY(),
        res[0].getX(),
        res[0].getY(),
      ...
      if (
        (bulge01 < 0 && bulge01 >= s1.bulge) ||
        (bulge01 > 0 && bulge01 <= s1.bulge)
      ) {
        var bulge02 = this._calculateBulge(
          s2.point.getX(),
          s2.point.getY(),
          res[0].getX(),
          res[0].getY(),
    ...
    if (
      ...
      var bulge11 = this._calculateBulge(
        s1.point.getX(),
        s1.point.getY(),
        res[1].getX(),
        res[1].getY(),
      ...
      if (
        (bulge11 < 0 && bulge11 >= s1.bulge) ||
        (bulge11 > 0 && bulge11 <= s1.bulge)
      ) {
        var bulge12 = this._calculateBulge(
          s2.point.getX(),
          s2.point.getY(),
          res[1].getX(),
          res[1].getY(),
...
IFVertexOffsetter.prototype._sortInsertsectionPoints = function (intPts) {
  intPts.sort(function (p1, p2) {
    return p1.segmIdx != p2.segmIdx
      ? p1.segmIdx - p2.segmIdx
      : p1.slope > 0
        ? p1.slope - p2.slope
        : p2.slope - p1.slope;
  });
    ...
    if (!segm.bulge) {
      ...
      if (
        ...
        if (segm == segmOrig) {
          ...
          bulge =
            (intPts[i].slope - intPts[i - 1].slope) /
            (1 + intPts[i].slope * intPts[i - 1].slope);
          split[i].insertSegment(
            new IFVertexOffsetter.PolySegment(
              segm.point,
              bulge,
        ...
        // TODO: check formula here
        bulge =
          (segmOrig.bulge - intPts[i].slope) /
          (1 + segmOrig.bulge * intPts[i].slope);
        segm = new IFVertexOffsetter.PolySegment(
          new IFPoint(intPts[i].x, intPts[i].y),
          bulge,
          segm.center,
        ...
        split[i].insertSegment(
          new IFVertexOffsetter.PolySegment(segm.point, 0),
        );
      }
    }
    startIdx = j;
    if (
      ifMath.isEqualEps(intPts[i].x, segmOrig.next.point.getX(), eps) &&
...
IFVertexOffsetter.prototype._excludeCircleInside = function (
  polyLn,
  intPts,
  intPtsMain,
  offset,
  ...
  var segmIdx = 0;
  var segm = new IFVertexOffsetter.PolySegment(
    polyLn.head.point,
    polyLn.head.bulge,
    polyLn.head.center,
    polyLn.head.radius,
    ...
    if (!segm.bulge) {
      ...
      if (res[0] != null) {
        if (res[0] <= 0) {
          ...
          tmpPoly.insertSegment(
            new IFVertexOffsetter.PolySegment(new IFPoint(x, y), 0),
          );
          resArray.push(tmpPoly);
        } else {
          tmpPoly.insertSegment(
          ...
          tmpPoly.insertSegment(
            new IFVertexOffsetter.PolySegment(s.next.point, 0),
          );
          resArray.push(tmpPoly);
        }
      }
      ...
      if (res[0] != null) {
        var bulge0 = this._calculateBulge(
          segm.point.getX(),
          segm.point.getY(),
          res[0].getX(),
          res[0].getY(),
      ...
      if (res[1] != null) {
        var bulge1 = this._calculateBulge(
          segm.point.getX(),
          segm.point.getY(),
          res[1].getX(),
          res[1].getY(),
 ...
 * @private
 */
IFVertexOffsetter.prototype._gcppClipping = function (
  polyLns,
  basePolyLn,
  offset,
  resPolyLns,
 ...
 * @private
 */
IFVertexOffsetter.prototype._gcppFilter = function (
  polyLns,
  basePolyLn,
  offset,
  tolerance,
...
IFVertexOffsetter.prototype._getPlSegmSqrDist = function (
  s1,
  pt12,
  s2,
  pt22,
...
IFVertexOffsetter.prototype._getSegmToArcSqrDist = function (
  apt1,
  bulge,
  cntr,
  rd,
  ...
  if (
    ifMath.isEqualEps(spt1.getX(), spt2.getX()) &&
    ifMath.isEqualEps(spt1.getY(), spt2.getY())
  ) {
  ...
  if (intRes[0] != null && intRes[1] == null) {
    ...
    var ptBulge = this._calculateBulge(
      apt1.getX(),
      apt1.getY(),
      intRes[0].getX(),
      intRes[0].getY(),
    ...
    if (intRes[0] == null) {
      ...
      if (ptMin[0] > 0 && ptMin < 1) {
        ...
        var tptBulge = this._calculateBulge(
          apt1.getX(),
          apt1.getY(),
          tptX,
          tptY,
...
IFVertexOffsetter.prototype._getPtToArcSqrDist = function (
  apt1,
  bulge,
  cntr,
  rd,
...
IFVertexOffsetter.prototype._getArcToArcSqrDist = function (
  s1,
  pt12,
  s2,
  pt22,
  ...
  if (
    !intRes[0] &&
    (!ifMath.isEqualEps(c1x, c2x) || !ifMath.isEqualEps(c1y, c2y))
  ) {
    var dxL = c2x - c1x;
    var dyL = c2y - c1y;
...
IFVertexOffsetter.prototype._genCurves = function (polyLns, outset, tolerance) {
  ...
  for (var i = 0; i < polyLns.length; ++i) {
    ...
    var segm = poln.head;
    var vertices = [];
    var stPt = new IFPoint(
      this._roundOut(segm.point.getX()),
      this._roundOut(segm.point.getY()),
    );
    var endPt = stPt;
    for (var j = 0; j < poln.count - 1; ++j) {
      if (!segm.bulge) {
        ...
        if (j == poln.count - 2) {
          endPt = new IFPoint(
            this._roundOut(segm.next.point.getX()),
            this._roundOut(segm.next.point.getY()),
          );
        }
      } else {
...
IFVertexOffsetter.prototype._getCubicBezierArcError = function (alpha, radius) {
  ...
  var c0 =
    -19.65763511 +
    0.00022979 * cosAlpha +
    0.00042715 * cos2Alpha +
    0.00103256 * cos3Alpha;
  var c1 =
    9.92683097 +
    0.00045907 * cosAlpha +
    0.00174813 * cos2Alpha -
    0.00034153 * cos3Alpha;
...
IFVertexOffsetter.prototype._readVertex = function (vertex) {
  if (
    ...
    this._pieces[this._pieceIdx].readVertex(vertex)
  ) {
    return true;
  } else {
    ++this._pieceIdx;
