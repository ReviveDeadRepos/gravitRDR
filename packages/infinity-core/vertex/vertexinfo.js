...
/**
     ...
     * @version 1.0
     */
IFVertexInfo.prototype._hitTestSegment = function (
  px1,
  py1,
  px2,
  py2,
  ...
  var ptMin = [];
  var sqrDst = ifMath.sqrSegmentDist(
    px1,
    py1,
    px2,
    py2,
...
/**
     ...
     * @version 1.0
     */
IFVertexInfo.prototype._hitTestCurve = function (
  px1,
  py1,
  px2,
  py2,
  ...
  if (d1 <= sqrOutline) {
    ...
    ifMath.segmentSide(px1, py1, px2, py2, x, y) *
      ifMath.segmentSide(px1, py1, px2, py2, middlePtx, middlePty) <
    0
  ) {
    insideFlag = false;
  }
  ...
  if (d2 <= sqrOutline) {
    ...
    ifMath.segmentSide(px1, py1, cx, cy, x, y) *
      ifMath.segmentSide(px1, py1, cx, cy, middlePtx, middlePty) <
    0
  ) {
    insideFlag = false;
  }
  ...
  if (d3 <= sqrOutline) {
    ...
    ifMath.segmentSide(cx, cy, px2, py2, x, y) *
      ifMath.segmentSide(cx, cy, px2, py2, middlePtx, middlePty) <
    0
  ) {
    insideFlag = false;
  }
...
/**
     ...
     * @version 1.0
     */
IFVertexInfo.prototype._hitTestCurve2 = function (
  px1,
  py1,
  px2,
  py2,
  ...
  for (var i = 0; i < nPoints - 1; ++i) {
    ...
    for (j = 0; j < 4; ++j) {
      ...
      if (
        ifMath.segmentSide(ctrlsx[j], ctrlsy[j], ctrlsx[k], ctrlsy[k], x, y) *
          ifMath.segmentSide(
            ctrlsx[j],
            ctrlsy[j],
            ctrlsx[k],
    ...
    if (nRoots > 1) {
      ...
      nSignVars = [];
      fVals = [v51, v52];
      nRootsNew = ifMath.countRootsNSturm(
        coeffFDeriv,
        5,
        coeffFDeriv2,
        A1,
...
/**
     ...
     * @version 1.0
     */
IFVertexInfo.prototype._hitUnderSegment = function (
  px1,
  py1,
  px2,
  py2,
...
/**
     ...
     * @version 1.0
     */
IFVertexInfo.prototype._evalConicReg = function (
  px1,
  py1,
  px2,
  py2,
  ...
  if (chullres == 0) {
    // point is on the foundation of conic region
...
/**
     ...
     * @version 1.0
     */
IFVertexInfo.prototype._hitUnderCurve = function (
  px1,
  py1,
  px2,
  py2,
  ...
  function curvFunc(x, y) {
    ...
    return (
      tmp1 * tmp1 +
      (2 * (xn * (py1 - py2) + yn * (px2 - px1)) + curvature) * curvature
    );
  }
  ...
  function cHullCheck(x, y) {
    ...
    if (
      s1 != ifMath.segmentSide(px1, py1, cx, cy, middlePtx, middlePty) ||
...
/**
     ...
     * @version 1.0
     */
IFVertexInfo.prototype._hitUnderCurve2 = function (
  px1,
  py1,
  px2,
  py2,
  ...
  if (splitPoints.length == 0) {
    ...
    if (tmp > 0) {
      // case of a cusp when tmp == 0 is not interested, as it was calculated in inflate points
      sqrtC = Math.sqrt(tmp);
      tv1 = (Cac - sqrtC) / (Cab * 2);
      tv2 = (Cac + sqrtC) / (Cab * 2);
  ...
  for (var i = 0; i < nPoints - 1; ++i) {
    ...
    function curvFunc(x, y) {
      ...
      var d = XaSqr * Xa;
      var e =
        Cad * YaSqr - 3 * Ya * (2 * Cab * Yc + Cac * Yb) + 9 * Cab * YbSqr;
      var f =
        (2 * Cad + 3 * Cbc) * Xa * Ya +
        9 * (2 * Cab * Xb * Yb + Xb * Xc * YaSqr - XaSqr * Yb * Yc);
      var g =
        Cad * XaSqr - 3 * (2 * Cab * Xc + Cac * Xb) * Xa + 9 * Cab * XbSqr;
      var h = CadSqr * Ya + 6 * mu1 * Yb + 3 * mu2 * Yc + 9 * CabCac * Yd;
      var i = CadSqr * Xa + 6 * mu1 * Xb + 3 * mu2 * Xc + 9 * CabCac * Xd;
      var j =
        Cad * (CadSqr - 9 * (Cac * Cbd + 2 * Cab * Ccd)) +
        27 * (CabCbd * Cbd + CacSqr * Ccd) -
        81 * CabCbc * Ccd;
    ...
    if (
      ifMath.isEqualEps(ctrlsx[0], ctrlsx[1]) &&
      ifMath.isEqualEps(ctrlsy[0], ctrlsy[1])
    ) {
      tot += this._evalConicReg(
        ctrlsx[0],
        ctrlsy[0],
        ctrlsx[3],
        ctrlsy[3],
...
/**
     ...
     * @version 1.0
     */
IFVertexInfo.prototype.hitTest = function (
  x,
  y,
  source,
  outlineWidth,
  ...
  if (source.rewindVertices(0)) {
    ...
    if (area) {
      if (
        (orientationCW == null && (tot == 2 || tot == -2)) ||
        (tot == 2 && !orientationCW) ||
        (tot == -2 && orientationCW)
      ) {
        if (result) {
    ...
    function measureCurve2(px1, py1, px2, py2, cx1, cy1, cx2, cy2) {
      ...
      function measureAtRoot(a, b, c) {
        ...
        if (a == 0) {
          if (
            b != 0
          ) // otherwise, P(t) is linear by x or y, no interest points between 0 and 1
          {
            t = -c / b;
            measureAtT(t);
          }
...
export var ifVertexInfo = new IFVertexInfo();
