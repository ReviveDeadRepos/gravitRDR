 ...
 */
IFMath.prototype.getIntersectionPoint = function (
  a1x,
  a1y,
  a2x,
  a2y,
 ...
 */
IFMath.prototype.circleLineIntersection = function (
  xL,
  yL,
  dxL,
  dyL,
 ...
 */
IFMath.prototype.circleCircleIntersection = function (
  xC1,
  yC1,
  rC1,
  xC2,
  ...
  if (!this.isEqualEps(dSqr, 0)) {
    ...
    if (
      this.isEqualEps(dSqr, rSumSqr) ||
      (!this.isEqualEps(dSqr, 0) && this.isEqualEps(dSqr, rSqr))
    ) {
...
IFMath.prototype.getCircumcircleCenter = function (x1, y1, x2, y2, x3, y3) {
  ...
  return this.getLinesIntersection(
    dx1,
    dy1,
    (-dx1 * (x1 + x2)) / 2 - (dy1 * (y1 + y2)) / 2,
    dx2,
    dy2,
 ...
 * @version 1.0
 */
IFMath.prototype.sqrSegmentDist = function (
  px1,
  py1,
  px2,
  py2,
  ...
  if (px1 == px2 && py1 == py2) {
    ...
    ptMin &&
    sqrEndAcc &&
    this.vDotProduct(ax, ay, ax, ay) <= sqrEndAcc
  ) {
    ptMin[0] = 0;
    sqrDst = 0.0;
  } else if (
    ptMin &&
    sqrEndAcc &&
    this.ptSqrDist(x, y, px2, py2) <= sqrEndAcc
  ) {
    ptMin[0] = 1;
    sqrDst = 0.0;
  } else {
 ...
 * @return (IFPoint}
 */
IFMath.prototype.getVectorProjection = function (
  x1,
  y1,
  x2,
  y2,
...
IFMath.prototype.solveLinear2Pseudo = function (a11, a12, b1, a21, a22, b2) {
  ...
  if (res == null) {
    // try to find pseudo solution
    var r1, r2;
    if (
      !this.isEqualEps(a11 + a12, 0) &&
      (!this.isEqualEps(b1, 0) || this.isEqualEps(b2, 0))
    ) {
      r1 = b1 / (a11 + a12);
      r2 = r1;
    } else if (
      !this.isEqualEps(a21 + a22, 0) &&
      (!this.isEqualEps(b2, 0) || this.isEqualEps(b1, 0))
    ) {
      r1 = b2 / (a21 + a22);
      r2 = r1;
    ...
    if (
      this.isEqualEps(a11 * r1 + a12 * r2, b1) &&
      this.isEqualEps(a21 * r1 + a22 * r2, b2)
    ) {
 ...
 */
IFMath.prototype.divideQuadraticCurve = function (
  p1,
  c,
  p2,
  t,
 ...
 */
IFMath.prototype.getCtrlPtsCasteljau = function (
  p1,
  c1,
  c2,
  p2,
 ...
 * @version 1.0
 */
IFMath.prototype.getCoeffPolyDeriv = function (
  coeffPoly,
  degreePoly,
  coeffPolyDeriv,
) {
 ...
 * @version 1.0
 */
IFMath.prototype.getCubicRoots = function (
  coeffG,
  p0,
  p1,
  gRoots,
...
/**
     ...
     * @version 1.0
     */
IFMath.prototype.locateByNewton = function (
  a,
  b,
  fa,
  fb,
    ...
    if (locateInitial) {
      ...
      while (a2 - a1 > acc && locateInitial) {
        ...
        if (accNeg <= val && val <= acc) {
          ...
          // sign(val) == sign(fa1) == sign(fa2) != sign(valDeriv2)
          fVals = [fa1, val];
          nSVars = [nsv[0], null];
 ...
 * @version 1.0
 */
IFMath.prototype.getSturmPRS = function (
  coeffPoly,
  degreePoly,
  coeffPolyDeriv,
  sturmPRS,
...
/**
     ...
     * @version 1.0
     */
IFMath.prototype.countRootsNSturm = function (
  coeffF,
  degreeF,
  coeffFDeriv,
  a,
 ...
 * @version 1.0
 */
IFMath.prototype.locRootsSturm = function (
  coeffF,
  degreeF,
  coeffFDeriv,
  a,
 ...
 * @version 1.0
 */
IFMath.prototype.shiftPoly = function (
  coeffPoly,
  degreePoly,
  b,
  coeffPolyShifted,
  ...
  for (i = 1; i <= degreePoly; ++i) {
    ...
    for (j = 0; j < i; ++j) {
      coeffPolyShifted[i] +=
        this.combNK(degreePoly - j, i - j) * coeffPoly[j] * bD[i - j];
    }
  }
};
 ...
 * @version 1.0
 */
IFMath.prototype.shiftPolyOne = function (
  coeffPoly,
  degreePoly,
  coeffPolyShifted,
) {
 ...
 * @version 1.0
 */
IFMath.prototype.inversePolyInterval = function (
  coeffPoly,
  degreePoly,
  a,
  b,
 ...
 * @version 1.0
 */
IFMath.prototype.inversePolyUnaryInterval = function (
  coeffPoly,
  degreePoly,
  coeffPolyInversed,
) {
 ...
 * @version 1.0
 */
IFMath.prototype.estimPositiveRootsDescartes = function (
  coeffPoly,
  degreePoly,
) {
  var nonZeroCoeff = [];
 ...
 * @version 1.0
 */
IFMath.prototype.convertToConstrain = function (
  prevX,
  prevY,
  origX,
  origY,
  ...
  dy = Math.abs(origTrY);
  if (
    !this.isEqualEps(dx, 0) &&
    !this.isEqualEps(dy, 0) &&
    !this.isEqualEps(dx - dy, 0)
  ) {
    if (dx > dy) {
      newX = origTrX;
      tan = dy / dx;
