import { IFObject } from "./object";
import { ifSystem } from "./system";
...
IFLocale.prototype.get = function (key) {
  return key != null && key instanceof IFLocale.Key
...
IFLocale.prototype.getValue = function (clazz, key, default_) {
  ...
  if (
    !result &&
    this._language !=
      IFLocale.Language.Default /* && typeof default_ == 'undefined'*/
  ) {
    result = this._getValue(IFLocale.Language.Default, type_id, key);
  }
  if (!result && typeof default_ == "undefined") {
    throw new Error(
      "No value found for " + IFObject.getName(clazz) + " and key " + key,
    );
  } else if (!result) {
    return default_;
  }
...
export var ifLocale = new IFLocale();
