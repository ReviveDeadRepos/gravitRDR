import { ifSystem, IFSystem } from "./system";
import { ifLocale } from "./locale";
...
IFKey.prototype.toLocalizedShort = function (keyCode) {
  ...
  if (
    ...
    if (keyCode == IFKey.Constant.TAB) {
      ...
      keyCode == IFKey.Constant.OPTION ||
      keyCode == IFKey.Constant.ALT
    ) {
      return "\u2325";
    } else if (
      keyCode == IFKey.Constant.META ||
      keyCode == IFKey.Constant.COMMAND
    ) {
      return "\u2318";
  ...
  keyCode = this.transformKey(keyCode);
  var name = ifLocale.getValue(
    IFKey,
    "key." + keyCode.toString() + ".short",
    null,
  );
  if (!name) {
    return ifLocale.getValue(IFKey, "key." + keyCode.toString());
