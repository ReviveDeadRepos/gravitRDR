      ...
      if (leftType === "number") {
        ...
        if (!objectByValue) {
          ...
          for (var i = 0; i < leftKeys.length; ++i) {
            if (
              (!this.equals(left[leftKeys[i]], right[leftKeys[i]]),
              objectByValue)
            ) {
              return false;
            }
 ...
 * @returns {string}
 */
IFUtil.prototype.formatNumber = function (
  number,
  decimalPlaces,
  decimalSeparator,
) {
  ...
  return ifMath
    .round(number, false, decimalPlaces)
    .toString()
