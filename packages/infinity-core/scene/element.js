...
IFElement.Transform.prototype._transformChildren = function (transform) {
  if (this.hasMixin(IFNode.Container)) {
    for (
      var child = this.getFirstChild(true);
      child != null;
...
IFElement.Style.prototype.renderStyle = function (context, style, styleIndex) {
  if (!context.configuration.isOutline(context)) {
    ...
    for (
      var child = style.getActualStyle().getFirstChild();
      child !== null;
      child = child.getNext()
    ) {
      if (child instanceof IFStyleEntry && child.getProperty("vs") === true) {
        if (child instanceof IFEffectEntry) {
          ...
          if (this.hasMixin(IFVertexSource)) {
            ...
            if (
              cmpOrBlend !== null &&
              cmpOrBlend !== IFPaintCanvas.CompositeOperator.SourceOver &&
              cmpOrBlend !== IFPaintCanvas.BlendMode.Normal
            ) {
              needSeparateCanvas = true;
            }
          }
    ...
    if (needSeparateCanvas) {
      ...
      if (
        context.configuration.paintMode ===
        IFScenePaintConfiguration.PaintMode.Fast
      ) {
 ...
 */
IFElement.prototype.hitTest = function (
  location,
  transform,
  acceptor,
  stacked,
...
IFElement.prototype.getCollisions = function (area, flags, acceptor) {
  ...
  if (
    (flags & IFElement.CollisionFlag.GeometryBBox) != 0 ||
    (flags & IFElement.CollisionFlag.PaintBBox) != 0
  ) {
    // Test ourself, first
...
IFElement.prototype.endUpdate = function (noGeometryInvalidation) {
  if (this._updateCounter != null && --this._updateCounter == 0) {
    ...
    this._notifyChange(
      ...
      noGeometryInvalidation ? false : true,
    );
...
IFElement.prototype.isRenderable = function (context) {
  ...
  if (context) {
    ...
    if (
      ...
      context.configuration.clipArea &&
      !context.configuration.clipArea.intersectsRect(paintBBox)
    ) {
      return false;
    }
  }
...
IFElement.prototype.render = function (context) {
  ...
  if (this.hasMixin(IFElement.Style)) {
    ...
    var styleIndex = 0;
    for (
      var style = styleSet.getFirstChild();
      style !== null;
      style = style.getNext()
    ) {
...
IFElement.prototype.toBitmap = function (width, height, ratio) {
  ...
  paintCanvas.prepare();
  paintCanvas.setOrigin(
    new IFPoint(
      paintArea.getX() * scale - deltaX,
      paintArea.getY() * scale - deltaY,
    ),
  );
 ...
 * a certain hit result depending on the element type
 */
IFElement.prototype._detailHitTest = function (
  location,
  transform,
  tolerance,
  force,
  ...
  if (change == IFElement._Change.InvalidationRequest) {
    ...
    if (this.getParent()) {
      this.getParent()._notifyChange(
        IFElement._Change.InvalidationRequested,
        args,
      );
    }
    ...
    if (this.isVisible()) {
      if (this._canEventBeSend(IFElement.GeometryChangeEvent)) {
        this._scene.trigger(
          new IFElement.GeometryChangeEvent(
            this,
            IFElement.GeometryChangeEvent.Type.Before,
          ),
        );
      }
    }
    ...
    if (this.isVisible()) {
      ...
      if (this._canEventBeSend(IFElement.GeometryChangeEvent)) {
        this._scene.trigger(
          new IFElement.GeometryChangeEvent(
            this,
            IFElement.GeometryChangeEvent.Type.After,
          ),
        );
      }
    ...
    if (this.isVisible()) {
      ...
      if (this._canEventBeSend(IFElement.GeometryChangeEvent)) {
        this._scene.trigger(
          new IFElement.GeometryChangeEvent(
            this,
            IFElement.GeometryChangeEvent.Type.Child,
          ),
        );
      }
 ...
 * @private
 */
IFElement.prototype._handleGeometryChangeForProperties = function (
  change,
  args,
  properties,
) {
 ...
 * @private
 */
IFElement.prototype._handleVisualChangeForProperties = function (
  change,
  args,
  properties,
) {
