...
var IFSelector = (function () {
  ...
  proto.queryAll = function (context, results, options) {
    ...
    if (!context || !(context instanceof IFNode)) {
      ...
      if (typeof context == "string") {
        ...
        (typeof context.length == "number" && context.item)
      ) {
        // simple isArray
        var filtered = [];
    ...
    if (results && results.length) {
      // fills unique ids, does not alter the given results
  ...
  proto.match = function (node, parent) {
    ...
    var found = this.queryAll(parent),
      i = found.length;
    while (i--) {
  ...
  IFSelector.recompile = function () {
    ...
    for (key in combinators) {
      if (key != " ") {
        combList[key.length > 1 ? "unshift" : "push"](
          IFSelector.escapeRegExp(key),
        );
      }
    }
  ...
  var prepareProperty = function (prop) {
    ...
    if (parser) {
      // @todo: Allow functions, not only regex
  ...
  var matchProperty = function (node, prop) {
    ...
    var read = node.hasMixin(IFNode.Properties)
      ? node.getProperty(prop.name)
      : null;
  ...
  proto.compute = function (selector) {
    ...
    if (id) {
      ...
      search = function (context) {
        ...
        return node && (!nodeName || matchNodeName(node, nodeName))
          ? [node]
          : [];
      };
    }
    ...
    if (tags.length > 0) {
      if (!search && tags.length == 1) {
        // optimised for typical .one-tag-only
    ...
    if (nodeName) {
      if (!search) {
        ...
        // search does not filter by name yet
    ...
    for (i = 0; (item = selector.pseudos[i]); i++) {
      if (item.name == "not") {
        // optimised :not(), fast as possible
  ...
  var combinators = (IFSelector.combinators = {
    ...
    ">": function (combined, context, selector, state, locate) {
      ...
      for (var i = 0, node; (node = nodes[i]); i++) {
        if (
          node.getParent() == context &&
          locate(node) &&
          selector.matchAux(node, state)
        )
          combined.push(node);
      }
      return combined;
    ...
    "^": function (combined, context, selector, state, locate) {
      if ((context = context.getFirstChild())) {
        if (locate(context) && selector.match(context, state))
          ...
          combined = IFSelector.combinators["+"](
            combined,
            context,
            selector,
            context,
    ...
    "++": function (combined, context, selector, state, locate) {
      while ((context = context.getNext())) {
        if (locate(context) && this.match(context, state))
          combined.push(context);
      }
      return combined;
    },
  ...
  IFSelector.getUid = window.ActiveXObject
    ? function (node) {
  ...
  IFSelector.parseNth = function (value) {
    ...
    var a = parseInt(parsed[1], 10),
      b = (parseInt(parsed[3], 10) || 0) - 1;
