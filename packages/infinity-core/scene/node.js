 ...
 */
IFNode.Properties.prototype.setProperties = function (
  properties,
  values,
  custom,
) {
 ...
 */
IFNode.Properties.prototype.storeProperties = function (
  blob,
  properties,
  filter,
) {
 ...
 */
IFNode.Properties.prototype.restoreProperties = function (
  blob,
  properties,
  filter,
) {
...
IFNode.Container.prototype.getChildren = function (shadow) {
  var result = [];
  for (
    var child = this.getFirstChild(shadow);
    child !== null;
    child = child.getNext(shadow)
  ) {
    result.push(child);
  }
  return result;
...
IFNode.Container.prototype.getIndexOfChild = function (child, shadow) {
  if (child._parent === this) {
    var index = 0;
    for (
      var node = this.getFirstChild(shadow);
      node !== null;
      node = node.getNext(shadow)
    ) {
      if (node === child) {
        return index;
      }
...
IFNode.prototype.setFlag = function (flag) {
  ...
  if (this._canModifyFlag(flag, true)) {
    if ((this._flags & flag) == 0) {
      this._notifyChange(IFNode._Change.BeforeFlagChange, {
        flag: flag,
        set: true,
      });
      ...
      this._notifyChange(IFNode._Change.AfterFlagChange, {
        flag: flag,
        set: true,
      });
    }
 ...
 * @private
 */
IFNode.prototype._getCompositeEvents = function (
  structural,
  properties,
  flags,
) {
 ...
 * @private
 */
IFNode.prototype._beginBlockCompositeEvents = function (
  structural,
  properties,
  flags,
) {
  this._beginBlockEvents(
    this._getCompositeEvents(structural, properties, flags),
  );
};
 ...
 * @private
 */
IFNode.prototype._endBlockCompositeEvents = function (
  structural,
  properties,
  flags,
) {
...
IFNode.prototype._handleChange = function (change, args) {
  if (change == IFNode._Change.BeforeChildInsert) {
    ...
    if (this._canEventBeSend(IFNode.BeforePropertiesChangeEvent)) {
      this._scene.trigger(
        new IFNode.BeforePropertiesChangeEvent(
          this,
    ...
    if (this._canEventBeSend(IFNode.AfterPropertiesChangeEvent)) {
      this._scene.trigger(
        new IFNode.AfterPropertiesChangeEvent(
          this,
          propertyArgs.properties,
          propertyArgs.values,
        ),
    ...
    if (this._canEventBeSend(IFNode.BeforeFlagChangeEvent)) {
      this._scene.trigger(
        new IFNode.BeforeFlagChangeEvent(this, flagArgs.flag, flagArgs.set),
      );
    }
    ...
    if (this._canEventBeSend(IFNode.AfterFlagChangeEvent)) {
      this._scene.trigger(
        new IFNode.AfterFlagChangeEvent(this, flagArgs.flag, flagArgs.set),
      );
    }
  }
};
