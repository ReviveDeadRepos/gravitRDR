 ...
 * @type {Number}
 */
IFImage.NO_IMAGE_BACKGROUND = new IFColor(
  IFColor.Type.RGB,
  [240, 240, 240, 100],
);
...
IFImage.prototype._paintStyle = function (context, style, styleIndex) {
  if (this._status === IFImage.ImageStatus.Loaded) {
    ...
    context.canvas = sourceCanvas.createCanvas(
      style.getBBox(this.getGeometryBBox()),
    );
...
/** @override */
IFImage.prototype._detailHitTest = function (
  location,
  transform,
  tolerance,
  force,
...
IFImage.prototype._setStatus = function (status) {
  if (status !== this._status) {
    ...
    if (
      this.isAttached() &&
      this._scene.hasEventListeners(IFImage.StatusEvent)
    ) {
