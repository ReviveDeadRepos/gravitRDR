...
IFPolygon.prototype.iterateSegments = function (iterator, includeTransform) {
  ...
  for (var arc = this.$oa; arc < endArc - ACC; arc += stepArc) {
    // Outside
    var point = new IFPoint(
      this.$or * Math.cos(arc) + this.$cx,
      this.$or * Math.sin(arc) + this.$cy,
    );
    ...
    // Inside
    point = new IFPoint(
      this.$ir * Math.cos(arc + deltaArc) + this.$cx,
      this.$ir * Math.sin(arc + deltaArc) + this.$cy,
    );
    ...
    this.iterateSegments(
      function (point, inside, angle) {
        ...
        anchorPoint.setProperties(
          ["tp", "x", "y", "cl", "cr"],
          [
            inside ? this.$ict : this.$oct,
            point.getX(),
            point.getY(),
            inside ? this.$icr : this.$ocr,
            inside ? this.$icr : this.$ocr,
          ],
        );
