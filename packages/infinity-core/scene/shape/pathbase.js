import { IFShape } from "./shape";
import { IFVertexContainer } from "../../vertex/vertexcontainer";
...
import { IFVertex } from "../../vertex/vertex";
...
/** @override */
IFPathBase.AnchorPoint.prototype.validateInsertion = function (
  parent,
  reference,
) {
...
IFPathBase.AnchorPoint.prototype.serialize = function () {
  ...
  if (
    this.$tp !== null &&
    this.$tp !== IFPathBase.AnchorPoint.GeometryProperties.tp
  ) {
    stream.push(this.$tp);
  }
  ...
  if (
    (this.$cl !== null &&
      !ifMath.isEqualEps(
        this.$cl,
        IFPathBase.AnchorPoint.GeometryProperties.cl,
      )) ||
    (this.$cr !== null &&
      !ifMath.isEqualEps(
        this.$cr,
        IFPathBase.AnchorPoint.GeometryProperties.cr,
      ))
  ) {
    stream.push("C");
    stream.push(this.$cl);
...
IFPathBase.AnchorPoint.prototype._getTransformedCopy = function (transform) {
  ...
  var leftH =
    this.$hlx !== null
      ? transform.mapPoint(new IFPoint(this.$hlx, this.$hly))
      : null;
  var rightH =
    this.$hrx !== null
      ? transform.mapPoint(new IFPoint(this.$hrx, this.$hry))
      : null;
 ...
 */
IFPathBase.AnchorPoint.prototype.getLeftShoulderPoint = function (
  fictiveCorner,
) {
 ...
 * @returns {IFPoint} a left shoulder point
 */
IFPathBase.AnchorPoint.prototype.getLeftShoulderPointTransformed = function (
  transform,
  fictiveCorner,
) {
  var shoulderPt = null;
 ...
 */
IFPathBase.AnchorPoint.prototype.getRightShoulderPoint = function (
  fictiveCorner,
) {
 ...
 * @returns {IFPoint} a right shoulder point
 */
IFPathBase.AnchorPoint.prototype.getRightShoulderPointTransformed = function (
  transform,
  fictiveCorner,
) {
  var shoulderPt = null;
...
IFPathBase.AnchorPoint.prototype._handleChange = function (change, args) {
  ...
  if (
    ...
    change == IFNode._Change.AfterPropertiesChange
  ) {
    if (
      ifUtil.containsObjectKey(
        args.properties,
        IFPathBase.AnchorPoint.GeometryProperties,
      )
    ) {
      if (change === IFNode._Change.BeforePropertiesChange) {
        ...
        if (
          path || // For Smooth point recalculate handles properly, even if point is not inserted yet
          (this.$tp == IFPathBase.AnchorPoint.Type.Symmetric &&
            !this.$ah &&
            this.$hlx != null &&
            this.$hrx != null) ||
          ...
          if (this._parent) {
            if (
              ...
              if (
                prevPt &&
                (prevPt.$ah ||
                  prevPt.$tp == IFPathBase.AnchorPoint.Type.Connector)
              ) {
              ...
              if (
                nextPt &&
                (nextPt.$ah ||
                  nextPt.$tp == IFPathBase.AnchorPoint.Type.Connector)
              ) {
...
IFPathBase.AnchorPoint.prototype._calculateConnectorPoint = function () {
  ...
  if (points) {
    ...
    if (nextPt) {
      dirLenNext = Math.sqrt(
        ifMath.ptSqrDist(this.$x, this.$y, nextPt.$x, nextPt.$y),
      );
    }

    if (prevPt) {
      dirLenPrev = Math.sqrt(
        ifMath.ptSqrDist(this.$x, this.$y, prevPt.$x, prevPt.$y),
      );
    }
    ...
    if (this.$ah) {
      if (
        nextPt &&
        prevPt &&
        (nextPt.$tp == IFPathBase.AnchorPoint.Type.Symmetric ||
          nextPt.$tp == IFPathBase.AnchorPoint.Type.Mirror) &&
        !ifMath.isEqualEps(dirLenNext, 0) &&
        !ifMath.isEqualEps(dirLenPrev, 0)
      ) {
      ...
      if (
        prevPt &&
        nextPt &&
        (prevPt.$tp == IFPathBase.AnchorPoint.Type.Symmetric ||
      ...
      if (this.$hlx != null && nextPt && !ifMath.isEqualEps(dirLenNext, 0)) {
        ...
        hLen = Math.sqrt(
          ifMath.ptSqrDist(this.$x, this.$y, this.$hlx, this.$hly),
        );
        hx = this.$x + ((this.$x - nextPt.$x) / dirLenNext) * hLen;
        hy = this.$y + ((this.$y - nextPt.$y) / dirLenNext) * hLen;
      ...
      if (this.$hrx != null && prevPt && !ifMath.isEqualEps(dirLenPrev, 0)) {
        // Use rotation if handle is already set
        hLen = Math.sqrt(
          ifMath.ptSqrDist(this.$x, this.$y, this.$hrx, this.$hry),
        );
        hx = this.$x + ((this.$x - prevPt.$x) / dirLenPrev) * hLen;
        hy = this.$y + ((this.$y - prevPt.$y) / dirLenPrev) * hLen;
        // TODO: use projection in editor when modifying handle
...
IFPathBase.AnchorPoint.prototype._calculateSmoothPoint = function () {
  ...
  if (this.$hlx != null && this.$hrx != null) {
    if (this._leadHr) {
      // the left handle shall be rotated to be in line with the right one
      dirLen = Math.sqrt(
        ifMath.ptSqrDist(this.$x, this.$y, this.$hrx, this.$hry),
      );
      ...
      // the right handle shall be rotated to be in line with the left one
      dirLen = Math.sqrt(
        ifMath.ptSqrDist(this.$x, this.$y, this.$hlx, this.$hly),
      );
      if (!ifMath.isEqualEps(dirLen, 0)) {
...
IFPathBase.AnchorPoint.prototype._calculateMirrorPoint = function () {
  ...
  } else if (!this._leadHr && this.$hlx != null) {
    // the right handle shall be rotated to be in line with the left one
  ...
  if (points) {
    ...
    if (
      ...
      if (!nextPt && !prevPt) {
        ...
        if (ccntr == null) {
          // prev and next points are the same, make handles to be perpendicular
          ...
          dirLen = Math.sqrt(
            ifMath.ptSqrDist(this.$x, this.$y, ccntr.getX(), ccntr.getY()),
          );
      ...
      // type != Smooth && type != Connector as this method should not be called for connector
      if (prevPt && (prevPt.$x != this.$x || prevPt.$y != this.$y)) {
        if (
          ...
          if (
            !prevprevPt ||
            (prevPt.$x == prevprevPt.$x && prevPt.$y == prevprevPt.$y)
          ) {
            hx = this.$x + (prevPt.$x - this.$x) * offs;
            hy = this.$y + (prevPt.$y - this.$y) * offs;
            this.setProperties(["hlx", "hly"], [hx, hy]);
            ...
            if (ccntr == null) {
              // this and prevprev points are the same, make handle to be perpendicular
              dx = (this.$y - prevPt.$y) * offs;
              dy = (prevPt.$x - this.$x) * offs;
          ...
          // prevPt.$tp != IFPathBase.AnchorPoint.Type.Symmetric || Mirror
          this.setProperties(["hlx", "hly"], [null, null]);
        }
      }
      ...
      if (nextPt && (nextPt.$x != this.$x || nextPt.$y != this.$y)) {
        if (
          ...
          if (
            !nextnextPt ||
            (nextPt.$x == nextnextPt.$x && nextPt.$y == nextnextPt.$y)
          ) {
            hx = this.$x + (nextPt.$x - this.$x) * offs;
            hy = this.$y + (nextPt.$y - this.$y) * offs;
            this.setProperties(["hrx", "hry"], [hx, hy]);
            ...
            if (ccntr == null) {
              // this and nextnext points are the same, make handle to be perpendicular
...
/** @override */
IFPathBase.AnchorPoints.prototype.validateInsertion = function (
  parent,
  reference,
) {
...
IFPathBase.AnchorPoints.prototype._invalidateLeft = function (anchorPt) {
  ...
  if (
    anchorPt.$tp == IFPathBase.AnchorPoint.Type.Symmetric ||
    anchorPt.$tp == IFPathBase.AnchorPoint.Type.Mirror
  ) {
 ...
 * @private
 */
IFPathBase.AnchorPoints.prototype._generateVertices = function (
  target,
  transform,
  styled,
) {
  ...
  while (nextPt && (ap != firstPt || !processingStarted)) {
    ...
    if (transform) {
      ...
      this._addMiddleVertices(target, aptmp, prevPttmp, nextPttmp, styled);

      prevPttmp = aptmp;
      aptmp = nextPttmp;
    } else {
      this._addMiddleVertices(target, ap, prevPt, nextPt, styled);
    }
 ...
 * @private
 */
IFPathBase.AnchorPoints.prototype._getShoulderPoint = function (
  pt1x,
  pt1y,
  pt1s,
  pt2x,
 ...
 * @returns {IFPoint} a left shoulder point
 */
IFPathBase.AnchorPoints.prototype._getRightShoulderPoint = function (
  curPt,
  nextPt,
  maxLen,
) {
 ...
 * @returns {IFPoint} a left shoulder point
 */
IFPathBase.AnchorPoints.prototype._getLeftShoulderPoint = function (
  curPt,
  prevPt,
  maxLen,
) {
 ...
 * @private
 */
IFPathBase.AnchorPoints.prototype._addMiddleVertices = function (
  target,
  curPt,
  prevPt,
  nextPt,
 ...
 * @private
 */
IFPathBase.AnchorPoints.prototype._addCornerToVertices = function (
  target,
  pt1x,
  pt1y,
  pt2x,
  ...
  if (
    (pt1x == edgePtx && pt1y == edgePty) ||
    (pt2x == edgePtx && pt2y == edgePty)
  ) {
    target.addVertex(IFVertex.Command.Line, pt2x, pt2y);
    return;
  }
  ...
  if (
    edgePtType == IFPathBase.CornerType.Rounded ||
    edgePtType == IFPathBase.CornerType.InverseRounded
  ) {
    if (
      (pt1x == pt2x && pt1x == edgePtx) ||
      (pt1y == pt2y && pt1y == edgePty)
    ) {
    ...
    // use IFPathBase.AnchorPoint.Type.Asymmetric for all the unsupported types
    target.addVertex(IFVertex.Command.Line, edgePtx, edgePty);
    target.addVertex(IFVertex.Command.Line, pt2x, pt2y);
  }
};
 ...
 * @private
 */
IFPathBase.AnchorPoints.prototype._addPathEndVertices = function (
  target,
  transform,
) {
  var hx, hy;
  ...
  if (
    ...
    (endPt.$hlx == null || endPt.$hly == null) &&
    (prevPt.$hrx == null || prevPt.$hry == null)
  ) {
  ...
  if (path) {
    ...
    if (change == IFNode._Change.BeforeChildInsert) {
      ...
      if (
        ...
        hx =
          anchorPoint.$x +
          (prevPt.$x - anchorPoint.$x) * IFPathBase.AnchorPoint.HANDLE_COEFF;
        hy =
          anchorPoint.$y +
          (prevPt.$y - anchorPoint.$y) * IFPathBase.AnchorPoint.HANDLE_COEFF;
        if (
          !ifMath.isEqualEps(anchorPoint.$x - hx, 0) ||
          !ifMath.isEqualEps(anchorPoint.$y - hy, 0)
        ) {
      ...
      if (
        ...
        hx =
          anchorPoint.$x +
          (nextPt.$x - anchorPoint.$x) * IFPathBase.AnchorPoint.HANDLE_COEFF;
        hy =
          anchorPoint.$y +
          (nextPt.$y - anchorPoint.$y) * IFPathBase.AnchorPoint.HANDLE_COEFF;
        if (
          !ifMath.isEqualEps(anchorPoint.$x - hx, 0) ||
          !ifMath.isEqualEps(anchorPoint.$y - hy, 0)
        ) {
