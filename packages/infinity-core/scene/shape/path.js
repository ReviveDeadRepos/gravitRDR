import { IFPathBase } from "./pathbase";
 ...
 * @constructor
 */
export function IFPath(closed, evenOdd, anchorPoints) {
...
IFPath.prototype.clone = function () {
  ...
  for (var i = 0; i < selectedAnchorPoints.length; ++i) {
    var anchorPointIndex = this.getAnchorPoints().getIndexOfChild(
      selectedAnchorPoints[i],
    );
    var cloneAnchorPoint = clone
      .getAnchorPoints()
      .getChildByIndex(anchorPointIndex);
    if (cloneAnchorPoint) {
...
IFPath.prototype.insertHitPoint = function (hitResult) {
  if (
    !hitResult ||
    !hitResult.slope ||
    ifMath.isEqualEps(hitResult.slope, 0) ||
    ifMath.isEqualEps(hitResult.slope, 1)
  ) {
    return null;
  }
  ...
  if (!aPrev || !aNext) {
    ...
    var zeroC1 =
      c1x == null ||
      c1y == null ||
      (ifMath.isEqualEps(c1x, p1x) && ifMath.isEqualEps(c1y, p1y));
    var zeroC2 =
      c2x == null ||
      c2y == null ||
      (ifMath.isEqualEps(c2x, p2x) && ifMath.isEqualEps(c2y, p2y));

    // If line
    if (zeroC1 && zeroC2) {
      newAPt = new IFPath.AnchorPoint();
      newAPt.setProperties(
        ["x", "y", "tp"],
        [p1x + slope * (p2x - p1x), p1y + slope * (p2y - p1y), tpaNew],
      );
      this.getAnchorPoints().insertChild(newAPt, aNext);
    } else if (zeroC1 || zeroC2) {
      ...
      if (zeroC1) {
        if (
          ifMath.isEqualEps(ctrls1X[1], ctrls1X[2]) &&
          ifMath.isEqualEps(ctrls1Y[1], ctrls1Y[2])
        ) {
        ...
        if (
          ifMath.isEqualEps(c1x, ctrls2X[0]) &&
          ifMath.isEqualEps(c1y, ctrls2Y[0])
        ) {
        ...
        if (
          ifMath.isEqualEps(c2x, ctrls2X[2]) &&
          ifMath.isEqualEps(c2y, ctrls2Y[2])
        ) {
        ...
        // zeroC2
        if (
          ifMath.isEqualEps(ctrls2X[0], ctrls2X[1]) &&
          ifMath.isEqualEps(ctrls2Y[0], ctrls2Y[1])
        ) {
          newAPt.setProperties(["hrx", "hry"], [null, null]);
        } else {
        ...
        if (
          ifMath.isEqualEps(c2x, ctrls1X[2]) &&
          ifMath.isEqualEps(c2y, ctrls1Y[2])
        ) {
          newAPt.setProperties(["hlx", "hly"], [null, null]);
        } else {
        ...
        if (
          ifMath.isEqualEps(c1x, ctrls1X[0]) &&
          ifMath.isEqualEps(c1y, ctrls1Y[0])
        ) {
      ...
      if (
        ifMath.isEqualEps(ctrls1X[1], p1x) &&
        ifMath.isEqualEps(ctrls1Y[1], p1y)
      ) {
        aPrev.setProperties(["hrx", "hry"], [null, null]);
      } else {
      ...
      this.getAnchorPoints().insertChild(newAPt, aNext);
      if (
        ifMath.isEqualEps(ctrls1X[2], ctrls1X[3]) &&
        ifMath.isEqualEps(ctrls1Y[2], ctrls1Y[3])
      ) {
        newAPt.setProperties(["hlx", "hly"], [null, null]);
      } else {
      ...
      if (
        ifMath.isEqualEps(ctrls2X[2], ctrls2X[3]) &&
        ifMath.isEqualEps(ctrls2Y[2], ctrls2Y[3])
      ) {
        aNext.setProperties(["hlx", "hly"], [null, null]);
      } else {
