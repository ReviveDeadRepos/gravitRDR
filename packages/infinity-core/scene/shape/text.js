...
import { IFFont, ifFont } from "../../paint/font";
...
IFText.Block.prototype._handleChange = function (change, args) {
  ...
  if (text) {
    if (
      text._handleGeometryChangeForProperties(
        change,
        args,
        IFText.Block.Properties,
      ) &&
...
IFText.Block.prototype.propertiesToCss = function (css) {
  return this._propertiesToCss(
    css,
    IFText.Block.Properties,
    IFText.Block.propertyToCss,
  );
};
...
IFText.Block.prototype.cssToProperties = function (css) {
  this._cssToProperties(
    css,
    IFText.Block.Properties,
    IFText.Block.cssToProperty,
  );
};

IFText.Block.prototype._propertiesToCss = function (
  css,
  propertyMap,
  propertyConverter,
) {
...
IFText.Block.prototype._cssToProperties = function (
  css,
  propertyMap,
  propertyConverter,
) {
...
IFNode.inheritAndMix("txPara", IFText.Paragraph, IFText.Block, [
  IFNode.Container,
]);
...
IFText.Paragraph.cssToProperty = function (property, css) {
  if (property === "cc") {
    ...
    var str =
      css["column-gap"] || css["-webkit-column-gap"] || css["-moz-column-gap"];
...
IFText.Paragraph.prototype.propertiesToCss = function (css) {
  this._propertiesToCss(
    css,
    IFText.Paragraph.Properties,
    IFText.Paragraph.propertyToCss,
  );
...
IFText.Paragraph.prototype.cssToProperties = function (css) {
  this._cssToProperties(
    css,
    IFText.Paragraph.Properties,
    IFText.Paragraph.cssToProperty,
  );
...
IFText.prototype.getContentBBox = function () {
  return this._size
    ? new IFRect(0, 0, this._size.getX(), this._size.getY())
    : null;
};
  ...
  if (this._runsDirty || this._runs == null) {
    ...
    var container = $("<div></div>")
      ...
      .css({
        ...
        visibility: "hidden",
        width:
          textBox.getWidth() > 1 && !this.$aw ? textBox.getWidth() + "px" : "",
        height:
          textBox.getHeight() > 1 && this.$ah ? textBox.getHeight() + "px" : "",
      })
    ...
    this._size =
      maxWidth !== null && maxHeight !== null
        ? new IFPoint(maxWidth, maxHeight)
        : null;
    ...
    container.find("span:not(:has(span))").each(
      function (index, span) {
        var $span = $(span);
        ...
        var baseline = ifFont.getGlyphBaseline(
          fontFamily,
          fontVariant,
          fontSize,
          char,
...
IFText.prototype.readVertex = function (vertex) {
  ...
  if (!this._runItOutline) {
    ...
    this._runItOutline = ifFont.getGlyphOutline(
      run.family,
      run.variant,
      run.size,
      run.x,
...
/** @override */
IFText.prototype._detailHitTest = function (
  location,
  transform,
  tolerance,
  force,
...
IFText.prototype._asHtml = function (parent, node, segments) {
  if (node instanceof IFText.Break) {
    ...
    if (content && content !== "") {
      if (segments) {
        for (var i = 0; i < content.length; ++i) {
          $("<span></span>").text(content[i]).appendTo(parent);
        }
      } else {
    ...
    parent = $("<span></span>").css(node.propertiesToCss({})).appendTo(parent);
  }
