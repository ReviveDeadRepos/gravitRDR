...
IFObject.inheritAndMix(IFShape, IFItem, [
  IFNode.Container,
  IFElement.Transform,
  IFElement.Style,
  IFVertexSource,
...
IFShape.prototype._paint = function (context, style, styleIndex) {
  ...
  if (context.configuration.isOutline(context)) {
    ...
    var transformedVertices = new IFVertexTransformer(
      style ? style.createVertexSource(this) : this,
      transform,
    );
...
IFShape.prototype._paintStyle = function (context, style, styleIndex) {
  ...
  var paints = null;
  for (
    var entry = style.getActualStyle().getFirstChild();
    entry !== null;
    entry = entry.getNext()
  ) {
  ...
  if (paints) {
    ...
    for (var i = 0; i < paints.length; ++i) {
      ...
      if (paint.isSeparate()) {
        ...
        context.canvas.drawCanvas(
          paintCanvas,
          0,
          0,
          paint.getPaintOpacity(),
          paint.getPaintCmpOrBlend(),
        );
      } else {
...
/** @override */
IFShape.prototype._detailHitTest = function (
  location,
  transform,
  tolerance,
  force,
  ...
  var styleHit = this.getStyleSet().hitTest(
    this,
    location,
    transform,
    tolerance,
  ...
  if (styleHit) {
    ...
    if (
      ifVertexInfo.hitTest(
        location.getX(),
        location.getY(),
        new IFVertexTransformer(this, transform),
        tolerance,
        true,
        vertexHit,
      )
