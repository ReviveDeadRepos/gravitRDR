...
IFStyle.prototype.getBBox = function (source) {
  ...
  for (
    var child = this.getActualStyle().getFirstChild();
    child !== null;
    child = child.getNext()
  ) {
...
IFStyle.prototype.createVertexSource = function (source) {
  for (
    var entry = this.getActualStyle().getFirstChild();
    entry !== null;
    entry = entry.getNext()
  ) {
...
IFStyle.prototype.hitTest = function (source, location, transform, tolerance) {
  ...
  for (
    var entry = this.getActualStyle().getLastChild();
    entry !== null;
 ...
 * @private
 */
IFStyle.prototype._handleGeometryChangeForProperties = function (
  change,
  args,
  properties,
) {
 ...
 * @private
 */
IFStyle.prototype._handleVisualChangeForProperties = function (
  change,
  args,
  properties,
) {
