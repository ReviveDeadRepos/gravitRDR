...
import { IFVertexOffsetter } from "../../../vertex/vertexoffsetter";
 ...
 * @constructor
 */
export function IFOffsetVEffect() {
...
IFOffsetVEffect.prototype.createEffect = function (source) {
  if (this.$r > 0) {
    return new IFVertexOffsetter(
      source,
      this.$r,
      this.$tp === IFOffsetVEffect.OffsetType.Inset ||
        this.$tp === IFOffsetVEffect.OffsetType.Both,
      this.$tp === IFOffsetVEffect.OffsetType.Outset ||
        this.$tp === IFOffsetVEffect.OffsetType.Both,
    );
  }
  return source;
