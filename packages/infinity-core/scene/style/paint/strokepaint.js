import { IFAreaPaint } from "./areapaint";
...
/** @override */
IFStrokePaint.prototype.hitTest = function (
  source,
  location,
  transform,
  tolerance,
  ...
  if (
    ifVertexInfo.hitTest(
      location.getX(),
      location.getY(),
      new IFVertexTransformer(source, transform),
      outlineWidth,
      false,
      vertexHit,
    )
...
IFStrokePaint.prototype.paint = function (canvas, bbox) {
  ...
  if (padding) {
    strokeBBox = strokeBBox.expanded(
      padding[0],
      padding[1],
      padding[2],
      padding[3],
  ...
  if (pattern) {
    ...
    if (patternTransform) {
      ...
      if (this.$sx !== 1.0 || this.$sy !== 1.0) {
        ...
        var oldTransform = canvas.setTransform(
          canvas.getTransform(true).multiplied(patternTransform),
        );
        var patternFillArea = patternTransform.inverted().mapRect(bbox);
        canvas.fillRect(
          patternFillArea.getX(),
          patternFillArea.getY(),
          patternFillArea.getWidth(),
          patternFillArea.getHeight(),
          pattern,
        );
        canvas.setTransform(oldTransform);

        // Now stroke as regular but use our stroke as mask
        canvas.strokeVertices(
          pattern,
          strokeWidth,
          this.$slc,
          this.$slj,
          this.$slm,
          1,
          IFPaintCanvas.CompositeOperator.DestinationIn,
        );
      } else {
        var oldTransform = canvas.setTransform(
          canvas.getTransform(true).multiplied(patternTransform),
        ...
        canvas.strokeVertices(
          pattern,
          strokeWidth / patternTransform.getScaleFactor(),
          this.$slc,
          this.$slj,
          this.$slm,
          opacity,
    ...
    if (this.$sa === IFStrokePaint.Alignment.Inside) {
      ...
      canvas.fillVertices(
        IFColor.BLACK,
        1,
        IFPaintCanvas.CompositeOperator.DestinationOut,
      );
    }
  }
};
