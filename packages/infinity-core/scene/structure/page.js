...
IFNode.inheritAndMix("page", IFPage, IFBlock, [
  IFNode.Container,
  IFNode.Reference,
]);
...
IFPage.prototype.getMasterPage = function () {
  var result =
    this.$msref && this.isAttached()
      ? this.getScene().getReference(this.$msref)
      : null;
...
/** @override */
IFPage.prototype._detailHitTest = function (
  location,
  transform,
  tolerance,
  force,
  ...
  return IFBlock.prototype._detailHitTest.call(
    this,
    location,
    transform,
    tolerance,
...
IFPage.prototype._handleChange = function (change, args) {
      ...
      if (xIndex >= 0 || yIndex >= 0) {
        ...
        if (dx !== 0 || dy !== 0) {
          ...
          for (
            ...
            if (
              child instanceof IFElement &&
              child.hasMixin(IFElement.Transform)
            ) {
  ...
  if (change === IFElement._Change.InvalidationRequested) {
    ...
    if (
      ...
      !area.isEmpty() &&
      this.isMaster() &&
      this.getScene().getProperty("singlePage") === false
    ) {
