...
IFNode.inheritAndMix("layer", IFLayer, IFBlock, [
  IFNode.Container,
  IFElement.Transform,
]);
    ...
    if (this.$tp === IFLayer.Type.Guide) {
      if (!context.configuration.isGuidesVisible(context)) {
        ...
        context.configuration.paintMode !==
          IFScenePaintConfiguration.PaintMode.Outline &&
        !this.$otl
      ) {
...
/** @override */
IFLayer.prototype._detailHitTest = function (
  location,
  transform,
  tolerance,
  force,
...
IFLayer.prototype._handleChange = function (change, args) {
  ...
  if (change == IFNode._Change.AfterPropertiesChange) {
    ...
    if (typeIndex >= 0) {
      ...
      if (
        oldTypeValue === IFLayer.Type.Guide &&
