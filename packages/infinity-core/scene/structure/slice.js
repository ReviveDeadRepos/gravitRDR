  ...
  if (context.configuration.isSlicesVisible(context)) {
    ...
    if (context.configuration.isOutline(context)) {
      ...
      var transformedRect = transform
        ? transform.mapRect(sourceBBox)
        : sourceBBox;
      context.canvas.strokeRect(
        transformedRect.getX(),
        transformedRect.getY(),
        transformedRect.getWidth(),
        transformedRect.getHeight(),
        1,
...
/** @override */
IFSlice.prototype._detailHitTest = function (
  location,
  transform,
  tolerance,
  force,
