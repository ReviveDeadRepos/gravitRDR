...
import { GEventTarget } from "../event/eventtarget";
import { IFPoint } from "../geometry/point";
import { IFCursor } from "../core/cursor";
import { GUIMouseEvent } from "../event/mouseevent";
import { GUIInputEvent } from "../event/inputevent";
import { GEvent } from "../event/event";
import { ifKey, IFKey } from "../core/key";
import { GUIKeyEvent } from "../event/keyevent";
/**
 ...
 * @version 1.0
 */
export function GUIWidget(container) {
 ...
 * @returns {IFPoint}
 */
GUIWidget.convertClientPositionFromMousePosition = function (
  htmlElement,
  mouseEvent,
) {
  var posX = null;
...
GUIWidget.prototype.isEnabled = function () {
  return (
    this._htmlElement.className == null ||
...
/** @override */
GUIWidget.prototype.addEventListener = function (
  eventClass,
  listener,
  target,
  args,
) {
  GEventTarget.prototype.addEventListener.call(
    this,
    eventClass,
    listener,
    target,
...
GUIWidget.prototype._stopListeningInputEvent = function (eventClass) {
  ...
  this._htmlElement.removeEventListener(
    domEventName,
    this._inputEventCache[event_id].domListener,
  );
...
GUIWidget.prototype._dragMouseMove = function (event) {
  if (this._dragStartPosition) {
    ...
    if (!this._dragIsDragging) {
      ...
      if (
        Math.abs(clientStart.getX() - event.client.getX()) >= 1 ||
        Math.abs(clientStart.getY() - event.client.getY()) >= 1
      ) {
        ...
        if (this.hasEventListeners(GUIMouseEvent.DragStart)) {
          /** @type GUIMouseEvent */
          var cachedEvent =
            this._inputEventCache[IFObject.getTypeId(GUIMouseEvent.DragStart)]
              .event;
  ...
  if (this._dragIsDragging && this.hasEventListeners(GUIMouseEvent.Drag)) {
    ...
    var cachedEvent =
      this._inputEventCache[IFObject.getTypeId(GUIMouseEvent.Drag)].event;
    this._dragAssignMouseEvent(event, cachedEvent);
...
GUIWidget.prototype._dragMouseRelease = function (event) {
  if (event.button == GUIMouseEvent.BUTTON_LEFT) {
    if (this._dragIsDragging) {
      ...
      var cachedEvent =
        this._inputEventCache[IFObject.getTypeId(GUIMouseEvent.DragEnd)].event;
      this._dragAssignMouseEvent(event, cachedEvent);
      this._dragAssignDragEvent(cachedEvent, event);
...
GUIWidget.prototype._dragAssignDragEvent = function (dragEvent, currentEvent) {
  ...
  dragEvent.clientDelta = currentEvent.client.subtract(
    this._dragPreviousPosition,
  );
};
 ...
 * @private
 */
GUIWidget.prototype._triggerWidgetEventFromDom = function (
  domEvent,
  widgetEvent,
  ignoreTarget,
) {
  ...
  if (
    widgetEvent instanceof GUIMouseEvent.Down &&
    widgetEvent.button === GUIMouseEvent.BUTTON_LEFT
  ) {
    this._setCapture();
 ...
 * @private
 */
GUIWidget.prototype._updateAndTriggerInputEvent = function (
  domEvent,
  eventClass,
) {
 ...
 * @private
 */
GUIWidget.prototype._updateAndTriggerMouseEvent = function (
  domEvent,
  event_id,
) {
  /** @type GUIMouseEvent */
  var cachedEvent = this._inputEventCache[event_id].event;
  cachedEvent.client = GUIWidget.convertClientPositionFromMousePosition(
    this._htmlElement,
    domEvent,
  );
...
GUIWidget.prototype._setCapture = function () {
  ...
  function addDocumentListener(eventClass) {
    ...
    document.addEventListener(
      domEventName,
      self._savedDocumentListeners[domEventName],
      true,
    );
  }
  ...
  if (this._savedDocumentListeners) {
    for (var domEventName in this._savedDocumentListeners) {
      document.removeEventListener(
        domEventName,
        this._savedDocumentListeners[domEventName],
        true,
      );
    }
