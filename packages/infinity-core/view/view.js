import { IFScene } from "../scene/scene";
import { GUIWidget } from "./widget";
import { IFScenePaintConfiguration } from "../scene/scenepaintconfiguration";
import { IFNode } from "../scene/node";
import { IFObject } from "../core/object";
import { GEvent } from "../event/event";
import { IFStage } from "./stage";
import { IFTransform } from "../geometry/transform";
import { IFRect } from "../geometry/rect";
import { IFPoint } from "../geometry/point";
import { IFSceneStage } from "./scenestage";
import { IFPage } from "../scene/structure/page";
/**
...
IFView.prototype.getViewBox = function (noMargin) {
  ...
  return new IFRect(
    ...
    yOffset,
    this.getWidth() -
      (this._viewOffset[2] + (!noMargin ? this._viewMargin[2] : 0) + xOffset),
    this.getHeight() -
      (this._viewOffset[3] + (!noMargin ? this._viewMargin[3] : 0) + yOffset),
  );
};
...
IFView.prototype.zoomAtCenter = function (center, zoom) {
  ...
  var normalizedZoom = Math.min(
    IFView.options.maxZoomFactor,
    Math.max(zoom, IFView.options.minZoomFactor),
  );
  if (
    normalizedZoom == this._zoom &&
    IFPoint.equals(viewWorldCenter, viewCenter)
  ) {
    return;
  }
...
IFView.prototype.zoomAt = function (pos, zoom) {
  ...
  this.zoomAtCenter(
    new IFPoint(
      pos.getX() + deltaPos.getX() / zoomDelta,
      pos.getY() + deltaPos.getY() / zoomDelta,
    ),
    zoom,
  );
};
...
IFView.prototype.zoomAll = function (rect, reverse) {
  ...
  if (reverse) {
    var viewRect = this._worldToViewTransform.mapRect(
      new IFRect(
        center.getX() - width / 2,
    ...
    var invZoom =
      ...
      Math.min(
        1.0,
        Math.max(
          viewRect.getWidth() / vbox.getWidth(),
          viewRect.getHeight() / vbox.getHeight(),
        ),
      );
    ...
    this.zoomAtCenter(
      center,
      1.0 / Math.max(width / vbox.getWidth(), height / vbox.getHeight()),
    );
  }
};
...
IFView.prototype._updateViewTransforms = function (noEvent) {
  ...
  var worldToViewTransform = new IFTransform()
    .scaled(this._zoom, this._zoom)
    .translated(-this._scrollX, -this._scrollY);
...
IFView.prototype._afterPropertiesChange = function (event) {
  ...
  if (
    ...
    event.properties.indexOf("singlePage") >= 0
  ) {
    this.zoomActivePage();
