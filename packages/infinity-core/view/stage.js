...
import { IFPaintConfiguration } from "../paint/paintconfiguration";
import { IFDirtyList } from "../paint/dirtylist";
import { IFRect } from "../geometry/rect";
import { ifPlatform } from "../platform";
/**
...
export function IFStage(view) {
  ...
  this._paintContext.configuration = view.getViewConfiguration()
    ? view.getViewConfiguration()
    : new IFPaintConfiguration();
...
IFStage.prototype.invalidate = function (area) {
  ...
  if (
    ...
    if (this._repaintRequestFrameId == null) {
      this._repaintRequestFrameId = ifPlatform.scheduleFrame(
        this._repaint.bind(this),
      );
    }
