 ...
 * @constructor
 */
export function IFSceneStage(view) {
...
IFSceneStage.prototype.paint = function (context) {
  ...
  if (
    this._view.getViewConfiguration().pixelMode &&
    !ifMath.isEqualEps(this._view.getZoom(), 1.0)
  ) {
    // Create and size our pixel content canvas
    if (!this._pixelContentCanvas) {
      this._pixelContentCanvas = new IFPaintCanvas();
      this._pixelContentCanvas.resize(
    ...
    this._pixelContentCanvas.setTransform(
      new IFTransform(1, 0, 0, 1, -tl.getX(), -tl.getY()),
    );
    this._pixelContentCanvas.resize(width, height);
    ...
    context.canvas.setOrigin(
      new IFPoint(this._view._scrollX, this._view._scrollY),
    );
