...
IFPaintCanvas.prototype.asJPEGImageBuffer = function (done, quality) {
  quality = quality || 1.0;
  this._canvasContext.canvas.toBlob(
    function (blob) {
      var reader = new FileReader();
      reader.onload = function (event) {
        done(event.target.result);
...
IFPaintCanvas.prototype.getTransform = function (local) {
  ...
  if (!local) {
    ...
    transform = transform.multiplied(
      new IFTransform().scaled(s, s).translated(-tx, -ty),
    );
  }
  return transform;
};
...
IFPaintCanvas.prototype.createCanvas = function (extents, clipDirty) {
  ...
  var sceneExtents = this.getTransform(false)
    .inverted()
    .mapRect(new IFRect(left, top, width, height));
...
IFPaintCanvas.prototype.clear = function () {
  var clearRect = this.getTransform(false)
    .inverted()
    .mapRect(new IFRect(0, 0, this.getWidth(), this.getHeight()));
 ...
 */
IFPaintCanvas.prototype.drawCanvas = function (
  canvas,
  dx,
  dy,
  opacity,
  ...
  var oldTranslation = oldTransform
  ...
  this._canvasContext.globalCompositeOperation = cmpOrBlend
    ? cmpOrBlend
    : IFPaintCanvas.CompositeOperator.SourceOver;

  this._canvasContext.drawImage(
    canvas._canvasContext.canvas,
    0,
    0,
    w,
    h,
 ...
 */
IFPaintCanvas.prototype.createLinearGradient = function (
  x1,
  y1,
  x2,
  y2,
  ...
  for (var i = 0; i < stops.length; ++i) {
    result.addColorStop(
      stops[i].position / 100.0,
      stops[i].color.asCSSString(),
    );
  }
 ...
 * @return {*} a pattern specific to this canvas-type
 */
IFPaintCanvas.prototype.createRadialGradient = function (
  cx,
  cy,
  radius,
  gradient,
) {
  var result = this._canvasContext.createRadialGradient(
    cx,
    cy,
    0,
    cx,
 ...
 */
IFPaintCanvas.prototype.strokeVertices = function (
  stroke,
  width,
  cap,
  join,
  ...
  this._canvasContext.miterLimit =
    typeof miterLimit == "number" ? miterLimit : 10;
...
IFPaintCanvas.prototype.fillCanvas = function (fill) {
  var fillRect = this.getTransform(false)
    .inverted()
    .mapRect(new IFRect(0, 0, this.getWidth(), this.getHeight()));
  this.fillRect(
    fillRect.getX(),
 ...
 */
IFPaintCanvas.prototype.fillRect = function (
  x,
  y,
  width,
  height,
 ...
 */
IFPaintCanvas.prototype.strokeRect = function (
  x,
  y,
  width,
  height,
 ...
 * @version 1.0
 */
IFPaintCanvas.prototype.strokeLine = function (
  x1,
  y1,
  x2,
  y2,
 ...
 * @version 1.0
 */
IFPaintCanvas.prototype.drawImage = function (
  image,
  x,
  y,
  noSmooth,
...
/** @private */
IFPaintCanvas.prototype._setImageSmoothingEnabled = function (
  smoothingEnabled,
) {
  for (var i = 0; i < _imageSmoothingProperties.length; ++i) {
    if (this._canvasContext.hasOwnProperty(_imageSmoothingProperties[i])) {
