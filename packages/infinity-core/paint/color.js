 ...
 * @constructor
 */
export function IFColor(type, value) {
  this._type = type;
...
IFColor.parseCSSColor = function (cssString) {
  ...
  if (str in kCSSColorTable)
    return new IFColor(IFColor.Type.RGB, kCSSColorTable[str].slice()); // dup.
  ...
  if (str[0] === "#") {
    if (str.length === 4) {
      ...
      return new IFColor(IFColor.Type.RGB, [
...
IFColor.prototype.asRGB = function () {
  if (this._type === IFColor.Type.RGB) {
    ...
    var t =
      255 -
      Math.min(255, Math.max(0, Math.round((this._value[0] / 100) * 255)));
