 ...
 * @version 1.0
 */
GEventTarget.prototype.addEventListener = function (
  eventClass,
  listener,
  target,
  args,
  ...
  this._listeners[event_id].listeners.push({
    listener: listener,
 ...
 * @version 1.0
 */
GEventTarget.prototype.removeEventListener = function (
  eventClass,
  listener,
  target,
) {
  ...
  if (this._listeners && event_id in this._listeners) {
    ...
    for (var i = 0; i < array.length; ++i) {
      if (
        array[i].sourceListener == listener &&
        (!target || array[i].target === target)
      ) {
        array.splice(i, 1);
      }
    }
