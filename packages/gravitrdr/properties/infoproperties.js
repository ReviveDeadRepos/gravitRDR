 ...
 * @constructor
 */
export function GInfoProperties() {
  this._elements = [];
}
...
GInfoProperties.prototype._geometryChange = function (event) {
  if (
    event.type === IFElement.GeometryChangeEvent.Type.After ||
...
GInfoProperties.prototype._afterPropertiesChange = function (event) {
  if (
    ...
    event.properties.indexOf("name") >= 0
  ) {
    this._updateProperties();
  }
};
...
GInfoProperties.prototype._updateProperties = function () {
  this._panel
    .find("input[data-apply]")
    .css("display", this._elements.length <= 1 ? "none" : "");

  this._panel
    .find("label[data-type]")
    .text(
      this._elements.length === 1
        ? this._elements[0].getNodeNameTranslated()
        : this._elements.length.toString() + " Elements",
    );
  ...
  this._panel
    ...
    .css(
      "visibility",
      this._elements.length === 1 && this._elements[0] instanceof IFBlock
        ? "visible"
        : "hidden",
    )
    .val(
      this._elements.length === 1 && this._elements[0] instanceof IFBlock
        ? this._elements[0].getLabel()
        : "",
    );
};
...
GInfoProperties.prototype._updateDimensions = function (noBBoxCalculation) {
  var _updateDimension = function (dimension, value) {
    this._panel
      .find('input[data-dimension="' + dimension + '"]')
      ...
      if (this._elements[i].hasMixin(IFElement.Transform)) {
        ...
        if (bbox && !bbox.isEmpty()) {
          this._elementsBBox = this._elementsBBox
            ? this._elementsBBox.united(bbox)
            : bbox;
...
GInfoProperties.prototype._assignDimension = function (dimension, valueString) {
  ...
  if (
    value === null ||
    typeof value !== "number" ||
    ((dimension === "w" || dimension == "h") && value <= 0)
  ) {
    this._updateDimensions();
    return;
  }
  ...
  var _getTransformation = function (bbox, keepRatio) {
    if (dimension === "w" || dimension === "h") {
      ...
      switch (dimension) {
        case "x":
          return new IFTransform().translated(value - bbox.getX(), 0);
        case "y":
          return new IFTransform().translated(0, value - bbox.getY());
        default:
          break;
      }
    }
  ...
  try {
    if (applyToSelection) {
      ...
      for (var i = 0; i < this._elements.length; ++i) {
        var transform = _getTransformation(
          this._elements[i].getGeometryBBox(),
          keepRatio,
        );
        this._elements[i].transform(transform);
      }
