 ...
 * @constructor
 */
export function GPathProperties() {
  this._pathes = [];
}
  ...
  var _createPointInput = function (property) {
    ...
    if (
      ...
      return $("<button></button>")
        ...
        .append($("<span></span>").addClass("fa fa-lock"));
    } else {
      throw new Error("Unknown input property: " + property);
    }
  }.bind(this);
  ...
  panel
      ...
      $("<div></div>")
        ...
        .append(
          $("<button></button>")
            ...
            .append($("<span></span>").addClass("fa fa-forward")),
        )
        .append(
          $("<button></button>")
            // TODO : I18N
            ...
            .append($("<span></span>").addClass("fa fa-backward")),
        )
        .append(
          $("<button></button>")
            // TODO : I18N
    ...
    .append(
      $("<label></label>")
        ...
        .append(_createPointInput("type").css("width", "100px")),
    )
    .append(
      $("<label></label>")
        .attr("data-point-property", "_row")
...
GPathProperties.prototype.update = function (document, elements) {
  if (this._document) {
    ...
    this._document
      .getScene()
      .removeEventListener(
        IFElement.AfterFlagChangeEvent,
  ...
  for (var i = 0; i < elements.length; ++i) {
    if (elements[i] instanceof IFPath) {
      ...
      for (
        var ap = path.getAnchorPoints().getFirstChild();
        ap !== null;
        ap = ap.getNext()
      ) {
...
GPathProperties.prototype._afterFlagChange = function (event) {
  if (
    ...
    event.node instanceof IFPathBase.AnchorPoint
  ) {
...
GPathProperties.prototype._updatePathProperties = function () {
  ...
  this._panel
    .find('input[data-path-property="evenodd"]')
    .prop("checked", path.getProperty("evenodd"));
  this._panel
...
GPathProperties.prototype._updatePointProperties = function () {
  ...
  if (point) {
    ...
    this._panel
      ...
      .val(this._document.getScene().pointToString(point.getProperty("x")));
    this._panel
      .find('input[data-point-property="y"]')
      .val(this._document.getScene().pointToString(point.getProperty("y")));
    this._panel
      .find('input[data-point-property="ah"]')
      .prop("checked", point.getProperty("ah"));
 ...
 * @private
 */
GPathProperties.prototype._assignPathProperties = function (
  properties,
  values,
) {
  ...
  try {
    for (var i = 0; i < this._points.length; ++i) {
      ...
      if (property === "x") {
        ...
        var hlx = point.getProperty("hlx")
          ? point.getProperty("hlx") + dx
          : null;
        var hrx = point.getProperty("hrx")
          ? point.getProperty("hrx") + dx
          : null;
        point.setProperties(["x", "hlx", "hrx"], [value, hlx, hrx]);
      } else if (property === "y") {
        var dy = value - point.getProperty("y");
        var hly = point.getProperty("hly")
          ? point.getProperty("hly") + dy
          : null;
        var hry = point.getProperty("hry")
          ? point.getProperty("hry") + dy
          : null;
 ...
 * @private
 */
GPathProperties.prototype._assignPointProperties = function (
  properties,
  values,
) {
  var editor = this._document.getEditor();
