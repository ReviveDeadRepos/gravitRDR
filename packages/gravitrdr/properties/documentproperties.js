...
import { IFColorSpace } from "@gravitrdr/infinity-core";
 ...
 * @constructor
 */
export function GDocumentProperties() {}
  ...
  var _createInput = function (property) {
    ...
    if (property === "unit") {
      ...
      return $("<div></div>")
        ...
        .append(
          $("<label></label>")
            ...
            .append(
              $("<span></span>").addClass("switch").attr({
                // TODO : I18N
                "data-on": "On",
                "data-off": "Off",
              }),
      ...
      return $("<select></select>")
        .attr("data-property", property)
        .append(
          $("<option></option>").attr("value", IFColorSpace.RGB).text("RGB"),
        )
        .append(
          $("<option></option>").attr("value", IFColorSpace.CMYK).text("CMYK"),
        )
        .on("change", function () {
          self._assignProperty(property, $(this).val());
        });
    ...
    .append(
      $("<tr></tr>")
        ...
        .append(
          $("<td></td>")
            .append(
              _createInput("gridSizeX")
                // TODO : I18N
                .attr("title", "Horizontal Grid-Size"),
            )
            .append(
              _createInput("gridSizeY")
                // TODO : I18N
      ...
      $("<tr></tr>")
        .append(
          $("<td></td>")
            ...
            // TODO : I18N
            .html(
              '<span class="fa fa-arrows"></span> / <span class="fa fa-magnet"></span>',
            ),
        )
        .append(
          $("<td></td>")
            .attr("colspan", "3")
            .append(
              _createInput("pickDist")
                // TODO : I18N
                .attr("title", "Pick Distance in Pixels"),
            )
            .append(
              _createInput("snapDist")
                // TODO : I18N
...
GDocumentProperties.prototype._updateProperties = function () {
  ...
  this._panel
    .find('select[data-property="unit"]')
    .val(scene.getProperty("unit"));
  this._panel
    .find('select[data-property="unitSnap"]')
    .val(scene.getProperty("unitSnap"));
  this._panel
    .find('input[data-property="gridSizeX"]')
    .val(scene.pointToString(scene.getProperty("gridSizeX")));
  this._panel
    .find('input[data-property="gridSizeY"]')
    .val(scene.pointToString(scene.getProperty("gridSizeY")));
  this._panel
    .find('input[data-property="gridActive"]')
    .prop("checked", scene.getProperty("gridActive"));
  this._panel
    .find('input[data-property="crDistSmall"]')
    .val(scene.pointToString(scene.getProperty("crDistSmall")));
  this._panel
    .find('input[data-property="crDistBig"]')
    .val(scene.pointToString(scene.getProperty("crDistBig")));
  this._panel
    .find('input[data-property="crConstraint"]')
    .val(
      ifUtil.formatNumber(
        ifMath.toDegrees(scene.getProperty("crConstraint")),
        2,
      ),
    );
  this._panel
    .find('input[data-property="snapDist"]')
    .val(scene.pointToString(scene.getProperty("snapDist")));
  this._panel
    .find('input[data-property="pickDist"]')
    .val(scene.pointToString(scene.getProperty("pickDist")));
  this._panel
    .find('select[data-property="clspace"]')
    .val(scene.getProperty("clspace"));
  this._panel
    .find('input[data-property="pathImage"]')
    .val(scene.getProperty("pathImage"));
  this._panel
    .find('input[data-property="pathFont"]')
    .val(scene.getProperty("pathFont"));
  this._panel
    .find('input[data-property="pathExport"]')
    .val(scene.getProperty("pathExport"));
};
 ...
 * @private
 */
GDocumentProperties.prototype._assignProperties = function (
  properties,
  values,
) {
