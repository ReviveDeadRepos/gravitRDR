import { GProperties } from "@gravitrdr/application";
...
import { IFTextEditor } from "@gravitrdr/infinity-editor";
import { ifFont, IFFont } from "@gravitrdr/infinity-core";
...
import { IFElementEditor } from "@gravitrdr/infinity-editor";
 ...
 * @constructor
 */
export function GTextProperties() {
  this._text = [];
}
  ...
  var _createInput = function (property) {
    ...
    if (property.indexOf("va-") === 0) {
      ...
      return $("<button></button>")
        ...
        .append($("<span></span>").addClass("fa " + icon));
      ...
      return $("<button></button>")
        ...
        .on("click", function () {
          self._assignProperty(
            "al",
            $(this).hasClass("g-active") ? null : alignment,
          );
        })
        .append($("<span></span>").addClass("fa fa-align-" + iconName));
      ...
      return $("<input>")
        ...
        .on("change", function () {
          var value = $(this).val();
          value =
            !value || value === "" ? null : IFLength.parseEquationValue(value);
  ...
  panel
    ...
    .append(
      $("<div></div>")
        ...
        .append(
          _createInput("al-" + IFText.Paragraph.Alignment.Left)
            // TODO : I18N
            .attr("title", "Align Left"),
        )
        .append(
          _createInput("al-" + IFText.Paragraph.Alignment.Center)
            // TODO : I18N
            .attr("title", "Align Centered"),
        )
        .append(
          _createInput("al-" + IFText.Paragraph.Alignment.Right)
            // TODO : I18N
            .attr("title", "Align Right"),
        )
        .append(
          _createInput("al-" + IFText.Paragraph.Alignment.Justify)
            // TODO : I18N
            .attr("title", "Justify"),
        ),
    )
    ...
    .append(
      $("<div></div>")
        ...
        .append(
          _createInput("va-" + IFText.VerticalAlign.Top)
            // TODO : I18N
            .attr("title", "Align Top"),
        )
        .append(
          _createInput("va-" + IFText.VerticalAlign.Middle)
            // TODO : I18N
            .attr("title", "Align Middle"),
        )
        .append(
          _createInput("va-" + IFText.VerticalAlign.Bottom)
            // TODO : I18N
...
GTextProperties.prototype.update = function (document, elements) {
  if (this._document) {
    ...
    this._document
      .getEditor()
      .removeEventListener(IFEditor.InlineEditorEvent, this._inlineEditorEvent);
    this._document = null;
  }
...
GTextProperties.prototype._afterPropertiesChange = function (event) {
  ...
  if (
    this._text.length > 0 &&
    (this._text[0] === event.node || this._text[0].getContent() === event.node)
  ) {
    this._updateProperties();
  }
};
...
GTextProperties.prototype._updateProperties = function () {
  ...
  var propertySource = this._textEditor
    ? this._textEditor
    : IFElementEditor.getEditor(this._text[0]);

  // Text
  this._panel.find('button[data-property^="va"]').each(
    function (index, element) {
      var $element = $(element);
      var disabled =
        this._textEditor && this._textEditor.isInlineEdit() ? true : false;
      $element
  ...
  this._panel
    .find('select[data-property="ff"]')
    .val(propertySource.getProperty("ff"));
  this._panel
    .find('input[data-property="fi"]')
    .val(
  ...
  for (var i = 100; i <= 900; i += 100) {
    weightSelect
      .find('[value="' + i + '"]')
      .prop("disabled", !fontWeights || fontWeights.indexOf(i) < 0);
  }
  ...
  // Paragraph
  this._panel
    .find('input[data-property="cc"]')
    .val(propertySource.getProperty("cc"));
  this._panel
    .find('input[data-property="cg"]')
    .val(
      this._document.getScene().pointToString(propertySource.getProperty("cg")),
    );

  var alignVal = propertySource.getProperty("al") || "";
  this._panel
    .find('button[data-property^="al"]')
    .each(function (index, element) {
      var $element = $(element);
      $element.toggleClass(
        "g-active",
        $element.attr("data-property") === "al-" + alignVal,
      );
    });

  this._panel
    .find('select[data-property="wm"]')
    .val(propertySource.getProperty("wm"));

  this._panel
    .find('input[data-property="in"]')
    .val(
      this._document.getScene().pointToString(propertySource.getProperty("in")),
    );

  var lh = propertySource.getProperty("lh");
  this._panel
    .find('input[data-property="lh"]')
    .val(lh !== null ? ifUtil.formatNumber(lh) : "");
};
