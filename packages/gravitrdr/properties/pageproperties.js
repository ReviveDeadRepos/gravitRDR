 ...
 * @constructor
 */
export function GPageProperties() {
  this._pages = [];
}
  ...
  var _createInput = function (property) {
    ...
    if (property === "msref") {
      ...
      var result = $("<select></select>")
        ...
        .on("change", function () {
          ...
          if (val.indexOf(",") >= 0) {
            var val2 = val.split(",");
            var sz =
              GPageProperties.SIZE_PRESETS[parseInt(val2[0])].sizes[
                parseInt(val2[1])
              ];
      ...
      for (var i = 0; i < GPageProperties.SIZE_PRESETS.length; ++i) {
        ...
        var groupEl = $("<optgroup></optgroup>").attr("label", group.name);
      ...
      return $("<input>")
        ...
        .on("change", function () {
          var value = self._document.getScene().stringToPoint($(this).val());
          if (value !== null && typeof value === "number" && value >= 0) {
            var lockMargins = self._panel
              .find("button[data-lock-margin]")
              .hasClass("g-active");
            if (lockMargins) {
...
GPageProperties.prototype.update = function (document, elements) {
  if (this._document) {
    ...
    this._document
      .getScene()
      .removeEventListener(IFNode.AfterInsertEvent, this._updatePages, this);
    this._document
      .getScene()
      .removeEventListener(IFNode.AfterRemoveEvent, this._updatePages, this);
    this._document = null;
  }
  ...
  if (this._pages.length === elements.length) {
    ...
    this._document
      .getScene()
      .addEventListener(IFNode.AfterInsertEvent, this._updatePages, this);
    this._document
      .getScene()
      .addEventListener(IFNode.AfterRemoveEvent, this._updatePages, this);
...
GPageProperties.prototype._afterPropertiesChange = function (event) {
  ...
  if (
    ...
    (this._pages[0] === event.node ||
      (event.node instanceof IFScene && event.properties.indexOf("unit") >= 0))
  ) {
    this._updateProperties();
  }
};
...
GPageProperties.prototype._updateProperties = function () {
  ...
  this._panel
    .find('select[data-property="msref"]')
    .val(page.getProperty("msref"));
  ...
  this._panel
    ...
    .val(scene.pointToString(page.getProperty("w")));
  this._panel
    .find('input[data-property="h"]')
    .val(scene.pointToString(page.getProperty("h")));
  this._panel
    .find('input[data-property="mt"]')
    .val(scene.pointToString(page.getProperty("mt")));
  this._panel
    .find('input[data-property="mb"]')
    .val(scene.pointToString(page.getProperty("mb")));
  this._panel
    .find('input[data-property="ml"]')
    .val(scene.pointToString(page.getProperty("ml")));
  this._panel
    .find('input[data-property="mr"]')
    .val(scene.pointToString(page.getProperty("mr")));
...
GPageProperties.prototype._updatePages = function () {
  ...
  $("<option></option>").attr("value", "").text("").appendTo(select);
