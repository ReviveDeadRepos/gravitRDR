 ...
 * @constructor
 */
export function GPolygonProperties() {
  this._polygons = [];
}
  ...
  panel
    ...
    .append(
      $("<label></label>")
        ...
        .html(
          '<span class="fa fa-stop" style="font-size:11px;transform:rotate(45deg)"></span>',
        )
        .append(
          _createInput("or").css({
            "margin-left": "5px",
    ...
    .append(
      $("<label></label>")
        ...
        .html(
          '<span class="fa fa-rotate-right" style="visibility: hidden"></span>',
        )
        .append(
          _createInput("ia").css({
            "margin-left": "3px",
    ...
    .append(
      $("<label></label>")
        ...
        .append(
          _createInput("ocr").css({
            "margin-left": "5px",
            width: "38px",
          }),
        )
        .append(_createInput("oct").css("width", "32px")),
    )
    .append(
      $("<label></label>")
        .css({
        ...
        .append(
          _createInput("icr").css({
            "margin-left": "3px",
            width: "38px",
          }),
        )
        .append(_createInput("ict").css("width", "32px")),
    );
};
...
GPolygonProperties.prototype._updateProperties = function () {
  ...
  this._panel
    .find('input[data-property="pts"]')
    .val(polygon.getProperty("pts"));
  this._panel
    .find('input[data-property="evenodd"]')
    .prop("checked", polygon.getProperty("evenodd"));
  this._panel
    .find('input[data-property="or"]')
    .val(this._document.getScene().pointToString(polygon.getProperty("or")));
  this._panel
    .find('input[data-property="ir"]')
    .val(this._document.getScene().pointToString(polygon.getProperty("ir")));
  this._panel
    .find('input[data-property="oa"]')
    .val(
      ifUtil.formatNumber(
        ifMath.toDegrees(ifMath.PI2 - polygon.getProperty("oa")),
        2,
      ),
    );
  this._panel
    ...
    .val(
      ifUtil.formatNumber(
        ifMath.toDegrees(ifMath.PI2 - polygon.getProperty("ia")),
        2,
      ),
    );
  this._panel
    .find('select[data-property="oct"]')
    .val(polygon.getProperty("oct"));
  this._panel
    .find('select[data-property="ict"]')
    .val(polygon.getProperty("ict"));
  this._panel
    .find('input[data-property="ocr"]')
    .val(this._document.getScene().pointToString(polygon.getProperty("ocr")));
  this._panel
    .find('input[data-property="icr"]')
    .val(this._document.getScene().pointToString(polygon.getProperty("icr")));
};
