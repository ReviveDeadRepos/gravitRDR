import { GPanel } from "@gravitrdr/application";
 ...
 * @constructor
 */
export function GPropertiesPanel() {
  GPanel.call(this);
...
GPropertiesPanel.prototype._afterFlagChange = function (evt) {
  ...
  if (
    evt.flag === IFNode.Flag.Active &&
    this._elements &&
    this._elements.length === 1 &&
    this._elements[0] instanceof IFScene
  ) {
    this._updatePropertyPanels();
  }
};
  ...
  for (var i = 0; i < this._propertyPanels.length; ++i) {
    ...
    var available =
      !this._elements || this._elements.length === 0
        ? false
        : propertyPanel.properties.update(this._document, this._elements);
