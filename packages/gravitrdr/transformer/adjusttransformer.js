 ...
 * @constructor
 */
export function GAdjustTransformer() {
  this._elements = [];
  this._savedValues = {
    Move: [0, 0],
    Scale: [100, 100],
...
GAdjustTransformer.prototype.init = function (panel) {
  ...
  panel
      ...
      $("<div></div>")
        ...
        .append(
          $("<button></button>")
            ...
            .attr("title", "Move")
            .attr("data-mode", "Move")
            .append($("<span></span>").addClass("fa fa-arrows")),
        )
        .append(
          $("<button></button>")
            .addClass("g-flat")
            ...
            .append($("<span></span>").addClass("fa fa-expand")),
        )
        .append(
          $("<button></button>")
            .addClass("g-flat")
            ...
            .append($("<span></span>").addClass("fa fa-rotate-right")),
        )
        .append(
          $("<button></button>")
            .addClass("g-flat")
            ...
            .attr("title", "Skew")
            .attr("data-mode", "Skew")
            .append($("<span></span>").addClass("fa fa-eraser")),
        )
        .append(
          $("<button></button>")
            .addClass("g-flat")
            ...
            .append($("<span></span>").addClass("fa fa-star-half-o")),
        )
        .append(
          $("<button></button>")
            // TODO : I18N
    ...
    .append(
      $("<label></label>")
        ...
        .append($("<span></span>").attr("data-prefix", ""))
        .append(
          $("<input>")
            .css({
              "margin-left": "3px",
        ...
        .append($("<span></span>").attr("data-postfix", "")),
    )
    .append(
      $("<label></label>")
        .attr("data-value", "2")
  ...
  panel.find("button[data-mode]").each(
...
GAdjustTransformer.prototype._setTransformMode = function (mode) {
  ...
  this._panel.find("button[data-mode]").each(
    function (index, element) {
      var $element = $(element);
  ...
  var _updateLabel = function (labelInfo, number) {
    var label = this._panel.find(
      'label[data-value="' + number.toString() + '"]',
    );
    if (!labelInfo) {
      ...
      label.css("display", "");
      label
        .find("span[data-prefix]")
        .text(labelInfo.prefix ? labelInfo.prefix + ":" : "");
      label
        .find("span[data-postfix]")
        .text(labelInfo.postfix ? labelInfo.postfix : "");
  ...
  this._panel
    .find('[data-property="pivot"]')
    .css("display", transModeInfo.pivot ? "" : "none");
};
...
GAdjustTransformer.prototype._apply = function () {
  ...
  var transformFunc = GAdjustTransformer._TransformMode[
    this._transformMode
  ].transformFunc(value1, value2, scene);
    ...
    IFEditor.tryRunTransaction(
      ...
      function () {
        ...
        for (var i = 0; i < this._elements.length; ++i) {
          ...
          if (pivot) {
            bbox = bbox
              ? bbox.united(element.getGeometryBBox())
              : element.getGeometryBBox();
          }
