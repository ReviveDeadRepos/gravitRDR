 ...
 * @constructor
 */
export function GLayerTypeAction(layerType) {
...
GLayerTypeAction.prototype.getTitle = function () {
  return ifLocale
    .get(GLayerTypeAction.TITLE)
    .replace("%name%", ifLocale.get(IFLayer.TypeName[this._layerType]));
};
...
GLayerTypeAction.prototype.isChecked = function () {
  var activeLayer = gApp.getActiveDocument()
    ? gApp.getActiveDocument().getScene().getActiveLayer()
    : null;
  if (activeLayer) {
