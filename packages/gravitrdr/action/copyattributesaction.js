 ...
 * @constructor
 */
export function GCopyAttributesAction() {}
...
GCopyAttributesAction.prototype.execute = function () {
  ...
  for (var i = 0; i < selection.length; ++i) {
    if (selection[i].hasMixin(IFElement.Attributes)) {
      ...
      if (
        attributes &&
        attributes.hasMixin(IFNode.Container) &&
        attributes.getFirstChild()
      ) {
        var serializedAttributes = IFNode.serialize(attributes);
        window.window.gShell.setClipboardContent(
          IFAttribute.MIME_TYPE,
