...
GPasteInsideAction.prototype.execute = function () {
  var nodes = IFNode.deserialize(
    window.window.gShell.getClipboardContent(IFNode.MIME_TYPE),
  );
    ...
    if (elements.length > 0) {
      ...
      try {
        for (var i = 0; i < selection.length; ++i) {
          ...
          var groupCntr = groupBBox
            ? groupBBox.getSide(IFRect.Side.CENTER)
            : new IFPoint(0, 0);
          var targBBox =
            target instanceof IFElement ? target.getGeometryBBox() : null;
          var targCntr = targBBox
            ? targBBox.getSide(IFRect.Side.CENTER)
            : new IFPoint(0, 0);
          for (var k = 0; k < insertGroup.length; ++k) {
            insertGroup[k].transform(
              new IFTransform(
                1,
                0,
                0,
                1,
                -groupCntr.getX() + targCntr.getX(),
                -groupCntr.getY() + targCntr.getY(),
              ),
            );
