...
GInvertSelectionAction.prototype.execute = function () {
  ...
  source.accept(function (node) {
    if (
      node instanceof IFItem &&
      node.getParent() instanceof IFLayer &&
      !node.hasFlag(IFNode.Flag.Selected)
    ) {
      selection.push(node);
    }
  });
