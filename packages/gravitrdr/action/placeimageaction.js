 ...
 * @constructor
 */
export function GPlaceImageAction() {}
  ...
  document
    ...
    .openResourcePrompt(
      ...
      function (url) {
        ...
        var name =
          suffix.length > 0
            ? filename.substr(0, filename.length - suffix.length - 1)
            : filename;

        var imagePos = document
          .getScene()
          .getActivePage()
          .getGeometryBBox()
        ...
        image.setProperties(
          ...
          [
            name,
            url,
            new IFTransform(1, 0, 0, 1, imagePos.getX(), imagePos.getY()),
          ],
        );
        image.transform();
