import { ifPlatform } from "@gravitrdr/infinity-core";
import { GSidebar } from "@gravitrdr/application";
...
import { IFItem } from "@gravitrdr/infinity-core";
...
import { IFText } from "@gravitrdr/infinity-core";
import { IFImage } from "@gravitrdr/infinity-core";
import { IFEllipse } from "@gravitrdr/infinity-core";
import { IFRectangle } from "@gravitrdr/infinity-core";
import { IFPath } from "@gravitrdr/infinity-core";
import { IFPolygon } from "@gravitrdr/infinity-core";
...
function canDropPage(target) {
  if (dragPage) {
    ...
    if (
      targetPage &&
      (targetPage !== dragPage || ifPlatform.modifiers.shiftKey)
    ) {
 ...
 * @constructor
 */
export function GPagesLayersSidebar() {
  GSidebar.call(this);
}
...
GPagesLayersSidebar.prototype.init = function (htmlElement) {
  ...
  // -- Pages --
  this._pagesPanel = $("<div></div>").addClass("pages");

  this._pageAddControl = $("<button></button>")
    // TODO : I18N
    .attr("title", "Add Page")
    .on(
      "click",
  ...
  this._pageDeleteControl = $("<button></button>")
    // TODO : I18N
  ...
  this._layerAddControl = $("<button></button>")
    // TODO : I18N
  ...
  this._layerDeleteControl = $("<button></button>")
    // TODO : I18N
  ...
  $("<div></div>")
    .gPanel({
      ...
      controls: [this._layerAddControl, this._layerDeleteControl],
    })
    .appendTo(htmlElement);
};
...
GPagesLayersSidebar.prototype._documentEvent = function (event) {
  if (event.type === GApplication.DocumentEvent.Type.Activated) {
    ...
    scene.removeEventListener(
      IFScene.ReferenceEvent,
      this._referenceEvent,
      this,
    );
    this._clear();
  }
  ...
  var block = $("<div></div>")
    ...
    .append(
      $("<div></div>")
        ...
        .append($("<span></span>").addClass("fa fa-fw")),
    )
    .append(
      $("<div></div>")
        .addClass("page-lock")
          ...
          $(element).append(
            $("<div></div>")
              ...
              .on("drop", function (evt) {
                ...
                if (
                  dragPage &&
                  dragPage.getParent() === targetPage.getParent()
                ) {
                  ...
                  IFEditor.tryRunTransaction(
                    ...
                    function () {
                      if (ifPlatform.modifiers.shiftKey) {
                        // Clone page
                        var insertPos = dragPage
                          .getScene()
                          .getPageInsertPosition();
                        var pageClone = dragPage.clone();
                        pageClone.setProperties(
                          ["x", "y", "name"],
                          [
                            insertPos.getX(),
                            insertPos.getY(),
                            pageClone.getProperty("name") + "-copy",
                          ],
                        );
                        parent.insertChild(
                          pageClone,
...
GPagesLayersSidebar.prototype._clearLayers = function () {
  ...
  if (this._document) {
    ...
    if (activePage) {
      for (
        var child = activePage.getFirstChild();
        child !== null;
        child = child.getNext()
      ) {
...
GPagesLayersSidebar.prototype._insertLayer = function (layerOrItem) {
  ...
  var nextNode = layerOrItem.getNext()
    ? this._getLayerTreeNode(layerOrItem.getNext())
    : null;
  if (nextNode) {
    this._layersTree.tree(
      "addNodeBefore",
      { id: treeId, layerOrItem: layerOrItem },
      nextNode,
    );
  } else {
    var parent = layerOrItem.getParent();
    var parentTreeNode =
      !parent || parent instanceof IFPage
        ? null
        : this._getLayerTreeNode(parent);
    this._layersTree.tree(
      "appendNode",
      { id: treeId, layerOrItem: layerOrItem },
      parentTreeNode,
  ...
  if (layerOrItem.hasMixin(IFNode.Container)) {
    for (
      var child = layerOrItem.getFirstChild();
      child !== null;
      child = child.getNext()
    ) {
  ...
  if (
    layerOrItem instanceof IFItem &&
    layerOrItem.hasFlag(IFNode.Flag.Selected)
  ) {
...
GPagesLayersSidebar.prototype._updateLayer = function (layerOrItem) {
  ...
  if (treeNode) {
    ...
    // Mark draft layers
    if (
      layerOrItem instanceof IFLayer &&
      layerOrItem.getProperty("tp") === IFLayer.Type.Draft
    ) {
      label = "*" + label;
    }
  ...
  if (event.node instanceof IFPage) {
    ...
    if (parent instanceof IFLayer || parent instanceof IFItem) {
      ...
      for (
        ...
        if (
          (child instanceof IFLayer || child instanceof IFItem) &&
          child !== event.node
        ) {
          hasChildren = true;
          break;
...
GPagesLayersSidebar.prototype._afterFlagChange = function (event) {
  if (
    event.node instanceof IFPage &&
    (event.flag === IFNode.Flag.Active || event.flag === IFNode.Flag.Selected)
  ) {
    this._pagesPanel.find(".page-block").each(function (index, element) {
      var $element = $(element);
      if ($element.data("page") === event.node) {
        $element.toggleClass(
          event.flag === IFNode.Flag.Active ? "g-active" : "g-selected",
          event.set,
        );
      }
    });
    ...
    event.node instanceof IFLayer &&
    (event.flag === IFNode.Flag.Active ||
      event.flag === IFNode.Flag.Selected ||
      event.flag === IFNode.Flag.Expanded)
  ) {
    this._updateLayer(event.node);
  } else if (
    event.node instanceof IFItem &&
    event.flag === IFNode.Flag.Selected
  ) {
    this._updateLayer(event.node);
  } else if (
    (event.node instanceof IFLayer || event.node instanceof IFItem) &&
    (event.flag === IFElement.Flag.Hidden ||
      event.flag === IFElement.Flag.Locked)
  ) {
    this._updateLayer(event.node);
  }
};
...
GPagesLayersSidebar.prototype._clickLayerTreeNode = function (evt) {
  if (evt.node.layerOrItem) {
    if (evt.node.layerOrItem instanceof IFLayer) {
      ...
      if (
        ifPlatform.modifiers.shiftKey ||
        !evt.node.layerOrItem.hasFlag(IFNode.Flag.Selected)
      ) {
        // Add element to selection
        this._document
          .getEditor()
          .updateSelection(ifPlatform.modifiers.shiftKey, [
            evt.node.layerOrItem,
          ]);
...
GPagesLayersSidebar.prototype._createLayerTreeItem = function (node, li) {
  if (node.layerOrItem) {
    ...
    var isOutlined =
      parentOutlined ||
      (layerOrItem instanceof IFLayer && layerOrItem.getProperty("otl"));
    ...
    for (var i = 0; i < itemLevel; ++i) {
      $("<span></span>").addClass("layer-spacer").prependTo(container);
    }

    // Figure an icon for the item if any
    var icon = null;
    if (layerOrItem instanceof IFLayer) {
      icon = layerOrItem.hasFlag(IFNode.Flag.Expanded)
        ? "folder-open"
        : "folder";
      ...
      .on("click", function (evt) {
        ...
        if (!parentLocked) {
          ...
          IFEditor.tryRunTransaction(
            layerOrItem,
            function () {
              layerOrItem.setProperty(
                "locked",
                !layerOrItem.getProperty("locked"),
              );
            },
            "Toggle Layer Locked",
          );
    ...
    if (layerOrItem instanceof IFLayer) {
      ...
      if (layerOrItem.getProperty("tp") !== IFLayer.Type.Guide) {
        $("<span></span>")
          .addClass(
            "layer-outline fa fa-" + (isOutlined ? "circle-o" : "circle"),
          )
...
/** @private */
GPagesLayersSidebar.prototype._canMoveLayerTreeNode = function (
  moved_node,
  target_node,
  position,
) {
  return (
    this._getLayerTreeNodeMoveInfo(
      position,
      moved_node.layerOrItem,
      target_node.layerOrItem,
    ) !== null
  );
};
...
GPagesLayersSidebar.prototype._moveLayerTreeNode = function (event) {
  ...
  var moveInfo = this._getLayerTreeNodeMoveInfo(
    event.move_info.position,
    event.move_info.moved_node.layerOrItem,
    event.move_info.target_node.layerOrItem,
  );
 ...
 * @private
 */
GPagesLayersSidebar.prototype._getLayerTreeNodeMoveInfo = function (
  position,
  source,
  target,
) {
