 ...
 * @constructor
 */
export function GStylesSwatchesSidebar() {
  GSidebar.call(this);
}
...
GStylesSwatchesSidebar.TITLE = new IFLocale.Key(
...
GStylesSwatchesSidebar.prototype.init = function (htmlElement) {
  ...
  $("<div></div>")
    .gPanel({
      ...
      controls: [this._swatchDeleteControl],
    })
    .appendTo(htmlElement);
};
...
GStylesSwatchesSidebar.prototype._documentEvent = function (event) {
  if (event.type === GApplication.DocumentEvent.Type.Activated) {
    ...
    this._stylePanel.gStylePanel(
      "value",
      scene.getStyleCollection().querySingle("sharedStyle:selected"),
    );
    this._swatchPanel.gSwatchPanel("attach", scene.getSwatchCollection());
    this._swatchPanel.gSwatchPanel(
      "value",
      scene.getSwatchCollection().querySingle("swatch:selected"),
    );
    this._updateStyleControls();
    this._updateSwatchControls();
