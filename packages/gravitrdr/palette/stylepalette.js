import { GPalette } from "@gravitrdr/application";
import { IFObject } from "@gravitrdr/infinity-core";
import { IFLocale, ifLocale } from "@gravitrdr/infinity-core";
import { GDocument } from "@gravitrdr/application";
import { IFElement } from "@gravitrdr/infinity-core";
import { IFStyle } from "@gravitrdr/infinity-core";
import { IFLinkedStyle } from "@gravitrdr/infinity-core";
import { IFInlineStyle } from "@gravitrdr/infinity-core";
import { IFFilterEntry } from "@gravitrdr/infinity-core";
import { IFEffectEntry } from "@gravitrdr/infinity-core";
import { IFVEffectEntry } from "@gravitrdr/infinity-core";
import { IFLength } from "@gravitrdr/infinity-core";
import { IFAppliedStyle } from "@gravitrdr/infinity-core";
import { IFStrokePaint } from "@gravitrdr/infinity-core";
import { IFFillPaint } from "@gravitrdr/infinity-core";
import { IFPaintEntry } from "@gravitrdr/infinity-core";
import { GMenu } from "@gravitrdr/application";
...
import { IFStyleEntry } from "@gravitrdr/infinity-core";
import { IFSharedStyle } from "@gravitrdr/infinity-core";
import { ifUtil } from "@gravitrdr/infinity-core";
...
GStylePalette.prototype.init = function (htmlElement, controls) {
    ...
    .on(
      ...
      function (evt) {
        $("<div></div>")
          ...
          .gStylePanel({
            nullStyle: $("<span></span>").addClass("fa fa-plus-circle").css({
              "font-size": "24px",
              "line-height": "30px",
            }),
            // TODO : I18N
            ...
            function (evt, style) {
              ...
              try {
                for (var i = 0; i < this._styleElements.length; ++i) {
                  ...
                  this._styleElements[i]
                    .getStyleSet()
                    .appendChild(appliedStyle);
  ...
  this._styleDeleteControl = $("<button></button>")
    ...
    .append($("<span></span>").addClass("fa fa-trash-o"))
    .appendTo(controls);
  ...
  for (var i = 0; i < gravitrdr.styleEntries.length; ++i) {
    ...
    if (
      IFFilterEntry.prototype.isPrototypeOf(
        styleEntry.getEntryClass().prototype,
      )
    ) {
      filterEntries.push({
        entry: styleEntry,
      ...
      IFEffectEntry.prototype.isPrototypeOf(
        styleEntry.getEntryClass().prototype,
      )
    ) {
      effectEntries.push({
      ...
      IFVEffectEntry.prototype.isPrototypeOf(
        styleEntry.getEntryClass().prototype,
      )
    ) {
      effectEntries.push({
  ...
  this._styleSelector = $("<div></div>")
    ...
    .on(
      "stylemove",
      function (evt, sourceStyle, targetStyle) {
        ...
        try {
          for (var i = 0; i < sourceStyles.length; ++i) {
            ...
            parent.insertChild(
              sourceStyles[i],
              sourceStyleIndex < targetStyleIndex
                ? targetStyles[i].getNext()
                : targetStyles[i],
            );
          }
        } finally {
        ...
        .append(
          $("<span></span>")
            ...
            .on(
              ...
              function () {
                var makeVisible =
                  this._styles[this._selectedStyleIndex][0].getProperty(
                    "vs",
                  ) === false;
                this._modifyEachSelectedStyle(function (style) {
  ...
  this._paintsPanel = $("<div></div>")
    ...
    .append($("<div></div>").addClass("style-entries-panel-table"));
    ...
    .gPanel({
      ...
      controls: [
        $("<button></button>")
          ...
          .append($("<span></span>").addClass("fa fa-pencil")),
        $("<button></button>")
          // TODO : I18N
          .attr("title", "Add Fill")
          .on(
          ...
          .append($("<span></span>").addClass("fa fa-stop")),
        $("<button></button>")
          // TODO : I18N
  ...
  for (var i = 0; i < filterEntries.length; ++i) {
    // TODO : I18N
    _addStyleEntryMenu(
      filterEntries[i].entry.getEntryClass(),
      filterEntries[i].name,
      filtersAddMenu,
    );
  }
...
GStylePalette.prototype._documentEvent = function (event) {
  if (event.type === GApplication.DocumentEvent.Type.Activated) {
    ...
    editor.addEventListener(
      IFEditor.SelectionChangedEvent,
...
GStylePalette.prototype._setSelectedStyle = function (selectedIndex) {
  this._styleSelector.gStylePanel(
    "value",
    selectedIndex >= 0 ? this._styles[selectedIndex][0] : null,
  );
  this._selectedStyleIndex = selectedIndex;
  this._updateSelectedStyle();
};

GStylePalette.prototype._modifyEachSelectedStyle = function (
  modifier,
  styleIndex,
) {
...
GStylePalette.prototype._visitEachSelectedStyle = function (
  visitor,
  styleIndex,
) {
  if (typeof styleIndex === "number" || this._selectedStyleIndex >= 0) {
    var style =
      this._styles[
        this._selectedStyleIndex >= 0 ? this._selectedStyleIndex : styleIndex
      ].slice();
...
GStylePalette.prototype._insertStyles = function (style) {
  ...
  if (this._styleElements.length === 1) {
    ...
    for (
      var node = styleSet.getFirstChild();
      node !== null;
      node = node.getNext()
    ) {
      ...
      for (
        ...
        if (node instanceof IFLinkedStyle) {
          ...
          for (var j = 0; j < linkedStyles.length; ++j) {
            if (
              linkedStyles[j][0].getProperty("ref") === node.getProperty("ref")
            ) {
...
GStylePalette.prototype._isSelectedStyle = function (style) {
  return (
    this._selectedStyleIndex >= 0 &&
    this._getStyleIndex(style, true) === this._selectedStyleIndex
  );
};
  ...
  for (var i = 0; i < this._styles.length; ++i) {
    for (var j = 0; j < this._styles[i].length; ++j) {
      if (style instanceof IFSharedStyle && handleSharedStyles) {
        if (
          this._styles[i][j] instanceof IFLinkedStyle &&
          this._styles[i][j].getProperty("ref") === style.getReferenceId()
        ) {
          return i;
        }
      } else {
  ...
  if (activeStyle instanceof IFLinkedStyle) {
    ...
    try {
      ...
      this._visitEachSelectedStyle(function (style) {
        ...
        // Transfer style
        inlineStyle.transferProperties(style, [
          IFAppliedStyle.GeometryProperties,
          IFAppliedStyle.VisualProperties,
        ]);
        for (
          var child = sharedStyle.getFirstChild();
          child !== null;
          child = child.getNext()
        ) {
    ...
    // TODO : I18N
    var styleDefaultName =
      "Style-" +
      (scene.getStyleCollection().queryCount("> sharedStyle") + 1).toString();
    ...
    if (name !== null) {
      ...
      try {
        ...
        // Transfer style
        for (
          var child = activeStyle.getFirstChild();
          child !== null;
          child = child.getNext()
        ) {
          if (child instanceof IFStyleEntry) {
        ...
        this._visitEachSelectedStyle(function (style) {
          ...
          linkedStyle.transferProperties(style, [
            IFAppliedStyle.GeometryProperties,
            IFAppliedStyle.VisualProperties,
          ]);
...
GStylePalette.prototype._updateStyleSettings = function () {
  ...
  this._styleAddControl.prop(
    "disabled",
    this._styleElements === null || this._styleElements.length === 0,
  );
  ...
  if (this._selectedStyleIndex >= 0) {
    ...
    this._styleSettings
      .find(".visibility > span")
      .toggleClass("fa-eye", style.getProperty("vs") == true)
      .toggleClass("fa-eye-slash", style.getProperty("vs") == false);

    this._styleSettings
      .find('[data-property="__link"]')
      // TODO : I18N
      .attr(
        "title",
        style instanceof IFLinkedStyle ? "Unlink style" : "Link style",
      )
      /* !! */
      .find("> span")
    ...
    this._styleSettings
      .find('[data-property="blm"]')
      .val(style.getProperty("blm"));
    this._styleSettings
      .find('[data-property="opc"]')
      .val(ifUtil.formatNumber(style.getProperty("opc") * 100));

    this._styleSettings
      .find('[data-property="tp"]')
      .val(style.getProperty("tp"));
  }
};
...
GStylePalette.prototype._insertEntryRow = function (entry) {
  ...
  if (handler) {
    ...
    var contents = handler.createContent(
    ...
    var row = $("<div></div>")
      ...
      .on("dragstart", function (evt) {
        ...
        // Add drag overlays
        $this
          .closest(".style-entries-panel-table")
          .find("> div")
      ...
      .append($("<div></div>").addClass("contents").append(contents))
      .appendTo(table);
  ...
  if (handler) {
    ...
    if (row) {
      row
        ...
        .attr(
          "class",
          "fa fa-eye" + (!entry.getProperty("vs") ? "-slash" : ""),
        );
      handler.updateProperties(
        row.find(".contents > :first-child"),
        row.data("entry"),
        this._document.getScene(),
      );
    }
...
GStylePalette.prototype._getPanelFromEntry = function (entry) {
  if (entry instanceof IFPaintEntry) {
    ...
    entry instanceof IFEffectEntry ||
    entry instanceof IFVEffectEntry
  ) {
