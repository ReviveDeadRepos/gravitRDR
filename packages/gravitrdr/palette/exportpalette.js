...
import { GExporter } from "@gravitrdr/application";
 ...
 * @constructor
 */
export function GExportPalette() {
  GPalette.call(this);
  this._extensions = [];
}
...
GExportPalette.prototype._afterPropertiesChange = function (evt) {
  if (
    evt.node === this._element &&
    (evt.properties.indexOf("export") >= 0 ||
      evt.properties.indexOf("name") >= 0)
  ) {
    this._updateExports();
  }
};
...
GExportPalette.prototype._updateFromSelection = function () {
  ...
  var selection = this._document
    ? this._document.getEditor().getSelection()
    : null;
...
GExportPalette.prototype._updateExports = function () {
  ...
  if (this._element) {
    ...
    $("<div></div>").addClass("title").text(title).appendTo(this._htmlElement);
      ...
      $("<div></div>")
        ...
        .append(
          $("<div></div>")
            ...
            .append(
              extSelect.val(exportRow.ex).on("change", function (evt) {
          ...
          $("<div></div>")
            ...
            .append(
              $("<button></button>")
                  ...
                  function () {
                    ...
                    if (storage) {
                      storage.saveResourcePrompt(
                        ...
                        this._element.getLabel() + exportRow.sf,
                        [exportRow.ex],
                        function (url) {
                          this._getExporterByExt(exportRow.ex).exportPart(
                            this._element,
                            exportRow.sz,
                            storage,
                            url,
                ...
                .append(
                  $("<span></span>").addClass("fa fa-external-link-square"),
                ),
            )
            .append(
              $("<button></button>")
                ...
                .on(
                  "click",
                  function () {
                    var exports = this._element
                      .getProperty("export", true)
                      .slice();
          ...
          $("<button></button>")
            .on(
              ...
              function () {
                ...
                if (storage) {
                  storage.openDirectoryPrompt(
                    this._document.getUrl(),
                    function (url) {
                      var exports = this._element.getProperty("export", true);
                      for (var i = 0; i < exports.length; ++i) {
                        var exportRow = exports[i];
                        var resourceUrl = new URI(url)
                          .filename(
            ...
            .append($("<span></span>").text("Export All...")),
        )
        .appendTo(this._htmlElement);
    }
  }
