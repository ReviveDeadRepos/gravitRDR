import { GPatternPaintEntry } from "./patternpaintentry";
...
import { IFGradient } from "@gravitrdr/infinity-core";
 ...
 * @constructor
 */
export function GAreaPaintEntry() {
...
GAreaPaintEntry.prototype.createContent = function (scene, assign, revert) {
  var content = GPatternPaintEntry.prototype.createContent.call(
    this,
    scene,
    assign,
    revert,
...
GAreaPaintEntry.prototype.updateProperties = function (content, entry, scene) {
  GPatternPaintEntry.prototype.updateProperties.call(
    this,
    content,
    entry,
    scene,
  ...
  if (patternType !== IFPattern.Type.Color) {
    ...
    transformContent
      .find('[data-property="tx"]')
      .val(ifUtil.formatNumber(entry.getProperty("tx") * 100));
    transformContent
      .find('[data-property="ty"]')
      .val(ifUtil.formatNumber(entry.getProperty("ty") * 100));
    transformContent
      .find('[data-property="sx"]')
      .val(ifUtil.formatNumber(entry.getProperty("sx") * 100));

    transformContent
      .find('[data-property="sy"]')
      .val(ifUtil.formatNumber(entry.getProperty("sy") * 100))
      .closest("div") // !! -->
      .css(
        "display",
        pattern.getType() === IFGradient.Type.Linear ? "none" : "",
      );

    transformContent
      .find('[data-property="rt"]')
      .val(ifUtil.formatNumber(ifMath.toDegrees(entry.getProperty("rt")), 2));
  } else {
...
/** @override */
GAreaPaintEntry.prototype._getPropertiesToAssign = function (
  content,
  entry,
  scene,
  properties,
  ...
  GPatternPaintEntry.prototype._getPropertiesToAssign.call(
    this,
    content,
    entry,
    scene,
  ...
  properties.push("tx", "ty", "sx", "sy", "rt");

  var tx = IFLength.parseEquationValue(
    content.find('[data-property="tx"]').val(),
  );
  var ty = IFLength.parseEquationValue(
    content.find('[data-property="ty"]').val(),
  );
  var sx = IFLength.parseEquationValue(
    content.find('[data-property="sx"]').val(),
  );
  var sy = IFLength.parseEquationValue(
    content.find('[data-property="sy"]').val(),
  );
  var rt = IFLength.parseEquationValue(
    content.find('[data-property="rt"]').val(),
  );
  ...
  if (patternType === IFPattern.Type.Color) {
    ...
    if (
      oldPatternType !== IFPattern.Type.Gradient ||
    ...
    if (oldPatternType === IFPattern.Type.Color) {
      ...
      pattern = new IFGradient(
        content.find(".g-gradient-editor").gGradientEditor("value"),
        patternSubType,
      );
    }
  }

  values.push(tx, ty, sx, sy, rt);
};
