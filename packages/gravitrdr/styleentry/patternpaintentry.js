import { GStyleEntry } from "@gravitrdr/application";
 ...
 * @constructor
 */
export function GPatternPaintEntry() {}
                  ...
                  $("<optgroup></optgroup>")
                    ...
                    .append(
                      $("<option></option>")
                        .attr(
                          "value",
                          IFPattern.Type.Gradient +
                            "#" +
                            IFGradient.Type.Linear,
                        )
                        // TODO : I18N
                        .text("Linear"),
                    )
          ...
          $("<div></div>")
            ...
            .append(
              $("<input>")
                ...
                .css("width", "3em")
                .on("change", function (evt) {
                  var opacity = IFLength.parseEquationValue(
                    $(evt.target).val(),
                  );
                  if (opacity !== null) {
                    assign();
...
/** @override */
GPatternPaintEntry.prototype.updateProperties = function (
  content,
  entry,
  scene,
) {
  ...
  content
    ...
    .val(patternSubType ? patternType + "#" + patternSubType : patternType);
  content
    .find('[data-element="color"]')
    .gColorButton(
      "value",
      patternType === IFPattern.Type.Color ? pattern : IFColor.BLACK,
    )
  ...
  content
    ...
    .val(ifUtil.formatNumber(entry.getProperty("opc") * 100));

  // Gradient
  if (patternType === IFPattern.Type.Gradient) {
    content.find('[data-element="gradient"]').css("display", "");
    content
      .find(".g-gradient-editor")
      .gGradientEditor("value", pattern.getStops());
  } else {
    content.find('[data-element="gradient"]').css("display", "none");
    content
      .find(".g-gradient-editor")
      .gGradientEditor("value", DEFAULT_GRADIENT.getStops());
  }
};

/** @override */
GPatternPaintEntry.prototype.assignProperties = function (
  content,
  entry,
  scene,
) {
...
GPatternPaintEntry.prototype._getPropertiesToAssign = function (
  content,
  entry,
  scene,
  properties,
  ...
  properties.push("pat", "blm", "opc");

  var opacity = IFLength.parseEquationValue(
    content.find('[data-property="opc"]').val(),
  );
