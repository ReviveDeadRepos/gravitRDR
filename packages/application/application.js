...
export     function GApplication() {
    ...
    GApplication.prototype._createMainMenu = function () {
        ...
        var _createMenuItem = function (item, parentMenu) {
            if (item.type === 'menu') {
                ...
                item.item = window.gShell.addMenuItem(parentMenu, ifLocale.get(item.action.getTitle()), item.action.isCheckable(), item.action.getShortcut(),
                    function () {
        ...
        var _createMenu = function (tree, parentMenu) {
            var menu = window.gShell.addMenu(parentMenu, tree.caption, function () {
                for (var i = 0; i < tree.items.length; ++i) {
                    ...
                    if (item.type === 'item') {
                        window.gShell.updateMenuItem(item.item, ifLocale.get(item.action.getTitle()),
    ...
    GApplication.prototype._addWindowMenuItem = function (window) {
        this._windowMenuMap.push({
            window: window,
            item: window.gShell.addMenuItem(this._windowMenu, window.getTitle(), true, null, function () {
    ...
    GApplication.prototype._updateWindowMenuItem = function (window) {
        for (var i = 0; i < this._windowMenuMap.length; ++i) {
            var map = this._windowMenuMap[i];
            if (map.window === window) {
                window.gShell.updateMenuItem(map.item, map.window.getTitle(), true, map.window === this._windows.getActiveWindow());
                break;
            }
        }
        this._updateTitle();
