...
import { GEvent } from "@gravitrdr/infinity-core";
import { GDocument } from "./document";
import { GHeader } from "./workspace/header";
import { GToolbar } from "./workspace/toolbar";
import { GPanels } from "./workspace/panels";
import { GSidebars } from "./workspace/sidebars";
import { GWindows } from "./workspace/windows";
import { GPalettes } from "./workspace/palettes";
import { GAction } from "./extension/action";
import { GWindow } from "./workspace/window";
import { GStorage } from "./extension/storage";
import { IFScene } from "@gravitrdr/infinity-core";
import { IFLength } from "@gravitrdr/infinity-core";
import { ifSystem, IFSystem } from "@gravitrdr/infinity-core";
...
GApplication.CATEGORY_MODIFY = new IFLocale.Key(
  GApplication,
  "category.modify",
);
GApplication.CATEGORY_MODIFY_ARRANGE = new IFLocale.Key(
  GApplication,
  "category.modify.arrange",
);
GApplication.CATEGORY_MODIFY_ALIGN = new IFLocale.Key(
  GApplication,
  "category.modify.align",
);
GApplication.CATEGORY_MODIFY_TRANSFORM = new IFLocale.Key(
  GApplication,
  "category.modify.transform",
);
GApplication.CATEGORY_MODIFY_PAGE = new IFLocale.Key(
  GApplication,
  "category.modify.page",
);
GApplication.CATEGORY_MODIFY_LAYER = new IFLocale.Key(
  GApplication,
  "category.modify.layer",
);
GApplication.CATEGORY_VIEW = new IFLocale.Key(GApplication, "category.view");
GApplication.CATEGORY_VIEW_MAGNIFICATION = new IFLocale.Key(
  GApplication,
  "category.view.magnification",
);
GApplication.CATEGORY_WINDOW = new IFLocale.Key(
  GApplication,
  "category.window",
);
...
GApplication.TOOL_CATEGORY_SELECT = new IFLocale.Key(
  GApplication,
  "tool-category.select",
);
GApplication.TOOL_CATEGORY_IMAGE = new IFLocale.Key(
  GApplication,
  "tool-category.image",
);
GApplication.TOOL_CATEGORY_VECTOR = new IFLocale.Key(
  GApplication,
  "tool-category.vector",
);
GApplication.TOOL_CATEGORY_OTHER = new IFLocale.Key(
  GApplication,
  "tool-category.other",
);
GApplication.TOOL_CATEGORY_COLOR = new IFLocale.Key(
  GApplication,
  "tool-category.color",
);
GApplication.TOOL_CATEGORY_VIEW = new IFLocale.Key(
  GApplication,
  "tool-category.view",
);
 ...
 */
GApplication.prototype.getMatchingStorage = function (
  prompt,
  save,
  extension,
  directory,
...
GApplication.prototype.addDocument = function (scene, temporaryTitle) {
  ...
  var document = new GDocument(
    ...
    temporaryTitle
      ? temporaryTitle
      : "Untitled-" + (++this._documentUntitledCount).toString(),
  );
    ...
    storage.saveResourcePrompt(
      ...
      function (url) {
        ...
        if (this.hasEventListeners(GApplication.DocumentEvent)) {
          this.trigger(
            new GApplication.DocumentEvent(
              GApplication.DocumentEvent.Type.UrlUpdated,
              this,
            ),
          );
        }
 ...
 */
GApplication.prototype.activateDocument = function (
  document,
  noWindowActivation,
) {
  if (document != this._activeDocument) {
    ...
    if (this._activeDocument) {
      if (this._activeDocument) {
        ...
        if (this.hasEventListeners(GApplication.DocumentEvent)) {
          this.trigger(
            new GApplication.DocumentEvent(
              GApplication.DocumentEvent.Type.Deactivated,
    ...
    if (document) {
      ...
      if (this.hasEventListeners(GApplication.DocumentEvent)) {
        this.trigger(
          new GApplication.DocumentEvent(
            GApplication.DocumentEvent.Type.Activated,
            document,
          ),
        );
      }
...
GApplication.prototype.closeDocument = function (document) {
  if (document._windows.length) {
    ...
    if (this.hasEventListeners(GApplication.DocumentEvent)) {
      this.trigger(
        new GApplication.DocumentEvent(
          GApplication.DocumentEvent.Type.Removed,
          document,
        ),
      );
    }
...
GApplication.prototype.canExecuteAction = function (id, args) {
  ...
  if (actionInstance) {
    return (
      actionInstance.isAvailable() &&
      actionInstance.isEnabled.apply(actionInstance, args)
    );
  }
...
GApplication.prototype.relayout = function () {
  ...
  setTimeout(
    function () {
      ...
      topOffset += this.isPartVisible(GApplication.Part.Header)
        ? headerPart.outerHeight()
        : 0;
      ...
      leftOffset += this.isPartVisible(GApplication.Part.Toolbar)
        ? toolbarPart.outerWidth()
        : 0;
      ...
      leftOffset += this.isPartVisible(GApplication.Part.Sidebars)
        ? sidebarsPart.outerWidth()
        : 0;
      ...
      rightOffset += this.isPartVisible(GApplication.Part.Palettes)
        ? palettesPart.outerWidth()
        : 0;
      ...
      panelsPart.css(
        "width",
        (this._view.width() - leftOffset - rightOffset).toString() + "px",
      );
      bottomOffset += this.isPartVisible(GApplication.Part.Panels)
        ? panelsPart.outerHeight()
        : 0;
...
GApplication.prototype.registerShortcut = function (shortcut, action) {
  Mousetrap.bind(
    this._shortcutToMouseTrapShortcut(shortcut),
    function () {
      action();
      return false;
    }.bind(this),
...
GApplication.prototype._addDocument = function (document) {
  ...
  if (this.hasEventListeners(GApplication.DocumentEvent)) {
    this.trigger(
      new GApplication.DocumentEvent(
        GApplication.DocumentEvent.Type.Added,
        document,
      ),
    );
  }
...
GApplication.prototype._createMainMenu = function () {
  ...
  for (var i = 0; i < this._actions.length; ++i) {
      ...
      for (var k = 0; k < categories.length; ++k) {
        ...
        if (!item) {
          item = {
            ...
            items: [],
            windowMenu:
              GApplication.CATEGORY_WINDOW === action.getCategory() &&
    ...
    _addItemGroupAndDivider(
      currentTree,
      actionItem,
      groups ? groups[groups.length - 1] : null,
    );
  ...
  var _createMenuItem = function (item, parentMenu) {
    if (item.type === "menu") {
      ...
      item.separator = window.gShell.addMenuSeparator(parentMenu);
    } else if (item.type === "item") {
      item.item = window.gShell.addMenuItem(
        parentMenu,
        ifLocale.get(item.action.getTitle()),
        item.action.isCheckable(),
        item.action.getShortcut(),
        function () {
  ...
  var _createMenu = function (tree, parentMenu) {
    var menu = window.gShell.addMenu(parentMenu, tree.caption, function () {
      for (var i = 0; i < tree.items.length; ++i) {
        ...
        if (item.type === "item") {
          window.gShell.updateMenuItem(
            ...
            item.action.isCheckable() ? item.action.isChecked() : false,
          );
        }
      }
    });
  ...
  for (var i = 0; i < treeRoot.items.length; ++i) {
    ...
    if (item.windowMenu) {
      ...
      window.gShell.addMenuSeparator(this._windowMenu);
    }
  }
};
  ...
  for (var i = 0; i < this._windowMenuMap.length; ++i) {
    ...
    if (map.window === window) {
      window.gShell.updateMenuItem(
        ...
        map.window.getTitle(),
        true,
        map.window === this._windows.getActiveWindow(),
      );
      break;
    }
  }
