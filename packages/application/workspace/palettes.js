...
import { GView } from "../extension/view";
 ...
 * @constructor
 */
export function GPalettes(htmlElement) {
...
GPalettes.prototype.setPaletteActive = function (paletteId, active) {
  ...
  if (
    ...
    if (!active) {
      ...
      groupInfo.container
        .find(".palette-group-tabs > button")
        .each(function () {
          var el = $(this);
...
GPalettes.prototype._detachPaletteFromGroup = function (paletteId) {
  ...
  if (group && paletteInfo) {
    // Remove Tab & Panel
    group.container
      .find('.palette-group-tabs > button[data-palette-id="' + paletteId + '"]')
      .remove();

    // Important: only detach, not remove our panel as it will be re-used!!
    group.container
      .find('.palette-group-panels > div[data-palette-id="' + paletteId + '"]')
      .detach();
  ...
  groupInfo.container = $("<div></div>")
    ...
    .append(
      $("<div></div>")
        ...
        .append(
          $("<div></div>")
            ...
            .append(
              $("<button></button>")
                .append(
                  $("<span></span>").addClass("fa fa-angle-double-down fa-fw"),
                )
                .on(
                  "click",
                  function () {
        ...
        .append($("<div></div>").addClass("palette-group-tabs"))
        .append($("<div></div>").addClass("palette-group-controls")),
    )
    .append($("<div></div>").addClass("palette-group-panels"))
...
GPalettes.prototype._setGroupExpanded = function (groupInfo, expanded) {
  if (expanded !== groupInfo.expanded) {
    ...
    var buttonSpan = groupInfo.container.find(
      ".palette-group-collapse > button > span",
    );
