 ...
 * @constructor
 */
export function GWindows(htmlElement) {
...
GWindows.prototype.activateWindow = function (window) {
  if (window != this._activeWindow) {
    ...
    if (window === null) {
      ...
      (window &&
        this._activeWindow &&
        window.getDocument() !== this._activeWindow.getDocument()) ||
      !this._activeWindow
    ) {
    ...
    if (this._activeWindow && this.hasEventListeners(GWindows.WindowEvent)) {
      this.trigger(
        new GWindows.WindowEvent(
          GWindows.WindowEvent.Type.Deactivated,
    ...
    if (window) {
      ...
      if (this.hasEventListeners(GWindows.WindowEvent)) {
        this.trigger(
          new GWindows.WindowEvent(GWindows.WindowEvent.Type.Activated, window),
        );
      }
    }
  }
...
GWindows.prototype.addWindow = function (source) {
  ...
  if (source instanceof GWindow) {
    ...
    window
      .getView()
      .transform(
        sourceView.getScrollX(),
        sourceView.getScrollY(),
        sourceView.getZoom(),
      );
  } else {
...
GWindows.prototype.closeWindow = function (window) {
  ...
  if (this.hasEventListeners(GWindows.WindowEvent)) {
    this.trigger(
      new GWindows.WindowEvent(GWindows.WindowEvent.Type.Removed, window),
    );
  }
...
GWindows.prototype._addWindow = function (document) {
  ...
  if (this.hasEventListeners(GWindows.WindowEvent)) {
    this.trigger(
      new GWindows.WindowEvent(GWindows.WindowEvent.Type.Added, window),
    );
  }
