 ...
 * @constructor
 */
export function GToolbar(htmlElement) {
  this._htmlElement = htmlElement;

  htmlElement
    .append($("<div></div>").addClass("section sidebars"))
    .append($("<div></div>").addClass("section toolpanel"));
}
...
GToolbar.prototype.init = function () {
  ...
  for (var i = 0; i < this._groupedTools.length; ++i) {
    ...
    if (category != lastCategory) {
      // Add a divider, first
      $("<div></div>").addClass("divider").text(category).appendTo(toolpanel);
  ...
  gApp
    .getToolManager()
    .addEventListener(IFToolManager.ToolChangedEvent, this._toolChanged, this);
  gApp
    .getWindows()
...
GToolbar.prototype._updateZoomFromWindow = function () {
  ...
  zoom.find("*").prop("disabled", !window);

  if (window) {
    zoom
      .find("input")
      .val(ifUtil.formatNumber(window.getView().getZoom()) + "x");
  }
};
...
GToolbar.prototype._updateGroupTool = function (toolInstance) {
  ...
  var groupButton = this._htmlElement.find(
    '.toolpanel button[data-group="' + toolInfo.group + '"]',
  );
  ...
  groupButton
    ...
    .toggleClass(
      "g-active",
      toolInstance == gApp.getToolManager().getActiveTool(),
    )
    ...
    .on(
      ...
      function (evt) {
        ...
        if (groupTools.length > 1) {
          var overlayTimeout = setTimeout(
            function () {
              var panel = $("<div></div>").gOverlay({
                releaseOnClose: true,
              });
                  ...
                  .append(
                    $("<span></span>")
                      ...
                      .css(
                        "visibility",
                        toolInfo.instance !==
                          gApp.getToolManager().getActiveTool()
                          ? "hidden"
                          : "",
                      ),
                  )
          ...
          groupButton.off("mouseup").on("mouseup", function (evt) {
  ...
  if (groupTools.length > 1) {
    groupButton.append($("<span></span>").addClass("fa fa-caret-down"));
  }
};
