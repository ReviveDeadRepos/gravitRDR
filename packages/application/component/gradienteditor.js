...
function updateStop($this, $stop) {
  ...
  $stop
    .find(".stop-color")
    .css("background", $stop.data("stop-color").asCSSString());
...
function updateGradient($this) {
  ...
  $this
    .find(".gradient")
    .css("background", "linear-gradient(90deg, " + cssStops.join(", ") + ")");
...
var methods = {
  init: function (options) {
    ...
    return this.each(function () {
        ...
        .on("patterndrop", function (evt, pattern, mouseEvent) {
          if (pattern && pattern instanceof IFColor) {
            ...
            var percentPos =
              relativePos <= 0
                ? 0
                : relativePos >= stopsWidth
                  ? 100
                  : (relativePos / stopsWidth) * 100;
      ...
      var container = $("<div></div>").addClass("container").appendTo($this);
            ...
            $("<div></div>")
              ...
              .on("dblclick", function (evt) {
                ...
                if ($stops.hasClass("stops")) {
                  ...
                  var finalPosition = methods.insertStop.call(
                    self,
                    percentPos,
  ...
  insertStop: function (position, color) {
    ...
    var $stop = $("<div></div>")
      ...
      .append($("<div></div>").addClass("stop-color"))
      ...
      .on("mousedown", function (evt) {
        ...
        var docMouseMove = function (evt) {
          ...
          if (top > moveMaxY) {
            ...
            $stop.css("display", "none").data("stop-delete", true);
          } else {
            $stop.css("display", "").data("stop-delete", false);
          }
          ...
          var percentPos =
            relativePos <= 0
              ? 0
              : relativePos >= relativeMoveArea
                ? 100
                : (relativePos / relativeMoveArea) * 100;
        ...
        var docMouseUp = function (evt) {
          ...
          $document.off("mousemove", docMouseMove).off("mouseup", docMouseUp);
