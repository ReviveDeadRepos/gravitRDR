 ...
 * @version 1.0
 */
export function GMenuItem(type) {
  ...
  if (this._type === GMenuItem.Type.Divider) {
    ...
    this._htmlElement
      .append(
        $("<span></span>").addClass("g-menu-item-icon").css("display", "none"),
      )
  ...
  if (icon !== this._icon) {
    ...
    if (this._icon) {
      if (typeof this._icon === "string") {
        iconElement.append($("<i></i>").addClass(icon));
      } else {
...
GMenuItem.prototype.setCaption = function (caption) {
  if (caption !== this._caption) {
    ...
    if (
      !this._caption ||
      this._caption instanceof IFLocale.Key ||
      typeof this._caption === "string"
    ) {
...
GMenuItem.prototype.isRootItem = function () {
  return (
    this._parent &&
    this._parent instanceof GMenu &&
    this._parent._parent != null &&
    !(this._parent._parent instanceof GMenuItem)
  );
};
...
GMenuItem.prototype._mouseOver = function (evt) {
  ...
  if (
    this._type == GMenuItem.Type.Menu &&
...
GMenuItem.prototype._mouseDown = function (evt) {
  ...
  if (evt.button == GUIMouseEvent.BUTTON_LEFT) {
    ...
    if (
      this._type === GMenuItem.Type.Menu &&
      (this.isRootItem() || ifSystem.hardware !== IFSystem.Hardware.Desktop)
    ) {
      // Toggle our menu
