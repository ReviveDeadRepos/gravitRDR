...
function updateSelectedStyle($this, style) {
  $this.find(".style-block").each(function (index, element) {
    var $element = $(element);
    $element.toggleClass("selected", $element.data("style") === style);
  });
}
...
function canDrop($this, target) {
  ...
  if (dragStyle) {
    ...
    if (
      targetStyle &&
      (targetStyle !== dragStyle || ifPlatform.modifiers.shiftKey)
    ) {
...
var methods = {
  init: function (options) {
    ...
    return this.each(function () {
      ...
      var $this = $(this).addClass("g-style-panel").data("gstylepanel", {
        selected: null,
        options: options,
      });
  ...
  insertStyle: function (style, index) {
    ...
    var block = $("<div></div>")
      ...
      .append(
        $("<div></div>")
          .addClass("style-content")
          .append($("<div></div>").addClass("style-preview").append($("<img>")))
          .append($("<div></div>").addClass("style-name")),
      )
        ...
        .on("dragstart", function (evt) {
          ...
          if (data.options.allowDrag) {
            ...
            event.dataTransfer.setData(
              IFNode.MIME_TYPE,
              IFNode.serialize(style),
            );
            event.dataTransfer.setDragImage(
              block.find(".style-preview > img")[0],
              data.options.previewWidth / 2,
              data.options.previewHeight / 2,
            );
          }
        })
              ...
              IFEditor.tryRunTransaction(
                ...
                function () {
                  if (ifPlatform.modifiers.shiftKey) {
                    ...
                    styleClone.setProperty(
                      "name",
                      styleClone.getProperty("name") + "-copy",
                    );
                    parent.insertChild(
                      styleClone,
    ...
    $this.find(".style-block").each(function (index, element) {
      ...
      if ($element.data("style") === style) {
        $element
          ...
          .attr(
            "src",
            style.createPreviewImage(
              data.options.previewWidth,
              data.options.previewHeight,
            ),
          );
  ...
  attach: function (container) {
    ...
    if (container) {
      ...
      if (scene) {
        ...
        data.afterPropertiesChangeHandler =
          afterPropertiesChangeEvent.bind(this);
        scene.addEventListener(
          IFNode.AfterInsertEvent,
          data.afterInsertHandler,
        );
        scene.addEventListener(
          IFNode.BeforeRemoveEvent,
          data.beforeRemoveHandler,
        );
        scene.addEventListener(
        ...
        scene.addEventListener(
          IFStyle.StyleChangeEvent,
          data.styleChangeHandler,
        );
      }
    }
    return this;
