        ...
        .append(
          $("<div></div>")
            ...
            .append(
              $("<div></div>")
                .addClass("title")
                .append($("<i></i>").addClass("fa fa-angle-down"))
                .append($("<span></span>").text(options.title))
                .on("click", function () {
                  methods.toggle.call(self);
                }),
            )
            .append(
              $("<div></div>").addClass("controls").append(options.controls),
            ),
        )
        .append($("<div></div>").addClass("content").append(options.content));
    });
  },
  ...
  expanded: function (value) {
    ...
    if (!arguments.length) {
      ...
      if (value !== data.expanded) {
        ...
        $this
          .find(".controls")
          .css("visibility", data.expanded ? "" : "hidden");
        $this
          .find(".header i.fa")
