...
import { IFPattern } from "@gravitrdr/infinity-core";
import { IFSwatch } from "@gravitrdr/infinity-core";
    ...
    components: [
      {
        ...
        stops: function (components) {
          ...
          for (var i = 0; i <= 360; i += steps) {
            result.push(
              new IFColor(IFColor.Type.HSL, [i, hsla[1], hsla[2], 100]),
            );
          }
          return result;
        },
...
function createSwatchesView($this) {
  ...
  return $("<div></div>")
    .gSwatchPanel({
      nullSwatch: $("<span></span>").addClass("fa fa-plus-circle"),
      // TODO : I18N
    ...
    .on("swatchchange", function (evt, swatch) {
      if (!swatch) {
        ...
        for (
          var node = swatches.getFirstChild();
          node !== null;
          node = node.getNext()
        ) {
          if (
            node instanceof IFSwatch &&
            node.getPatternType() === IFPattern.Type.Color
          ) {
        ...
        // Ask for a name
        var name = prompt(
          "Enter a name for the new swatch:",
          data.color.asString(),
        );
        if (name === null) {
...
function createTrendsView($this) {
  var _addTrend = function (index, view) {
    ...
    $("<div></div>").addClass("color-trend-label").text(label).appendTo(view);
    ...
    var container = $("<div></div>")
      ...
      .append(
        $("<div></div>")
          ...
          .append($("<div></div>").addClass("color-preview")),
      )
      .append(
        $("<div></div>")
    ...
    for (var i = 1; i <= 10; ++i) {
      $("<div>&nbsp;</div>")
        .addClass(
          "g-input color-trend-box-" + (i === 10 ? "current" : i.toString()),
        )
        .css("width", "10%")
...
function createImageView($this) {
  return $("<div></div>")
            ...
            if (file.type.match(imageType)) {
              ...
              reader.onload = function (event) {
                ...
                image.onload = function () {
                  var mainColor = cvColorThiefColor(
                    ColorThief.getColorSync(image).array(),
                  );

                  var palette = ColorThief.getPaletteSync(image, {
                    colorCount: 8,
                  });
                  for (var i = 0; i < palette.length; ++i) {
    ...
    .append($("<div></div>").addClass("image-palette"));
}
  ...
  if (data.matcher && data.color) {
    ...
    if (matches && matches.length > 0) {
      ...
      for (var i = 0; i < len; ++i) {
        ...
        var match = data.color
          ? matches[i].toType(data.color.getType())
          : matches[i];
...
function activateColorMode($this, colorMode) {
  ...
  $this
    .find(".color-mode-select")
    .val(data.colorMode ? data.colorMode.type.key : IFColor.Type.RGB.key);
}
...
function updateToComponents($this) {
  ...
  if (components) {
    for (var i = 0; i < data.colorMode.components.length; ++i) {
      ...
      var val = Math.min(
        component.max,
        Math.max(component.min, components[i]),
      ).toFixed(0);
...
function assignValue($this, value, overwritePrevious) {
  ...
  $this
    .find(".current-color")
    .css("background", IFPattern.asCSSBackground(data.color));
  $this
    .find(".color-input")
    .val(data.color ? data.color.asHTMLHexString() : "");
  ...
  if (data.previousColor && data.color) {
    ...
    colorDiff =
      "&Delta;&nbsp;" +
      (diff < 0 ? ifUtil.formatNumber(diff, 2) : diff.toFixed(0));
  }
...
var methods = {
  init: function (options) {
    ...
    return this.each(function () {
      ...
      var $this = $(this).data("gcolorpanel", data);
      ...
      $this
              ...
              $("<div></div>")
                ...
                .append(
                  $("<button></button>")
                    ...
                    .css("display", "none")
                    .append($("<span></span>").addClass("fa fa-ban"))
                    .on("click", function () {
                ...
                .append(
                  $("<button></button>")
                    ...
                    // TODO : I18N
                    .attr("title", "System")
                    .append($("<span></span>").addClass("fa fa-cog"))
                    .on("click", function () {
              ...
              $("<div></div>")
                ...
                .append(
                  $("<button></button>")
                    ...
                    // TODO : I18N
                    .attr("title", "Palette")
                    .append($("<span></span>").addClass("fa fa-th")),
                )
                .append(
                  $("<button></button>")
                    ...
                    .append($("<span></span>").addClass("fa fa-bars")),
                )
                .append(
                  $("<button></button>")
                    ...
                    // TODO : I18N
                    .attr("title", "Trends")
                    .append($("<span></span>").addClass("fa fa-sliders")),
                )
                .append(
                  $("<button></button>")
                    ...
                    .append($("<span></span>").addClass("fa fa-image")),
                ),
            )
            .append(
              $("<div></div>")
        ...
        .append($("<div></div>").addClass("color-view"))
        .append($("<div></div>").addClass("color-components"))
        .append($("<hr/>"))
        .append(
          $("<div></div>")
            .addClass("color")
            .append(
              $("<div></div>")
                ...
                .append(
                  $("<div>&nbsp;</div>").addClass("current-color g-input"),
                )
                .append(
                  $("<div></div>")
              ...
              $("<div></div>").append(
                $("<select></select>")
                  ...
                  .on("change", function () {
                    activateMatcher(
                      $this,
                      $(this).find(":selected").data("matcher"),
                    );
                  }),
              ),
            ),
        )
        .append($("<div></div>").addClass("matcher-palette"));
      ...
      var _addComponent = function (index) {
        $("<div></div>")
          ...
          .append($("<div></div>").addClass("color-unit"))
          .appendTo(components);
      }.bind(this);
    ...
    if (!arguments.length) {
      ...
      if (value !== data.view) {
        ...
        $this.find("[data-activate-view]").each(function (index, element) {
          ...
          $element.toggleClass(
            "g-active",
            $element.attr("data-activate-view") === value,
          );
        });
        ...
        $this.find(".color-view > *").each(function (index, element) {
          ...
          $element.css(
            "display",
            $element.attr("data-view") === value ? "" : "none",
          );
        });
      }
      return this;
  ...
  allowClear: function (value) {
    ...
    if (!arguments.length) {
      ...
      $this.find('[data-action="clear"]').css("display", value ? "" : "none");
      return this;
    }
  },
  ...
  scene: function (value) {
    ...
    if (!arguments.length) {
      ...
      $this
        .find('[data-activate-view="' + ViewType.Swatches + '"]')
  ...
  value: function (value) {
    ...
    if (!arguments.length) {
      ...
      if (
        data.scene &&
        (!data.color ||
          data.color.getType() === IFColor.Type.Black ||
          data.color.getType() === IFColor.Type.White ||
          data.color.getType() === IFColor.Type.Registration)
      ) {
