...
function updateSelectedSwatch($this, swatch) {
  if ($this.data("gswatchpanel").options.allowSelect) {
    $this.find(".swatch-block").each(function (index, element) {
      var $element = $(element);
      $element.toggleClass("selected", $element.data("swatch") === swatch);
    });
  }
}
...
var methods = {
        ...
        .on("drop", function (evt) {
          ...
          if (sourcePattern) {
            ...
            if (pattern) {
              ...
              // TODO : I18N
              var sourceName =
                pattern instanceof IFColor ? pattern.asString() : "pattern";
  ...
  insertSwatch: function (swatch, index) {
    ...
    if (
      types !== null &&
      types.length > 0 &&
      types.indexOf(swatch.getPatternType()) < 0
    ) {
      return;
    }
    ...
    var block = $("<div></div>")
      ...
      .append(
        $("<div></div>")
          ...
          .append($("<div></div>").addClass("swatch-name")),
      )
        ...
        .on("dragstart", function (evt) {
          ...
          if (data.options.allowDrag) {
            ...
            event.dataTransfer.setData(
              IFPattern.MIME_TYPE,
              IFPattern.asString(pattern),
            );
            event.dataTransfer.setDragImage(
              block.find(".swatch-preview > div")[0],
    ...
    $this.find(".swatch-block").each(function (index, element) {
      ...
      if ($element.data("swatch") === swatch) {
        $element
          ...
          .css(
            "background",
            IFPattern.asCSSBackground(swatch.getProperty("pat")),
          );
