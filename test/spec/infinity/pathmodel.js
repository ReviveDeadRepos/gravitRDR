...
(function () {
  describe("GXPath", function () {
    ...
    it("#setCType updates corner type and shoulders length for all anchor points in the path", function () {
      var path = new GXPath();
      path.appendAnchorPoint(
        new GXPath.AnchorPoint(
          new IFPoint(10, 10),
          null,
          null,
          new IFPoint(15, 0),
      ...
      for (var i = 0; i < aCTypes.length; ++i, ++cx, ++cy) {
        ...
        for (
          var anchorPoint = path.getAnchorPoints().getFirstChild();
          anchorPoint != null;
    ...
    it("#setAuto updates handles to be auto or not for all anchor points in the path", function () {
      ...
      var anchorPoint;
      for (
        anchorPoint = path.getAnchorPoints().getFirstChild();
        anchorPoint != null;
        anchorPoint = anchorPoint.getNext()
      ) {
    ...
    describe("AnchorPoint", function () {
      ...
      describe("#setProperty with GXPath.AnchorPoint.PROPERTY_CTYPE", function () {
        ...
        it("Recalculate handles of anchor point for Connector type: Start point - path closed", function () {
          ...
          path
            .getAnchorPoints()
            .appendChild(new GXPath.AnchorPoint(new IFPoint(50, 70)));
          path
            .getAnchorPoints()
            .appendChild(new GXPath.AnchorPoint(new IFPoint(x1, y2)));
          path.setProperty(GXPath.PROPERTY_CLOSED, true);

          var aPt = path.getAnchorPoints().getFirstChild();
          aPt.setProperty(
            GXPath.AnchorPoint.PROPERTY_CTYPE,
            GXPath.AnchorPoint.CType.Connector,
          );
        ...
        it("Recalculate (not auto-)handles of anchor point for Smooth type: Start point - path closed", function () {
          ...
          path
            .getAnchorPoints()
            .appendChild(new GXPath.AnchorPoint(new IFPoint(x1 - 10, y2)));
          path.setProperty(GXPath.PROPERTY_CLOSED, true);
      ...
      describe("#setProperty with GXPath.AnchorPoint.PROPERTY_AUTO_HANDLES", function () {
        it("Updates $ah property", function () {
          ...
          for (var i = 0; i < anchorCTypes.length; ++i) {
            ...
            anchorPt.setProperty(
              GXPath.AnchorPoint.PROPERTY_AUTO_HANDLES,
              true,
            );
        ...
        it("Recalculate handles of anchor point for Smooth type: Start point - path closed", function () {
          ...
          path
            .getAnchorPoints()
            .appendChild(new GXPath.AnchorPoint(new IFPoint(x4, y4)));
          path.setProperty(GXPath.PROPERTY_CLOSED, true);
          ...
          dirLen = Math.sqrt(
            gMath.ptSqrDist(x1, y1, ccntr.getX(), ccntr.getY()),
          );
          dx = (y1 - ccntr.getY()) / dirLen;
          dy = (ccntr.getX() - x1) / dirLen;
          if (
            gMath.segmentSide(x1, y1, (x4 + x2) / 2, (y4 + y2) / 2, x4, y4) !=
            gMath.segmentSide(
              x1,
              y1,
              (x4 + x2) / 2,
        ...
        it("Recalculate handles of anchor point for Smooth type: Start point - path opened", function () {
          ...
          path
            .getAnchorPoints()
            .appendChild(new GXPath.AnchorPoint(new IFPoint(x1 - 10, -10)));
        ...
        it("Recalculate handles of Regular and styled corners: Start point - path opened", function () {
          ...
          path
            .getAnchorPoints()
            .appendChild(new GXPath.AnchorPoint(new IFPoint(x3, y3)));
          path
            .getAnchorPoints()
            .appendChild(new GXPath.AnchorPoint(new IFPoint(x1 - 10, -10)));
          ...
          dy = (ccntr.getX() - x1) / dirLen;
          if (
            gMath.segmentSide(x1, y1, (x2 + x3) / 2, (y2 + y3) / 2, x2, y2) !=
            gMath.segmentSide(
              x1,
              y1,
              (x2 + x3) / 2,
      ...
      it("setting corner type Smooth when auto-handles are marked works the same as setting auto handles for Smooth corner type", function () {
        expect(1).to.be.false;
      });
      describe("#transform", function () {
        ...
        it("Makes scaling of anchor point with handles and shoulders(if handles or neighbour points present)", function () {
          expect(1).to.be.false;
        });
    ...
    describe("AnchorPointContainer", function () {
      describe("#appendChild", function () {
        it("should add anchorPoint to container", function () {
          ...
          anchorPointOrig = new GXPath.AnchorPoint(
